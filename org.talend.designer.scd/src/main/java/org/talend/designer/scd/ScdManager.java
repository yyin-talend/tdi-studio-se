...
public class ScdManager {
	...
	private static final String GENERATE_COLUMN = "Column generated by scd editor.";
	...
	public List<IMetadataColumn> getInputColumns(INode node) {
		...
		if (incomingConnections != null && incomingConnections.size() > 0) {
			for (IConnection incomingConnection : incomingConnections) {
				if (incomingConnection.getLineStyle().hasConnectionCategory(
						IConnectionCategory.DATA)) {
					IMetadataTable schemaTable = incomingConnection
							.getMetadataTable();
					if (schemaTable != null) {
	...
	public void restoreUIData() {
		List<? extends IElementParameter> list = component
	 ...
	 * @return
	 */
	private Collection<? extends String> getUsedType3Columns(
			List<Type3Field> type3) {
		if (type3 == null) {
			return Collections.EMPTY_LIST;
		} else {
			List<String> columns = new ArrayList<String>();
	...
	private Collection<? extends String> getUsedColumns(Versioning ver) {
		if (ver == null) {
			...
			if (ver.getStartType() == VersionStartType.INPUT_FIELD
					&& StringUtils.isNotEmpty(ver.getStartComplement())) {
	...
	public void reloadSourceKeyParameter() {
		IElementParameter param = paramsMap
				.get(ScdParameterConstants.SOURCE_KEYS_PARAM_NAME);
		List<Map<String, String>> values = (List<Map<String, String>>) param
				.getValue();
	 ...
	 * @return
	 */
	private List<String> convertTableParameterValue(
	...
	public void saveUIData(List<String> unusedFields, List<String> sourceKeys,
			List<SurrogateKey> surrogateKeys, List<String> type0Table,
			List<String> type1Table, List<String> type2Table,
			Versioning versionData, List<Type3Field> type3Table) {
	...
	private void setUpdateComponent(List<? extends IElementParameter> list) {
		...
		for (IElementParameter elementParameter : list) {
			if (EParameterName.UPDATE_COMPONENTS.getName().equals(
	...
	public void updateSourceKeyParameter(List<String> keys) {
		IElementParameter param = paramsMap
				.get(ScdParameterConstants.SOURCE_KEYS_PARAM_NAME);
		List<Map<String, String>> values = createTableFieldValues(
				ScdParameterConstants.SOURCE_KEYS_ITEM_NAME, keys);
	 ...
	 * @return
	 */
	private List<Map<String, String>> createTableFieldValues(String itemName,
			List<String> keys) {
	...
	public void reloadType0Parameter() {
		...
		if (useL0 != null && useL0.getValue().equals(Boolean.TRUE)) {
			IElementParameter l0FieldsParam = paramsMap
					.get(ScdParameterConstants.L0_FIELDS_PARAM_NAME);
			List<Map<String, String>> values = (List<Map<String, String>>) l0FieldsParam
					.getValue();
	...
	public void reloadType1Parameter() {
		...
		if (useL1 != null && useL1.getValue().equals(Boolean.TRUE)) {
			IElementParameter l1FieldsParam = paramsMap
					.get(ScdParameterConstants.L1_FIELDS_PARAM_NAME);
			List<Map<String, String>> values = (List<Map<String, String>>) l1FieldsParam
					.getValue();
	...
	public void reloadType2Parameter() {
		...
		if (useL2 != null && useL2.getValue().equals(Boolean.TRUE)) {
			...
			IElementParameter l2FieldsParam = paramsMap
					.get(ScdParameterConstants.L2_FIELDS_PARAM_NAME);
			List<Map<String, String>> values = (List<Map<String, String>>) l2FieldsParam
					.getValue();
			type2Table = convertTableParameterValue(values);

			// start date
			versionData
					.setStartName(getStringParameter(ScdParameterConstants.L2_STARTDATE_FIELD));

			IElementParameter param = paramsMap
					.get(ScdParameterConstants.L2_STARTDATE_VALUE);
			if (param != null) {
				versionData.setStartType(VersionStartType
						.getTypeByValue((String) param.getValue()));

				if (versionData.getStartType() == VersionStartType.INPUT_FIELD) {
					versionData
							.setStartComplement(getStringParameter(ScdParameterConstants.L2_STARTDATE_INPUT_FIELD));
				}
			}

			// end date
			versionData
					.setEndName(getStringParameter(ScdParameterConstants.L2_ENDDATE_FIELD));

			param = paramsMap.get(ScdParameterConstants.L2_ENDDATE_VALUE);
			if (param != null) {
				versionData.setEndType(VersionEndType
						.getTypeByValue((String) param.getValue()));

				if (versionData.getEndType() == VersionEndType.FIXED_YEAR) {
					versionData
							.setEndComplement(getStringParameter(ScdParameterConstants.L2_ENDDATE_FIXED_VALUE));
				}
			}

			// version
			versionData
					.setVersionChecked(getBooleanParameter(ScdParameterConstants.USE_L2_VERSION));
			if (versionData.isVersionChecked()) {
				versionData
						.setVersionName(getStringParameter(ScdParameterConstants.L2_VERSION_FIELD));
			}

			// activate
			versionData
					.setActiveChecked(getBooleanParameter(ScdParameterConstants.USE_L2_ACTIVE));
			if (versionData.isActiveChecked()) {
				versionData
						.setActiveName(getStringParameter(ScdParameterConstants.L2_ACTIVE_FIELD));
			}
	...
	public void reloadType3Parameter() {
		...
		if (useL3 != null && useL3.getValue().equals(Boolean.TRUE)) {
			IElementParameter l3FieldsParam = paramsMap
					.get(ScdParameterConstants.L3_FIELDS_PARAM_NAME);

			type3Table = new ArrayList<Type3Field>();
			List<Map<String, String>> values = (List<Map<String, String>>) l3FieldsParam
					.getValue();
			for (Map<String, String> entry : values) {
				String current = entry
						.get(ScdParameterConstants.L3_ITEM_CURRENT_VALUE);
				String previous = entry
						.get(ScdParameterConstants.L3_ITEM_PREV_VALUE);
	...
	private void reloadSurrogateKeyParameter() {
		...
		key
				.setCreation(SurrogateCreationType
						.getTypeByValue(getStringParameter(ScdParameterConstants.SK_CREATION)));

		if (key.getCreation() == SurrogateCreationType.INPUT_FIELD) {
			key
					.setComplement(getStringParameter(ScdParameterConstants.SK_INPUT_FIELD));
		} else if (key.getCreation() == SurrogateCreationType.ROUTINE) {
			key
					.setComplement(getStringParameter(ScdParameterConstants.SK_ROUTINE));
		}
	}
	...
	public void updateType0Parameter(List<String> keys) {
		...
		if (keys == null || keys.size() == 0) {
			...
			List<Map<String, String>> values = createTableFieldValues(
					ScdParameterConstants.L0_FIELDS_ITEM_NAME, keys);
	...
	public void updateType1Parameter(List<String> keys) {
		...
		if (keys == null || keys.size() == 0) {
			...
			List<Map<String, String>> values = createTableFieldValues(
					ScdParameterConstants.L1_FIELDS_ITEM_NAME, keys);
	...
	public void updateType2Parameter(List<String> keys, Versioning ver) {
		...
		if (keys == null || keys.size() == 0) {
			...
			// start date
			updateElementParameter(ScdParameterConstants.L2_STARTDATE_FIELD,
					ver.getStartName());

			updateElementParameter(ScdParameterConstants.L2_STARTDATE_VALUE,
					ver.getStartType().getValue());

			if (ver.getStartType() == VersionStartType.INPUT_FIELD) {
				updateElementParameter(
						ScdParameterConstants.L2_STARTDATE_INPUT_FIELD, ver
								.getStartComplement());
			}

			// end date
			updateElementParameter(ScdParameterConstants.L2_ENDDATE_FIELD, ver
					.getEndName());

			updateElementParameter(ScdParameterConstants.L2_ENDDATE_VALUE, ver
					.getEndType().getValue());

			if (ver.getEndType() == VersionEndType.FIXED_YEAR) {
				updateElementParameter(
						ScdParameterConstants.L2_ENDDATE_FIXED_VALUE, ver
								.getEndComplement());
			}

			// version
			updateElementParameter(ScdParameterConstants.USE_L2_VERSION, ver
					.isVersionChecked());

			if (ver.isVersionChecked()) {
				updateElementParameter(ScdParameterConstants.L2_VERSION_FIELD,
						ver.getVersionName());
			}

			// activate
			updateElementParameter(ScdParameterConstants.USE_L2_ACTIVE, Boolean
					.valueOf(ver.isActiveChecked()));

			if (ver.isActiveChecked()) {
				updateElementParameter(ScdParameterConstants.L2_ACTIVE_FIELD,
						ver.getActiveName());
			}
		}
	}
	...
	public void updateType3Parameter(List<Type3Field> model) {
		...
		if (model == null || model.size() == 0) {
			...
			for (Type3Field field : model) {
				Map<String, String> row = new HashMap<String, String>();
				row.put(ScdParameterConstants.L3_ITEM_CURRENT_VALUE, field
						.getCurrentValue());
				row.put(ScdParameterConstants.L3_ITEM_PREV_VALUE, field
						.getPreviousValue());
				table.add(row);
			}

			updateElementParameter(ScdParameterConstants.L3_FIELDS_PARAM_NAME,
					table);
		}
	}
	...
	public void updateSurrogateKeyParameter(SurrogateKey key) {
		...
		updateElementParameter(ScdParameterConstants.SURROGATE_KEY, key
				.getColumn());

		updateElementParameter(ScdParameterConstants.SK_CREATION, key
				.getCreation().getValue());

		if (key.getCreation() == SurrogateCreationType.INPUT_FIELD) {
			updateElementParameter(ScdParameterConstants.SK_INPUT_FIELD, key
					.getComplement());
		} else if (key.getCreation() == SurrogateCreationType.ROUTINE) {
			updateElementParameter(ScdParameterConstants.SK_ROUTINE, key
					.getComplement());
		}
	 ...
	 * @return
	 */
	private Map<String, IMetadataColumn> removeUnusedAndGeneratedColumns(
			List<IMetadataTable> metadataList, List<String> unusedFields) {
		Map<String, IMetadataColumn> columnsMap = new HashMap<String, IMetadataColumn>();
		if (metadataList != null) {
			for (IMetadataTable table : metadataList) {
				List<IMetadataColumn> columns = new ArrayList<IMetadataColumn>(
						table.getListColumns());
				// remove from backend
				for (int i = columns.size() - 1; i >= 0; i--) {
					IMetadataColumn col = columns.get(i);
					if (unusedFields.contains(col.getLabel())
							|| GENERATE_COLUMN.equals(col.getComment())) {
	...
	public void createOutputSchema() {
		...
		// output schema
		Map<String, IMetadataColumn> outputColumns = removeUnusedAndGeneratedColumns(
				component.getMetadataList(), unusedFields);
		...
		for (IMetadataColumn column : inputColumns) {
			...
			if (!unusedFields.contains(column.getLabel())
					&& outputColumns.get(column.getLabel()) == null) {
				schema.getListColumns().add(
						cloneColumn(column, column.getLabel()));
			}
		}
		...
		if (type3Table != null) {
			for (Type3Field type3 : type3Table) {
				if (outputColumns.get(type3.getPreviousValue()) != null
						|| StringUtils.isEmpty(type3.getPreviousValue())) {
					continue;
				}
				IMetadataColumn column = inputColumnsMap.get(type3
						.getCurrentValue());
				IMetadataColumn previous = cloneColumn(column, type3
						.getPreviousValue());
		...
		if (type2Table != null && !type2Table.isEmpty()) {
			// start date
			createMetadataColumn(outputColumns, schema, versionData
					.getStartName(), Date.class, lang);
			// end date
			IMetadataColumn endColumn = createMetadataColumn(outputColumns,
					schema, versionData.getEndName(), Date.class, lang);
			if (endColumn != null) {
				endColumn.setNullable(true);
			}
			if (versionData.isVersionChecked()) {
				createMetadataColumn(outputColumns, schema, versionData
						.getVersionName(), Integer.class, lang);
			}
			if (versionData.isActiveChecked()) {
				createMetadataColumn(outputColumns, schema, versionData
						.getActiveName(), Boolean.class, lang);
			}
		}
		fixKeyColumnsInOutputSchema(schema, inputColumnsMap, lang);
		// sort column by name
		Collections.sort(schema.getListColumns(),
				new Comparator<IMetadataColumn>() {
	 ...
	 * @param lang
	 */
	private void fixKeyColumnsInOutputSchema(IMetadataTable schema,
			Map<String, IMetadataColumn> inputColumnsMap, ECodeLanguage lang) {
		Map<String, IMetadataColumn> columnsMap = new HashMap<String, IMetadataColumn>();
		...
		if (surrogateKeys != null && !surrogateKeys.isEmpty()) {
			for (SurrogateKey key : surrogateKeys) {
				...
				if (column == null) {
					...
					if (key.getCreation() == SurrogateCreationType.INPUT_FIELD) {
						IMetadataColumn inputCol = inputColumnsMap.get(key
								.getComplement());
						if (inputCol != null) {
						...
						column = createMetadataColumn(columnsMap, schema, key
								.getColumn(), Integer.class, lang);
						column.setKey(true); // set as key in output schema
	 ...
	 * @param strings
	 */
	private void removeVersionFieldsFromOutput(
			Map<String, IMetadataColumn> outputColumns, IMetadataTable schema,
			String[] fields) {
	 ...
	 * @return
	 */
	private IMetadataColumn createMetadataColumn(
			Map<String, IMetadataColumn> outputColumns, IMetadataTable schema,
			String name, Class clazz, ECodeLanguage lang) {
