...
public class RunProcessContext {
	...
	public RunProcessContext(IProcess process) {
		...
		setMonitorPerf(RunProcessPlugin.getDefault().getPreferenceStore()
				.getBoolean(RunProcessPrefsConstants.ISSTATISTICSRUN));
		setMonitorTrace(RunProcessPlugin.getDefault().getPreferenceStore()
				.getBoolean(RunProcessPrefsConstants.ISTRACESRUN));
		setWatchAllowed(RunProcessPlugin.getDefault().getPreferenceStore()
				.getBoolean(RunProcessPrefsConstants.ISEXECTIMERUN));
		setSaveBeforeRun(RunProcessPlugin.getDefault().getPreferenceStore()
	...
	private void showErrorMassage(String category) {
		String title = Messages.getString(
				"RunProcessContext.PortErrorTitle", category); //$NON-NLS-1$
		String message = Messages.getString(
				"RunProcessContext.PortErrorMessage", category.toLowerCase()); //$NON-NLS-1$
	...
	private void testPort() {
		...
		if (monitorPerf) {
			...
			if (getStatisticsPort() == IProcessor.NO_STATISTICS) {
				showErrorMassage(Messages
						.getString("RunProcessContext.PortErrorStats")); //$NON-NLS-1$
				// disable the check.
		...
		if (monitorTrace) {
			...
			if (getTracesPort() == IProcessor.NO_TRACES) {
				showErrorMassage(Messages
						.getString("RunProcessContext.PortErrorTraces")); //$NON-NLS-1$
				// disable the check.
			...
			try {
				progressService.run(false, true, new IRunnableWithProgress() {
					...
					public void run(final IProgressMonitor monitor) {
						...
						final EventLoopProgressMonitor progressMonitor = new EventLoopProgressMonitor(
								monitor);
						...
						try {
							...
							if (monitorPerf) {
								...
								new Thread(perfMonitor, "PerfMonitor_"
										+ process.getLabel()).start();
							}
							// findNewTracesPort();
							if (monitorTrace) {
								traceMonitor = new TraceMonitor();
								new Thread(traceMonitor, "TraceMonitor_"
										+ process.getLabel()).start();
							}

							final String watchParam = RunProcessContext.this
									.isWatchAllowed() ? WATCH_PARAM : null;
							processor.setContext(context);
							processor
									.setTargetExecutionConfig(getSelectedTargetExecutionConfig());
	...
	public void addErrorMessage(Throwable e) {
		...
		IProcessMessage processMsg = new ProcessMessage(MsgType.CORE_ERR,
				message.toString());
		processMessageManager.addMessage(processMsg);
	}

	private void findNewStatsPort() {
		statsPort = monitorPerf ? RunProcessPlugin.getDefault()
				.getRunProcessContextManager().getPortForStatistics()
	...
	private void findNewTracesPort() {
		tracesPort = monitorTrace ? RunProcessPlugin.getDefault()
				.getRunProcessContextManager().getPortForTraces()
	...
	protected class ProcessMonitor implements IProcessMonitor {
		...
		public ProcessMonitor(Process ps) {
			...
			this.process = ps;
			this.outIs = new BufferedReader(new InputStreamReader(ps
					.getInputStream()));
			this.errIs = new BufferedReader(new InputStreamReader(ps
					.getErrorStream()));
		}
		...
		private boolean extractMessages(boolean flush) {
			...
			try {
				...
				if (messageErr != null) {
					if (messageErr.getContent().contains(
		 ...
		 *             Extraction failure.
		 */
		private IProcessMessage extractMessage(final BufferedReader is,
				MsgType type, boolean flush) throws IOException {
			...
			if (is.ready()) {
				...
				while (is.ready()) {
					...
					if (sb.length() > 1024
							|| System.currentTimeMillis() - timeStart > 100) {
						break;
					}
				}
		...
		public void run() {
			...
			if (processSocket != null && !stopThread) {
				try {
					...
					while (!stopThread) {
						final String data = reader.readLine();
						PrintWriter pred = new java.io.PrintWriter(
	...
	public void setSelectedTargetExecutionConfig(
			ITargetExecutionConfig selectedTargetExecutionConfiguration) {
	...
	public void setDebugProcess(
			org.eclipse.debug.core.model.IProcess debugProcess) {
