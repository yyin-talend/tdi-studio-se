...
public class ProcessComposite extends Composite {
	...
	private void initGraphicComponents(Composite parent) {
		...
		contextTabItem.setText(Messages
				.getString("ProcessComposite.contextTab")); //$NON-NLS-1$
		contextComposite = new ProcessContextComposite(leftTabFolder, SWT.NONE);
		contextComposite.setBackground(leftTabFolder.getDisplay()
		...
		targetExecutionComposite.setBackground(leftTabFolder.getDisplay()
				.getSystemColor(SWT.COLOR_WHITE));

		targetExecutionTabItem = new CTabItem(leftTabFolder, SWT.BORDER);
		targetExecutionTabItem.setText(Messages
				.getString("ProcessComposite.targetExecutionTab")); //$NON-NLS-1$
		...
		ScrolledComposite execScroll = new ScrolledComposite(execGroup,
		...
		debugBtn.setToolTipText(Messages
				.getString("ProcessComposite.debugHint")); //$NON-NLS-1$
		debugBtn.setImage(ImageProvider.getImage(RunProcessPlugin
				.imageDescriptorFromPlugin(RunProcessPlugin.PLUGIN_ID,
		...
		execBtn.setImage(ImageProvider
				.getImage(ERunprocessImages.RUN_PROCESS_ACTION));
		...
		killBtn.setImage(ImageProvider.getImage(RunProcessPlugin
				.imageDescriptorFromPlugin(RunProcessPlugin.PLUGIN_ID,
		...
		saveJobBeforeRunButton.setText(Messages
				.getString("ProcessComposite.saveBeforeRun")); //$NON-NLS-1$
		saveJobBeforeRunButton.setToolTipText(Messages
				.getString("ProcessComposite.saveBeforeRunHint")); //$NON-NLS-1$
		...
		clearBeforeExec.setText(Messages
				.getString("ProcessComposite.clearBefore")); //$NON-NLS-1$
		clearBeforeExec.setToolTipText(Messages
				.getString("ProcessComposite.clearBeforeHint")); //$NON-NLS-1$
		...
		watchBtn.setToolTipText(Messages
				.getString("ProcessComposite.execTimeHint")); //$NON-NLS-1$
		watchBtn.setEnabled(false);
		watchBtn.setSelection(RunProcessPlugin.getDefault()
				.getPreferenceStore().getBoolean(
		...
		statisticsComposite.setText(Messages
				.getString("ProcessComposite2.statsComposite")); //$NON-NLS-1$
		...
		Composite statisticsButtonComposite = new Composite(
				statisticsComposite, SWT.NONE);
		layout = new GridLayout(1, false);
		layout.marginWidth = 0;
		statisticsButtonComposite.setLayout(layout);
		statisticsButtonComposite.setLayoutData(new GridData(
		...
		traceBtn.setToolTipText(Messages
				.getString("ProcessComposite.traceHint")); //$NON-NLS-1$
		traceBtn.setEnabled(false);
		traceBtn.setSelection(RunProcessPlugin.getDefault()
				.getPreferenceStore().getBoolean(
						RunProcessPrefsConstants.ISTRACESRUN));
		...
		clearTracePerfBtn.setToolTipText(Messages
				.getString("ProcessComposite.clearHint")); //$NON-NLS-1$
		...
		consoleText = new StyledText(execContent, SWT.BORDER | SWT.WRAP
		...
		execScroll
				.setMinSize(execContent.computeSize(SWT.DEFAULT, SWT.DEFAULT));
		...
		streamListener = new IStreamListener() {
			...
			public void streamAppended(String text, IStreamMonitor monitor) {
				IProcessMessage message = new ProcessMessage(
						ProcessMessage.MsgType.STD_OUT, text);
	...
	protected void addListeners() {
		...
		saveJobBeforeRunButton.addSelectionListener(new SelectionAdapter() {
			...
			@Override
			public void widgetSelected(SelectionEvent e) {
				processContext.setSaveBeforeRun(saveJobBeforeRunButton
						.getSelection());
			}
		});
		...
		clearBeforeExec.addSelectionListener(new SelectionAdapter() {
			...
			@Override
			public void widgetSelected(SelectionEvent e) {
				processContext.setClearBeforeExec(clearBeforeExec
						.getSelection());
			}
		});
	}
	...
	public void setProcessContext(RunProcessContext processContext) {
		...
		clearBeforeExec.setSelection(processContext != null
				&& processContext.isClearBeforeExec());
		contextComposite
				.setProcess(((processContext != null) && !disableAll ? processContext
						.getProcess()
						: null));
		fillConsole(processContext != null ? processContext.getMessages()
				: new ArrayList<IProcessMessage>());
	}
	...
	private void setExecBtn(final boolean runnable) {
		if (traceBtn.getSelection()) {
			...
			if (!runnable && b) {
				...
				execBtn.setText(Messages.getString("ProcessComposite.exec")); //$NON-NLS-1$
				execBtn.setData(ProcessView.EXEC_ID);
				execBtn.setToolTipText(Messages
						.getString("ProcessComposite.execHint")); //$NON-NLS-1$
				execBtn.setImage(ImageProvider
						.getImage(ERunprocessImages.RUN_PROCESS_ACTION));
			}
	...
	private void appendToConsole(final IProcessMessage message) {
		getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				...
				if (message.getType() == MsgType.CORE_OUT
						|| message.getType() == MsgType.CORE_ERR) {
	...
	private boolean setConsoleFont() {
		IPreferenceStore preferenceStore = RunProcessPlugin.getDefault()
				.getPreferenceStore();
		String fontType = preferenceStore
				.getString(RunProcessPrefsConstants.CONSOLE_FONT);
	...
	private void addInHistoryRunningList() {
		...
		if (CorePlugin.getDefault().getDesignerCoreService().isTalendEditor(
				activeEditor)) {
	...
	public void exec() {
		...
		CorePlugin.getDefault().getDesignerCoreService().saveJobBeforeRun(
				getProcessContext().getProcess());
		...
		processContext
				.setSelectedContext(contextComposite.getSelectedContext());
	...
	public void debug() {
		...
		if ((processContext.getProcess()) instanceof org.talend.designer.core.ui.editor.process.Process) {
			((org.talend.designer.core.ui.editor.process.Process) processContext
					.getProcess()).checkDifferenceWithRepository();
		}

		final IPreferenceStore preferenceStore = DebugUIPlugin.getDefault()
				.getPreferenceStore();
		...
		if (contextComposite.promptConfirmLauch()) {
			...
			IRunnableWithProgress worker = new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor) {
					IProcessor processor = ProcessorUtilities.getProcessor(
							processContext.getProcess(), context);
					monitor.beginTask(
							"Launching debugger", IProgressMonitor.UNKNOWN); //$NON-NLS-1$
					try {
						// use this function to generate childrens also.
						ProcessorUtilities.generateCode(processContext
								.getProcess(), context, false, false, true);

						// For bug 5430, modifyed by xzhang
						processContext.startPerformanceMonitor();

						ILaunchConfiguration config = ((Processor) processor)
								.getDebugConfiguration(processContext
										.getStatisticsPort(), processContext
										.getTracesPort(), null);

						// see feature 0004820: The run job doesn't verify if
						// code is correct before launching
						if (!JobErrorsChecker.hasErrors(ProcessComposite.this
								.getShell())) {
							...
							if (config != null) {
								...
								MessageDialog
										.openInformation(
												getShell(),
												Messages
														.getString("ProcessDebugDialog.debugBtn"), //$NON-NLS-1$
												Messages
														.getString("ProcessDebugDialog.errortext")); //$NON-NLS-1$ 
							}
						}
						...
						MessageDialog.openError(getShell(), Messages
								.getString("ProcessDebugDialog.debugBtn"), ""); //$NON-NLS-1$ //$NON-NLS-2$
					} finally {
						monitor.done();
					}
				}
			...
			try {
				progressService.runInUI(PlatformUI.getWorkbench()
						.getProgressService(), worker, ResourcesPlugin
		...
		try {
			Thread thread = new Thread() {
				...
				public void run() {
					while (debugMode) {
						final IProcess process = DebugUITools
								.getCurrentProcess();
						if (process != null && process.isTerminated()) {
							getDisplay().asyncExec(new Runnable() {
								...
								public void run() {
									...
									preferenceStore
											.setValue(
													IDebugPreferenceConstants.CONSOLE_OPEN_ON_OUT,
																...
																.format(new Object[] {
																		...
																		new Integer(
																				process
																						.getExitValue()) });
												processContext
														.addDebugResultToConsole(new ProcessMessage(
																MsgType.CORE_OUT,
																byeMsg));
	...
	private void checkSaveBeforeRunSelection() {
		if (saveJobBeforeRunButton.getSelection()) {
			SaveJobBeforeRunAction action = new SaveJobBeforeRunAction(
	...
	private void runProcessContextChanged(final PropertyChangeEvent evt) {
		...
		if (ProcessMessageManager.PROP_MESSAGE_ADD.equals(propName)
				|| ProcessMessageManager.PROP_DEBUG_MESSAGE_ADD
						.equals(propName)) {
