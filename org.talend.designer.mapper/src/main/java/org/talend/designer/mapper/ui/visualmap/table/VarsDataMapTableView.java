...
public class VarsDataMapTableView extends DataMapTableView {
	...
	public VarsDataMapTableView(Composite parent, int style,
			VarsTable abstractDataMapTable, MapperManager mapperManager) {
	...
	protected void addListeners() {
				...
				.addAfterSelectionListener(new ILineSelectionListener() {
					...
					public void handle(LineSelectionEvent e) {
						if (!getMapperManager().componentIsReadOnly()) {
							boolean atLeastOneItemIsSelected = tableViewerCreatorForColumns
									.getTable().getSelectionCount() > 0;
							removeEntryItem
									.setEnabled(atLeastOneItemIsSelected);
							moveUpEntryItem
									.setEnabled(atLeastOneItemIsSelected);
							moveDownEntryItem
									.setEnabled(atLeastOneItemIsSelected);
		...
		getExtendedTableViewerForColumns().getExtendedTableModel()
					...
					public void handleEvent(ListenableListEvent event) {
						if (event.type == ListenableListEvent.TYPE.SWAPED) {
							DataMapTableView varsDataMapTableView = mapperManager
									.retrieveDataMapTableView(getExtendedTableViewerForColumns()
											.getTable());
							UIManager uiManager = mapperManager.getUiManager();
							uiManager.parseAllExpressions(varsDataMapTableView,
									false);
							mapperManager.getProblemsManager()
									.checkProblemsForAllEntries(
											varsDataMapTableView, true);
							uiManager.refreshBackground(true, false);
							List<ITableEntry> list = uiManager
									.extractSelectedTableEntries(varsDataMapTableView
											.getTableViewerCreatorForColumns()
											.getTableViewer().getSelection());

							uiManager.selectLinks(varsDataMapTableView, list,
									false, false);
						}
					}
	...
	public void initColumnsOfTableColumns(
		...
		ECodeLanguage codeLanguage = LanguageProvider.getCurrentLanguage()
				.getCodeLanguage();

		TableViewerCreatorColumn column = new TableViewerCreatorColumn(
				tableViewerCreatorForColumns);
		column.setTitle(Messages
				.getString("VarsDataMapTableView.columnTitle.expression")); //$NON-NLS-1$
		...
		final ExtendedTextCellEditor expressionCellEditor = createExpressionCellEditor(
				tableViewerCreatorForColumns, column, new Zone[] { Zone.INPUTS,
						Zone.VARS }, false);
		column
				.setBeanPropertyAccessors(new IBeanPropertyAccessors<VarTableEntry, String>() {
		...
		if (codeLanguage == ECodeLanguage.JAVA) {
			...
			CellEditorValueAdapter comboValueAdapter = new JavaTypeComboValueAdapter(
					JavaTypesManager.getDefaultJavaType(), nullableAccessors);

			column = new TableViewerCreatorColumn(tableViewerCreatorForColumns);
			column.setTitle(Messages
					.getString("VarsDataMapTableView.columnTitle.type")); //$NON-NLS-1$
			column
					.setBeanPropertyAccessors(new IBeanPropertyAccessors<VarTableEntry, String>() {
						...
						public void set(VarTableEntry bean, String value) {
							...
							mapperManager
									.getProblemsManager()
									.checkProblemsForAllEntriesOfAllTables(true);
						}
			...
			column.setCellEditor(new ComboBoxCellEditor(
					tableViewerCreatorForColumns.getTable(), arrayTalendTypes),
					comboValueAdapter);

			column = new TableViewerCreatorColumn(tableViewerCreatorForColumns);
			column.setTitle("Nullable");
			column
					.setBeanPropertyAccessors(new IBeanPropertyAccessors<VarTableEntry, Boolean>() {
		...
		column = new TableViewerCreatorColumn(tableViewerCreatorForColumns);
		column.setTitle(Messages
				.getString("VarsDataMapTableView.columnTitle.variable")); //$NON-NLS-1$
		...
		final TextCellEditor cellEditor = new TextCellEditor(
				tableViewerCreatorForColumns.getTable());
		cellEditor.addListener(new DialogErrorForCellEditorListener(cellEditor,
				column) {

			@Override
			public void newValidValueTyped(int itemIndex, Object previousValue,
					Object newValue, CELL_EDITOR_STATE state) {
				if (state == CELL_EDITOR_STATE.APPLYING) {
					ModifiedObjectInfo modifiedObjectInfo = tableViewerCreatorForColumns
							.getModifiedObjectInfo();
					String originalValue = (String) modifiedObjectInfo
							.getOriginalPropertyBeanValue();
					Object currentModifiedBean = modifiedObjectInfo
							.getCurrentModifiedBean();
					mapperManager.getUiManager().processColumnNameChanged(
							originalValue.toString(), newValue.toString(),
							VarsDataMapTableView.this,
			...
			public String validateValue(String newValue, int beanPosition) {
				return ((VarsTable) getDataMapTable()).validateColumnName(
	...
	protected void createToolItems() {
		...
		addEntryItem
				.setToolTipText(Messages
						.getString("VarsDataMapTableView.entryItemTooltip.addVariable")); //$NON-NLS-1$
		addEntryItem.setImage(org.talend.commons.ui.image.ImageProvider
				.getImage(org.talend.commons.ui.image.ImageProvider
		...
		addEntryItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				IDataMapTable dataMapTable = VarsDataMapTableView.this
						.getDataMapTable();
				String varName = null;
				if (dataMapTable instanceof VarsTable) {
					varName = ((VarsTable) dataMapTable)
							.findUniqueColumnName("var"); //$NON-NLS-1$
				} else {
				...
				ILanguage currentLanguage = LanguageProvider
						.getCurrentLanguage();
				...
				mapperManager.addNewVarEntry(VarsDataMapTableView.this,
						varName, indexInsert, type); //$NON-NLS-1$
				VarsDataMapTableView.this.changeSize(VarsDataMapTableView.this
						.getPreferredSize(true, true, false), true, true);
		...
		removeEntryItem
				.setToolTipText(Messages
						.getString("VarsDataMapTableView.entryItemTooltip.removeVariable")); //$NON-NLS-1$
		...
		removeEntryItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				IStructuredSelection selection = (IStructuredSelection) tableViewerCreatorForColumns
						.getTableViewer().getSelection();
		...
		moveUpEntryItem
				.setToolTipText(Messages
						.getString("VarsDataMapTableView.entryItemTooltip.moveUpVariable")); //$NON-NLS-1$
		...
		moveUpEntryItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				ExtendedTableMoveCommand moveCommand = new ExtendedTableMoveCommand(
						viewer.getExtendedTableModel(), true, viewer
		...
		moveDownEntryItem
				.setToolTipText(Messages
						.getString("VarsDataMapTableView.entryItemTooltip.movedownVariable")); //$NON-NLS-1$
		...
		moveDownEntryItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				AbstractExtendedTableViewer viewer = getExtendedTableViewerForColumns();
				ExtendedTableMoveCommand moveCommand = new ExtendedTableMoveCommand(
						viewer.getExtendedTableModel(), false, viewer
								.getTableViewerCreator().getTable()
								.getSelectionIndices());
				viewer.executeCommand(moveCommand);
			}
