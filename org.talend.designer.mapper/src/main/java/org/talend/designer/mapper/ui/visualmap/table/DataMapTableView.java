...
public abstract class DataMapTableView extends Composite {
	 ...
	 * @param mapperManager
	 */
	public DataMapTableView(Composite parent, int style,
	...
	private void createComponents() {
		...
		final Color listBackground = display
		...
		nameLabel
				.setFont(FontProviderMapper.getFont(FontInfo.FONT_SYSTEM_BOLD));
		...
		if (mapperManager.isAdvancedMap()
				&& this instanceof OutputDataMapTableView) {
	...
	protected void createTableForColumns() {
		this.extendedTableViewerForColumns = new AbstractExtendedTableViewer<IColumnEntry>(
				abstractDataMapTable.getTableColumnsEntriesModel(),
				centerComposite) {

			@Override
			protected void createColumns(
					TableViewerCreator<IColumnEntry> tableViewerCreator,
					Table table) {
			...
			protected void initTableListeners() {
				...
				getTableViewerCreator().addCellValueModifiedListener(
						new ITableCellValueModifiedListener() {

							public void cellValueModified(
									TableCellValueModifiedEvent e) {
								getTableViewerCreator().getTable()
										.deselectAll();
							}
			...
			protected void setTableViewerCreatorOptions(
					final TableViewerCreator<IColumnEntry> newTableViewerCreator) {
				...
				if (getDataMapTable() instanceof AbstractInOutTable) {
					...
					if (imageKey == null) {
						imageKey = org.talend.commons.ui.image.ImageProvider
								.getImage(EImage.KEY_ICON);
					}
					if (imageEmpty == null) {
						imageEmpty = org.talend.commons.ui.image.ImageProvider
								.getImage(EImage.EMPTY);
					}
				}
				newTableViewerCreator
						.setLabelProvider(new DefaultTableLabelProvider(
							...
							public Color getBackground(Object element,
									int columnIndex) {
								return getBackgroundCellColor(
										newTableViewerCreator, element,
										columnIndex);
							}
							...
							public Image getColumnImage(Object element,
									int columnIndex) {
								return getColumnImageExecute(element,
										columnIndex);
							}

							/**
							 * DOC amaumont Comment method
							 * "getColumnImageExecute".
							 * 
							 * @param element
							 * @param columnIndex
							 * @return
							 */
							private Image getColumnImageExecute(Object element,
									int columnIndex) {
								TableViewerCreatorColumn column = newTableViewerCreator
								...
								if (column.getImageProvider() != null) {
									return column.getImageProvider().getImage(
											element);
								}
								return null;
							}
		...
		tableViewerCreatorForColumns = this.extendedTableViewerForColumns
				.getTableViewerCreator();
		this.extendedTableViewerForColumns.setCommandStack(mapperManager
				.getCommandStack());

		tableForEntries = tableViewerCreatorForColumns.getTable();

		GridData tableEntriesGridData = new GridData(SWT.FILL, SWT.FILL, true,
				true);
		tableEntriesGridData.grabExcessVerticalSpace = true;
		tableEntriesGridData.minimumHeight = tableForEntries.getHeaderHeight()
				+ tableForEntries.getItemHeight();
	...
	private void addTableForColumnsListeners() {
		...
		final TableViewer tableViewerForEntries = tableViewerCreatorForColumns
				.getTableViewer();

		tableViewerForEntries
				.addSelectionChangedListener(new ISelectionChangedListener() {

					public void selectionChanged(SelectionChangedEvent event) {
						int[] selectionIndices = tableViewerForEntries
								.getTable().getSelectionIndices();
						if (selectionIndices.length > 0) {
							selectThisDataMapTableView();
							onSelectedEntries(event.getSelection(),
									selectionIndices);
						}
					}
		...
		tableForEntries.addListener(SWT.DragDetect, new Listener() {
			...
			public void handleEvent(Event event) {
				onSelectedEntries(tableViewerForEntries.getSelection(),
						tableViewerForEntries.getTable().getSelectionIndices());
			}

		});

		tableViewerCreatorForColumns.getSelectionHelper()
				.addAfterSelectionListener(new ILineSelectionListener() {

					public void handle(LineSelectionEvent e) {
						if (forceExecuteSelectionEvent) {
							forceExecuteSelectionEvent = false;
							onSelectedEntries(tableViewerForEntries
									.getSelection(), tableViewerForEntries
									.getTable().getSelectionIndices());
						}
					}
		...
		if (abstractDataMapTable instanceof OutputTable) {
			...
			outputTable.getTableFiltersEntriesModel()
					.addAfterOperationListListener(
							new IListenableListListener<FilterTableEntry>() {

								public void handleEvent(
										ListenableListEvent<FilterTableEntry> event) {
			...
			outputTable.getTableFiltersEntriesModel().addModifiedBeanListener(
					new IModifiedBeanListener<FilterTableEntry>() {
						...
						public void handleEvent(
								ModifiedBeanEvent<FilterTableEntry> event) {
		...
		showErrorMessageListener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				case SWT.MouseMove:
					...
					Point cursorPositionFromTableOrigin = TableUtils
							.getCursorPositionFromTableOrigin(table, event);
					TableColumn tableColumn = TableUtils.getTableColumn(table,
							cursorPositionFromTableOrigin);
					if (tableColumn == null) {
						setTableToolTipText(table, null, null, null);
						return;
					}
					TableItem tableItem = TableUtils.getTableItem(table,
							cursorPositionFromTableOrigin);
					...
					if (!WindowSystem.isGTK()
							|| WindowSystem.isGTK()
							&& ((tableToolTip == null || tableToolTip
									.equals("")) && toolTip != null || tableToolTip != null //$NON-NLS-1$
									&& toolTip == null || toolTip != null
									&& !toolTip.equals(tableToolTip))) {
						setTableToolTipText(table, tableColumn, tableEntry,
								toolTip);
					}
					break;
				default:
			 ...
			 * @param text
			 */
			private void setTableToolTipText(final Table table,
					TableColumn tableColumn, ITableEntry tableEntry, String text) {
	...
	protected void addListeners() {
		...
		headerComposite.addListener(SWT.MouseDown,
				onSelectDataMapTableViewListener);
	...
	private void addManualTableResizeListeners(final UIManager uiManager) {
		...
		Listener tableResizerListener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				MousePositionAnalyser mpa = new MousePositionAnalyser(
						DataMapTableView.this);
				Point eventPoint = new Point(event.x, event.y);

				boolean leftMouseButton = (event.stateMask & SWT.BUTTON1) != 0
						|| event.button == 1;

				switch (event.type) {
				case SWT.MouseMove:
					if (resizeHelper.isDragging()) {
						Point newPoint = convertToParentOrigin(
								DataMapTableView.this, eventPoint);
						Point dragPoint = resizeHelper.getLastDragPoint();
						Point diff = new Point(newPoint.x - dragPoint.x,
								newPoint.y - dragPoint.y);
						if (mpa.isOnLeftBorder(eventPoint)) {
							diff.x *= -1;
						}
						Rectangle rect = DataMapTableView.this.getClientArea();
						rect.width += 2 * DataMapTableView.this
								.getBorderWidth();
						rect.height += 2 * DataMapTableView.this
								.getBorderWidth();
						...
						if (newHeight < MINIMUM_HEIGHT + OFFSET_HEIGHT_TRIGGER
								&& diff.y < 0) {
							changeMinimizeState(true);
							newHeight = MINIMUM_HEIGHT;
						} else if (newHeight > MINIMUM_HEIGHT
								+ OFFSET_HEIGHT_TRIGGER
								&& diff.y > 0) {
						...
						if (mpa.isOnLeftBottomCorner(eventPoint)) {
							...
							cursor = new Cursor(DataMapTableView.this
									.getDisplay(),
									org.eclipse.swt.SWT.CURSOR_SIZES);
				...
				case SWT.MouseDown:
					if (leftMouseButton) {
						if (mpa.isOnLeftBorder(eventPoint)
								|| mpa.isOnRightBorder(eventPoint)
								|| mpa.isOnBottomBorder(eventPoint)) {
							resizeHelper.startDrag(convertToParentOrigin(
									DataMapTableView.this, new Point(event.x,
											event.y)));
						} else {
							setDefaultCursor();
						}
						parentLayout = DataMapTableView.this.getParent()
								.getLayout();
				...
				case SWT.MouseUp:
					if (leftMouseButton) {
						...
						DataMapTableView.this.getParent().setLayout(
								parentLayout);
	 ...
	 */
	public abstract void initColumnsOfTableColumns(
	 ...
	 * @param newHeight
	 * @param refreshNow
	 *            refresh background with links if true
	 * @param recalculateParentSize
	 */
	public void changeSize(Point newSize, boolean refreshNow,
			boolean recalculateParentSize) {
		...
		if (refreshNow) {
			...
			uiManager.refreshBackground(true,
					TIME_BEFORE_NEW_REFRESH_BACKGROUND, true);
		}
	...
	public void onSelectedEntries(ISelection selection, int[] selectionIndices) {
		UIManager uiManager = mapperManager.getUiManager();
		List<ITableEntry> selectionEntries = uiManager
				.extractSelectedTableEntries(selection);
		if (executeSelectionEvent) {
			uiManager.selectLinks(DataMapTableView.this, selectionEntries,
					false, false);
		...
		if (selectionIndices.length == 1 && mapperManager.componentIsReadOnly()) {
			...
			StyledText styledText = mapperManager.getUiManager()
					.getTabFolderEditors().getStyledTextHandler()
					.getStyledText();
			styledText.setEnabled(true);
			styledText.setEditable(false);
			styledText.setText(tableEntry.getExpression() == null ? ""
					: tableEntry.getExpression());
		}
	}
	...
	public void setTableSelection(int[] selectionIndices) {
		...
		this.tableViewerCreatorForColumns.getSelectionHelper().setSelection(
				selectionIndices);
	...
	protected void createFiltersToolItems() {
		...
		if (mapperManager.isAdvancedMap()) {
			...
			addFilterButton.setToolTipText(Messages
					.getString("DataMapTableView.buttonTooltip.addFilterRow")); //$NON-NLS-1$
			addFilterButton.setImage(ImageProviderMapper
					.getImage(ImageInfo.ADD_FILTER_ICON));
			...
			if (addFilterButton != null) {
				...
				addFilterButton.addSelectionListener(new SelectionListener() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						Table tableConstraints = tableViewerCreatorForFilters
								.getTable();
						int index = tableConstraints.getItemCount();
						int[] selection = tableViewerCreatorForFilters
								.getTable().getSelectionIndices();
						if (selection.length > 0) {
							index = selection[selection.length - 1] + 1;
						}
						mapperManager.addNewFilterEntry(DataMapTableView.this,
								"newFilter" + ++constraintCounter, index); //$NON-NLS-1$
						updateGridDataHeightForTableConstraints();
						DataMapTableView.this.changeSize(DataMapTableView.this
								.getPreferredSize(false, true, true), true,
								true);
						tableViewerCreatorForFilters.getTableViewer().refresh();
						mapperManager.getUiManager().refreshBackground(true,
								false);
		...
		final ToolItem rejectFilterCheck = new ToolItem(toolBarActions,
				SWT.CHECK);
		rejectFilterCheck.setEnabled(!mapperManager.componentIsReadOnly());
		rejectFilterCheck
				.setToolTipText(Messages
						.getString("DataMapTableView.widgetTooltip.enableOutputReject")); //$NON-NLS-1$
		...
		Image image = ImageProviderMapper
				.getImage(isReject ? ImageInfo.REJECT_FILTER_ICON
						: ImageInfo.REJECT_FILTER_ICON);
		if (WindowSystem.isGTK()) {
			rejectFilterCheck.setImage(image);
			rejectFilterCheck.setHotImage(image);
		} else {
			rejectFilterCheck.setImage(ImageProviderMapper
					.getImage(ImageInfo.REJECT_FILTER_ICON));
			rejectFilterCheck.setHotImage(image);
		}
		...
		// // /////////////////////////////////////////////////////////////////
		final ToolItem rejectInnerJoinFilterCheck = new ToolItem(
				toolBarActions, SWT.CHECK);
		rejectInnerJoinFilterCheck.setEnabled(!mapperManager
				.componentIsReadOnly());
		rejectInnerJoinFilterCheck
				.setToolTipText(Messages
						.getString("DataMapTableView.widgetTooltip.enableLookupInnerJoin")); //$NON-NLS-1$
		boolean isRejectInnerJoin = ((OutputTable) abstractDataMapTable)
				.isRejectInnerJoin();
		// image = ImageProviderMapper.getImage(isRejectInnerJoin ?
		// ImageInfo.CHECKED_ICON : ImageInfo.UNCHECKED_ICON);
		image = ImageProviderMapper
				.getImage(isRejectInnerJoin ? ImageInfo.REJECT_LOOKUP_ICON
						: ImageInfo.REJECT_LOOKUP_ICON);
		if (WindowSystem.isGTK()) {
			rejectInnerJoinFilterCheck.setImage(image);
			rejectInnerJoinFilterCheck.setHotImage(image);
		} else {
			rejectInnerJoinFilterCheck.setImage(ImageProviderMapper
					.getImage(ImageInfo.REJECT_LOOKUP_ICON));
	...
	protected void createActivateFilterCheck() {
		...
		activateFilterCheck.setImage(ImageProviderMapper
				.getImage(ImageInfo.ACTIVATE_FILTER_ICON));
		...
		if (activateFilterCheck != null) {
			...
			activateFilterCheck.addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					previousStateCheckFilter = activateFilterCheck
							.getSelection();
				}
	...
	public void minimizeTable(boolean minimize) {
		...
		if (minimize) {
			...
			if (heightForRestore != getHeaderHeight() && heightForRestore > 0) {
				...
				size = DataMapTableView.this.getPreferredSize(false, true,
						false);
			}
	...
	public void changeMinimizeState(boolean newStateIsMinimized) {
		if (newStateIsMinimized) {
			...
			minimizeButton.setImage(ImageProviderMapper
					.getImage(ImageInfo.RESTORE_ICON));
			minimizeButton.setToolTipText(Messages
					.getString("DataMapTableView.buttonTooltip.restore")); //$NON-NLS-1$
		} else {
			abstractDataMapTable.setMinimized(false);
			minimizeButton.setSelection(false);
			minimizeButton.setImage(ImageProviderMapper
					.getImage(ImageInfo.MINIMIZE_ICON));
			minimizeButton.setToolTipText(Messages
					.getString("DataMapTableView.buttonTooltip.minimize")); //$NON-NLS-1$
		}
	}
	 ...
	 * @return
	 */
	public Point getPreferredSize(boolean newTableEntryWillBeAdded,
			boolean expandedSize, boolean newConstraintEntryWillBeAdded) {
		int heightOffset = 0;
		int tableEntryItemHeight = tableViewerCreatorForColumns.getTable()
				.getItemHeight();
		// heightOffset += newTableEntryWillBeAdded ? tableEntryItemHeight : 0;
		heightOffset += newConstraintEntryWillBeAdded ? tableViewerCreatorForFilters
				.getTable().getItemHeight()
				: 0;

		int newHeight = this.computeSize(SWT.DEFAULT, SWT.DEFAULT).y
				- tableEntryItemHeight + heightOffset;
		if (WindowSystem.isGTK()) {
	...
	public void registerStyledExpressionEditor(
			final StyledTextHandler styledTextHandler) {
		if (columnExpressionTextEditor != null) {
			attachCellExpressionToStyledTextEditor(
					tableViewerCreatorForColumns, columnExpressionTextEditor,
					styledTextHandler);
		}
		if (constraintExpressionTextEditor != null) {
			attachCellExpressionToStyledTextEditor(
					tableViewerCreatorForFilters,
					constraintExpressionTextEditor, styledTextHandler);
		}
	}
	 ...
	 */
	protected void attachCellExpressionToStyledTextEditor(
			final TableViewerCreator tableViewerCreator,
			final Text expressionTextEditor,
			final StyledTextHandler styledTextHandler) {
		expressionTextEditor.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				...
				if (Math.abs(text.getText().length()
						- styledTextHandler.getStyledText().getText().length()) > 1) {
					styledTextHandler.setTextWithoutNotifyListeners(text
							.getText());
		...
		expressionTextEditor
				.addKeyListener(new TextCellEditorToMapperStyledTextKeyListener(
						expressionTextEditor, styledTextHandler));
	...
	private void highlightLineAndSetSelectionOfStyledText(
			final Text expressionTextEditor) {
	 ...
	 */
	protected void highlightLineAndSetSelectionOfStyledTextFromTextControl(
			final Control textWidget) {
		final StyledTextHandler styledTextHandler = mapperManager
				.getUiManager().getTabFolderEditors().getStyledTextHandler();
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				String lineDelimiter = ControlUtils
						.getLineDelimiter(textWidget);
				if (selection.x - 1 > 0) {
		...
		new AsynchronousThreading(50, true, DataMapTableView.this.getDisplay(),
				runnable).start();
	}

	/**
	 * 
	 * Init proposals of Text control, and StyledText in tab "Expression
	 * editor".
	 * 
	 * @param textControl
	 * @param zones
	 ...
	 */
	protected void initExpressionProposals(
			final ExtendedTextCellEditorWithProposal cellEditor, Zone[] zones,
			final TableViewerCreator tableViewerCreator,
			ITableEntry currentModifiedEntry) {
		if (this.expressionProposalProvider == null) {
			this.expressionProposalProvider = createExpressionProposalProvider();
		}
		this.expressionProposalProvider.init(abstractDataMapTable, zones,
				currentModifiedEntry);
		...
		StyledTextHandler styledTextHandler = mapperManager.getUiManager()
				.getTabFolderEditors().getStyledTextHandler();
		...
		expressionProposalStyledText
				.setContentProposalProvider(this.expressionProposalProvider);
	...
	protected ExpressionProposalProvider createExpressionProposalProvider() {
		...
		if (!MapperMain.isStandAloneMode()) {
			contentProposalProviders = new IContentProposalProvider[] { new ProcessProposalProvider(
					mapperManager.getAbstractMapComponent().getProcess()) };
		}
		return new ExpressionProposalProvider(mapperManager,
				contentProposalProviders);
	}

	protected ExtendedTextCellEditor createExpressionCellEditor(
			final TableViewerCreator tableViewerCreator,
			TableViewerCreatorColumn column, final Zone[] zones,
			boolean isConstraintExpressionCellEditor) {
		...
		final ExtendedTextCellEditorWithProposal cellEditor = new ExtendedTextCellEditorWithProposal(
				tableViewerCreator.getTable(), SWT.MULTI | SWT.BORDER, column);

		dialog = ((IExpressionBuilderDialogService) expressionBuilderDialogService)
				.getExpressionBuilderInstance(tableViewerCreator
						.getCompositeParent(), cellEditor);
		...
		cellEditor.addListener(new ICellEditorListener() {
			...
			public void applyEditorValue() {
				ModifiedObjectInfo modifiedObjectInfo = tableViewerCreator
						.getModifiedObjectInfo();
				mapperManager.getUiManager().parseNewExpression(
						text.getText(),
						(ITableEntry) modifiedObjectInfo
								.getCurrentModifiedBean(), true);
			}

			public void cancelEditor() {
				ModifiedObjectInfo modifiedObjectInfo = tableViewerCreator
						.getModifiedObjectInfo();
				text.setText((String) modifiedObjectInfo
						.getOriginalPropertyBeanValue());
				ITableEntry tableEntry = (ITableEntry) (modifiedObjectInfo
						.getCurrentModifiedBean() != null ? modifiedObjectInfo
						.getCurrentModifiedBean() : modifiedObjectInfo
						.getPreviousModifiedBean());
				String originalExpression = (String) modifiedObjectInfo
						.getOriginalPropertyBeanValue();
				mapperManager.getUiManager().parseNewExpression(
						originalExpression, tableEntry, true);
			}
				...
				if (executionLimiter == null) {
					executionLimiter = new ExecutionLimiter(50, true) {
						...
						public void execute(boolean isFinalExecution) {
							...
							if (isFinalExecution) {
								expressionTextEditor.getDisplay().syncExec(
										new Runnable() {

											public void run() {
												if (expressionTextEditor
														.isDisposed()) {
													return;
												}
												resizeTextEditor(
		...
		expressionTextEditor.addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				ITableEntry currentModifiedEntry = (ITableEntry) tableViewerCreator
						.getModifiedObjectInfo().getCurrentModifiedBean();
				initExpressionProposals(cellEditor, zones, tableViewerCreator,
						currentModifiedEntry);
				resizeTextEditor(expressionTextEditor, tableViewerCreator);
				StyledTextHandler styledTextHandler = mapperManager
						.getUiManager().getTabFolderEditors()
						.getStyledTextHandler();
				styledTextHandler.setCurrentEntry(currentModifiedEntry);
				styledTextHandler
						.setTextWithoutNotifyListeners(currentModifiedEntry
								.getExpression() == null ? "" : currentModifiedEntry.getExpression()); //$NON-NLS-1$
			}

			public void focusLost(FocusEvent e) {
				expressionEditorTextSelectionBeforeFocusLost = expressionTextEditor
						.getSelection();
				checkChangementsAfterEntryModifiedOrAdded(false);
				if (WindowSystem.isGTK()) {

					new AsynchronousThreading(50, false, expressionTextEditor
		...
		column.setCellEditor(cellEditor, new CellEditorValueAdapter() {
			...
			@Override
			public Object getCellEditorTypedValue(CellEditor cellEditor,
					Object originalTypedValue) {
				return super.getCellEditorTypedValue(cellEditor,
						originalTypedValue);
			}

			@Override
			public String getColumnText(CellEditor cellEditor, Object bean,
					Object cellEditorTypedValue) {
				return super.getColumnText(cellEditor, bean,
						cellEditorTypedValue).replaceAll("[\r\n\t]+", " ... "); //$NON-NLS-1$ //$NON-NLS-2$
			}

			@Override
			public Object getOriginalTypedValue(CellEditor cellEditor,
					Object cellEditorTypedValue) {
				return super.getOriginalTypedValue(cellEditor,
						cellEditorTypedValue);
			}
	...
	private void resizeTextEditor(Text textEditor,
			TableViewerCreator tableViewerCreator) {
	...
	public void updateGridDataHeightForTableConstraints() {
		...
		int moreSpace = WindowSystem.isGTK() ? tableForConstraints
				.getItemHeight() : 0;
		tableForConstraintsGridData.heightHint = ((OutputTable) abstractDataMapTable)
				.getFilterEntries().size()
				* tableForConstraints.getItemHeight()
				+ tableForConstraints.getItemHeight() / 2 + moreSpace;
	}
	...
	protected void selectThisDataMapTableView() {
		...
		if (uiManager.getCurrentSelectedInputTableView() != DataMapTableView.this
			...
			uiManager
					.selectDataMapTableView(DataMapTableView.this, true, false);
		}
	}
	 ...
	 * @return
	 */
	protected Color getBackgroundCellColor(
			TableViewerCreator tableViewerCreator, Object element,
			int columnIndex) {
		ITableEntry entry = (ITableEntry) element;
		TableViewerCreatorColumn column = (TableViewerCreatorColumn) tableViewerCreator
				.getColumns().get(columnIndex);
		if (column.getId().equals(ID_EXPRESSION_COLUMN)) {
			return expressionColorProvider.getBackgroundColor(entry
					.getProblems() == null ? true : false);
		}
		return null;
	}
	 ...
	 * @return
	 */
	protected Color getForegroundCellColor(
			TableViewerCreator tableViewerCreator, Object element,
			int columnIndex) {
		ITableEntry entry = (ITableEntry) element;
		TableViewerCreatorColumn column = (TableViewerCreatorColumn) tableViewerCreator
				.getColumns().get(columnIndex);
		if (column.getId().equals(ID_EXPRESSION_COLUMN)) {
			return expressionColorProvider.getForegroundColor(entry
					.getProblems() == null ? true : false);
		}
		return null;
	}
	 ...
	 * @param e
	 */
	protected void unselectAllEntriesIfErrorDetected(
			TableCellValueModifiedEvent e) {
		if (e.column.getId().equals(ID_EXPRESSION_COLUMN)) {
			...
			if (currentEntry instanceof IColumnEntry) {
				tableViewer = DataMapTableView.this
						.getTableViewerCreatorForColumns().getTableViewer();
			} else if (currentEntry instanceof FilterTableEntry) {
				tableViewer = DataMapTableView.this
						.getTableViewerCreatorForFilters().getTableViewer();
			}
	 ...
	 * @param event
	 */
	protected void processEnterKeyDown(
			final TableViewerCreator tableViewerCreator, Event event) {
		if (event.character == '\r') {
			Object element = null;
			if (tableViewerCreator.getTable().getSelectionCount() == 1) {
				element = tableViewerCreator.getTable().getSelection()[0]
						.getData();
			} else {
				element = tableViewerCreator.getModifiedObjectInfo()
						.getPreviousModifiedBean();
			}
			if (element != null) {
				int indexColumn = tableViewerCreator.getColumn(
						ID_EXPRESSION_COLUMN).getIndex();
				tableViewerCreator.getTableViewer().editElement(element,
						indexColumn);
			}
		}
	}
	 ...
	 * @param tableEntry
	 */
	private void parseExpression(ModifiedBeanEvent event,
			TableViewerCreator tableViewerCreator, ITableEntry tableEntry) {
		if (event.column == tableViewerCreator
				.getColumn(DataMapTableView.ID_EXPRESSION_COLUMN)) {
			mapperManager.getUiManager()
					.parseExpression(tableEntry.getExpression(), tableEntry,
							false, false, false);
	...
	protected void createExpressionFilter() {
		if (mapperManager.isAdvancedMap()
				&& getDataMapTable() instanceof AbstractInOutTable) {
			...
			// SWT.WRAP | SWT.BORDER);
			expressionFilterText = new UnnotifiableColorStyledText(
					getCenterComposite(), SWT.MULTI | SWT.WRAP | SWT.BORDER
							| SWT.V_SCROLL, colorManager, LanguageManager
			...
			String expressionFilter = table.getExpressionFilter()
					.getExpression();
			...
			expressionFilterText.addFocusListener(new FocusListener() {
				...
				public void focusGained(FocusEvent e) {
					...
					ExpressionFilterEntry currentExpressionFilterEntry = table
							.getExpressionFilter();
					...
					mapperManager
							...
							.selectLinks(
									DataMapTableView.this,
									Arrays
											.<ITableEntry> asList(currentExpressionFilterEntry),
									true, false);
					...
					StyledTextHandler styledTextHandler = mapperManager
							...
							.getStyledTextHandler();
					styledTextHandler
							.setCurrentEntry(currentExpressionFilterEntry);
					previousTextForExpressionFilter = currentExpressionFilterEntry
							.getExpression() == null ? "" : currentExpressionFilterEntry.getExpression(); //$NON-NLS-1$
					styledTextHandler.getStyledText().setText(
							previousTextForExpressionFilter);
					expressionFilterText.setToolTipText(null);
					styledTextHandler.getStyledText().setEnabled(true);
					styledTextHandler.getStyledText().setEditable(true);
					expressionProposalStyledText = styledTextHandler
							.getContentProposalAdapter();
					expressionProposalProviderForExpressionFilter.init(table,
							getValidZonesForExpressionFilterField(), table
									.getExpressionFilter());
					expressionProposalStyledText
				...
				public void handleEvent(Event event) {
					...
					case SWT.MouseMove:
						if (table.getExpressionFilter().getProblems() != null
							...
							String tooltip = createErrorContentForTooltip(table
									.getExpressionFilter().getProblems());
			...
			expressionFilterText.addListener(SWT.MouseMove,
					showTooltipErrorListener);

			if (executionLimiterForExpressionFilterSetText == null) {
				executionLimiterForExpressionFilterSetText = new ExecutionLimiter(
						50, true) {
					...
					public void execute(boolean isFinalExecution) {
						...
						if (isFinalExecution) {
							expressionFilterText.getDisplay().syncExec(
									new Runnable() {

										public void run() {
											if (expressionFilterText
													.isDisposed()) {
												return;
											}
			...
			if (getZone() == Zone.INPUTS) {
				scrolledCompositeView = getMapperManager().getUiManager()
						.getScrolledCompositeViewInputs();
			} else if (getZone() == Zone.OUTPUTS) {
				scrolledCompositeView = getMapperManager().getUiManager()
						.getScrolledCompositeViewOutputs();
			}
			...
			scrolledCompositeView.getVerticalBar().addSelectionListener(
					selectionListenerToCorrectWrapBug);

			expressionFilterText.getVerticalBar().addSelectionListener(
					selectionListenerToCorrectWrapBug);
			...
			ExpressionEditorToMapperStyledTextKeyListener keyAndModifyListener = new ExpressionEditorToMapperStyledTextKeyListener(
					expressionFilterText, mapperManager.getUiManager()
							.getTabFolderEditors().getStyledTextHandler());
			expressionFilterText
					.addExtendedModifyListener(keyAndModifyListener);
	...
	public void checkProblemsForExpressionFilterWithDelay() {
		if (this.executionLimiterForCheckProblemsExpressionFilter == null) {
			...
			this.executionLimiterForCheckProblemsExpressionFilter = new ExecutionLimiter(
					2000, true) {
				...
				protected void execute(boolean isFinalExecution) {
					if (!expressionFilterText.isDisposed()) {
						expressionFilterText.getDisplay().asyncExec(
								new Runnable() {
		...
		executionLimiterForCheckProblemsExpressionFilter
				.startIfExecutable(true);

	}

	public void checkProblemsForExpressionFilter(boolean forceRecompile,
			boolean colorAllowed) {
		if (this.getDataMapTable() instanceof AbstractInOutTable) {
			AbstractInOutTable table = (AbstractInOutTable) this
					.getDataMapTable();
			if (table.isActivateExpressionFilter()
					&& !DEFAULT_EXPRESSION_FILTER.equals(expressionFilterText
							.getText())) {
				String nextText = expressionFilterText.getText();
				if (nextText != null
						&& previousTextForExpressionFilter != null
						&& !nextText.trim().equals(
								previousTextForExpressionFilter.trim())
						|| forceRecompile) {
					mapperManager.getProblemsManager()
							.checkProblemsForTableEntry(
									table.getExpressionFilter(), true);
				} else {
					mapperManager.getProblemsManager()
							.checkProblemsForTableEntry(
									table.getExpressionFilter(), false);
	 ...
	 *            TODO
	 */
	private void colorExpressionFilterFromProblems(AbstractInOutTable table,
			boolean colorAllowed) {
		...
		if (problems != null && colorAllowed) {
			...
			expressionFilterText
					.setBackground(ColorProviderMapper
							.getColor(ColorInfo.COLOR_BACKGROUND_ERROR_EXPRESSION_CELL));
			expressionFilterText
					.setForeground(ColorProviderMapper
							.getColor(ColorInfo.COLOR_FOREGROUND_ERROR_EXPRESSION_CELL));
		} else {
	...
	public void configureExpressionFilter() {
		if (mapperManager.isAdvancedMap()
			...
			this.proposalForExpressionFilterText = ProposalUtils
					.getCommonProposal(expressionFilterText,
							expressionProposalProviderForExpressionFilter);
			this.proposalForExpressionFilterText
					.addContentProposalListener(new IContentProposalListener() {

						public void proposalAccepted(IContentProposal proposal) {
							new AsynchronousThreading(50, false,
									expressionFilterText.getDisplay(),
									new Runnable() {
	...
	private void correctAsynchStyledTextWrapBug() {
		new AsynchronousThreading(100, false,
				expressionFilterText.getDisplay(), new Runnable() {
	 ...
	 * @param text
	 */
	private void setExpressionFilterFromStyledText(
			final AbstractInOutTable table, Control text) {
	...
	protected void updateViewAfterChangingFilterCheck() {
		...
		if (activateFilterCheck.getSelection()) {
			...
			mapperManager.getUiManager().parseExpression(
					expressionFilterText.getText(),
		...
		// updateGridDataHeightForTableConstraints();
		DataMapTableView.this.changeSize(DataMapTableView.this
				.getPreferredSize(false, true, false), true, true);
		DataMapTableView.this.layout();

		mapperManager.getUiManager().refreshBackground(true, false);

		if (expressionFilterText.isVisible()) {
			new AsynchronousThreading(50, false, mapperManager.getUiManager()
					.getDisplay(), new Runnable() {

				public void run() {
					checkProblemsForExpressionFilter(expressionFilterText
			...
			if (DataMapTableView.this.getZone() == Zone.INPUTS) {
				ScrolledComposite scrolledZoneView = mapperManager
						.getUiManager().getScrolledCompositeViewInputs();
				Point point = expressionFilterText.getDisplay().map(
						expressionFilterText,
						mapperManager.getUiManager().getTablesZoneViewInputs(),
						new Point(0, 0));
				mapperManager.getUiManager()
						.setPositionOfVerticalScrollBarZone(scrolledZoneView,
								point.y - 20);
			}
		}
	...
	/**
	 * 
	 * DOC amaumont InputDataMapTableView class global comment. Detailled
	 * comment <br/>
	 * 
	 */
	class ExpressionEditorToMapperStyledTextKeyListener implements
			ExtendedModifyListener, KeyListener {
		 ...
		 */
		public ExpressionEditorToMapperStyledTextKeyListener(
				StyledText textWidgetSrc, StyledTextHandler textTarget) {
			super();
		...
		public void keyReleased(KeyEvent e) {
			...
			mapperManager.getUiManager().parseNewExpression(
					textTarget.getStyledText().getText(),
		...
		public void modifyText(ExtendedModifyEvent event) {
			...
			if (DEFAULT_EXPRESSION_FILTER.equals(ControlUtils
					.getText(textWidget))) {
				textTarget.setTextWithoutNotifyListeners("");
			} else {
				textTarget.setTextWithoutNotifyListeners(ControlUtils
						.getText(textWidget));
			}
			highlightLineAndSetSelectionOfStyledTextFromTextControl(textWidget);
			// }
	...
	class TextCellEditorToMapperStyledTextKeyListener implements KeyListener {
		 ...
		 * DOC amaumont TextKeyListener constructor comment.
		 */
		public TextCellEditorToMapperStyledTextKeyListener(Text textWidgetSrc,
				StyledTextHandler textTarget) {
			super();
			this.textWidget = textWidgetSrc;
			this.textTarget = textTarget;
		}
				...
				if (e.character == '\r' || e.character == '\u001b') {
					...
					if (e.character == SWT.BS || e.character == SWT.DEL) {
						if (selection.x == selection.y) {
							...
							if (e.character == SWT.BS) {
								if (selection.x - 1 > 0) {
									char previousChar = mapperColorStyledText
											.getText().charAt(selection.x - 1);
									if (previousChar == '\n') {
										mapperColorStyledText
												.replaceTextRangeWithoutNotifyListeners(
								...
								if (selection.x < mapperColorStyledText
										.getText().length()) {
									char nextChar = mapperColorStyledText
											.getText().charAt(selection.x);
									if (nextChar == '\r') {
										mapperColorStyledText
												.replaceTextRangeWithoutNotifyListeners(
														selection.x, 2, ""); //$NON-NLS-1$
									} else {
										mapperColorStyledText
												.replaceTextRangeWithoutNotifyListeners(
														selection.x, 1, ""); //$NON-NLS-1$
									}
								}
							}
	 ...
	 * 
	 * @param previousStateCheckFilter
	 *            the previousStateCheckFilter to set
	 */
