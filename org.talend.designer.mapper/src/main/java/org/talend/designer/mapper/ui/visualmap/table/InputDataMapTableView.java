...
public class InputDataMapTableView extends DataMapTableView {
	...
	private ILookupType previousMultipleModeSelected = getInputTable()
			.getMatchingMode() != null
			&& getInputTable().getMatchingMode() != TMAP_MATCHING_MODE.ALL_ROWS ? getInputTable()
			.getMatchingMode()
	...
	public InputDataMapTableView(Composite parent, int style,
			InputTable inputTable, MapperManager mapperManager) {
	...
	public void initColumnsOfTableColumns(
			TableViewerCreator tableViewerCreatorForColumns) {
		boolean isMainConnection = ((InputTable) getDataMapTable())
				.isMainConnection();
		...
		if (!isMainConnection) {
			...
			column.setTitle(Messages
					.getString("InputDataMapTableView.columnTitle.Expr")); //$NON-NLS-1$
			column.setId(DataMapTableView.ID_EXPRESSION_COLUMN);
			final ExtendedTextCellEditor expressionCellEditor = createExpressionCellEditor(
					tableViewerCreatorForColumns, column,
					new Zone[] { Zone.INPUTS }, false);
			column
					.setBeanPropertyAccessors(new IBeanPropertyAccessors<InputColumnTableEntry, String>() {
			...
			column
					.setImageProvider(new IColumnImageProvider<InputColumnTableEntry>() {

						public Image getImage(InputColumnTableEntry bean) {
							if (bean.getExpression() != null
									&& !bean.getExpression().trim().equals("")) {
								if (mapperManager.isAdvancedMap()) {
									return ImageProviderMapper
											.getImage(ImageInfo.LOOKUP_KEY_ICON);
								} else {
									return null;
								}
							} else {
	...
	protected boolean addToolItems() {
		...
		if (!getInputTable().isMainConnection()) {
			...
			if (mapperManager.isAdvancedMap()) {
				...
				// DROP DOWN
				dropDownItem = new ToolItem(toolBarActions, SWT.DROP_DOWN
						| SWT.BORDER);
				dropDownItem.setEnabled(!mapperManager.componentIsReadOnly());
				refreshLabelForLookupTypeDropDown();
				dropDownItem
						.addSelectionListener(new DropDownSelectionListener());
			...
			// ImageInfo.CHECKED_ICON :
			// ImageInfo.UNCHECKED_ICON);
			Image image = ImageProviderMapper
					.getImage(isInnerJoin ? ImageInfo.CHECKED_ICON
							: ImageInfo.UNCHECKED_ICON);
			if (WindowSystem.isGTK()) {
				innerJoinCheck.setImage(image);
				innerJoinCheck.setHotImage(image);
			} else {
				innerJoinCheck.setImage(ImageProviderMapper
						.getImage(ImageInfo.UNCHECKED_ICON));
				innerJoinCheck.setHotImage(image);
			}
			innerJoinCheck.setSelection(isInnerJoin);
			innerJoinCheck
					.setText(Messages
							.getString("InputDataMapTableView.widgetTooltip.innerJoin")); //$NON-NLS-1$
			...
			innerJoinCheck.addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					if (innerJoinCheck.getSelection()) {
						...
						image = ImageProviderMapper
								.getImage(ImageInfo.CHECKED_ICON);
					} else {
			...
			if (menu == null) {
				...
				for (int i = 0; i < availableJoins.length; ++i) {
					...
					if (text.length() != 0) {
						...
						if (availableJoins[i] == getInputTable()
								.getMatchingMode()) {
							menuItem.setImage(ImageProviderMapper
									.getImage(ImageInfo.CHECKED_ICON));
						}
						...
						menuItem.addSelectionListener(new SelectionAdapter() {
							...
							public void widgetSelected(SelectionEvent e) {
								...
								ILookupType lookupType = (ILookupType) menuItem
										.getData();
	...
	public void refreshLabelForLookupTypeDropDown() {
		...
		Point sizeToolBar = toolBarActions
	...
	public void checkChangementsAfterEntryModifiedOrAdded(
		...
		if (!getInputTable().isMainConnection()
				&& mapperManager.isAdvancedMap()) {
			boolean stateAtLeastOneHashKey = mapperManager
					.isTableHasAtLeastOneHashKey(getInputTable());
			if (forceEvaluation
					|| previousStateAtLeastOneHashKey != stateAtLeastOneHashKey) {
				if (stateAtLeastOneHashKey) {
					dropDownItem.setText(previousMultipleModeSelected
							.getLabel());
					dropDownItem.setToolTipText(previousMultipleModeSelected
							.getLabel());
					getInputTable().setMatchingMode(
							previousMultipleModeSelected);
					...
					dropDownItem
							.setText(TMAP_MATCHING_MODE.ALL_ROWS.getLabel());
					dropDownItem.setToolTipText(TMAP_MATCHING_MODE.ALL_ROWS
							.getLabel());
					getInputTable()
							.setMatchingMode(TMAP_MATCHING_MODE.ALL_ROWS);
