...
public class RowSelectionEditPolicy extends NonResizableEditPolicy {
    ...
    private IFigure getLocatorFigure(GraphicalEditPart owner) {
        ...
        if (owner instanceof TreeNodeEditPart) {
            TreeNodeFigure treeNodeFigure = (TreeNodeFigure) figure;
            // table figure to restrict the width
            AbstractInOutTreeEditPart abstractInOutTreePart = XmlMapUtil.getAbstractInOutTreePart((TreeNodeEditPart) owner);
        ...
        } else if (figure instanceof VarNodeFigure) {
