 ...
 * 
 * $Id: ExpressionBuilderDialog.java ä¸Šå�ˆ10:12:13 2007-7-24 +0000 (2007-7-24)
 * yzhang $
 * 
 */
public class ExpressionBuilderDialog extends Dialog implements
	...
	protected Control createDialogArea(Composite parent) {
		...
		categoryComposite = new CategoryComposite(lowerComposite, SWT.NONE,
				manager);
	...
	/**
	 * yzhang ExpressionBuilderDialog class global comment. Detailled comment
	 * <br/>
	 * 
	 * $Id: ExpressionBuilderDialog.java ä¸‹å�ˆ08:32:27 2007-9-13 +0000
	 * (2007-9-13) yzhang $
	 * 
	 */
	...
	protected void createButtonsForButtonBar(Composite parent) {
		...
		importButton.setToolTipText(Messages
				.getString("ExpressionBuilderDialog.import"));//$NON-NLS-1$
		...
		exportButton.setToolTipText(Messages
				.getString("ExpressionBuilderDialog.export"));//$NON-NLS-1$
		exportButton.setImage(ImageProvider.getImage(EImage.IMPORT_ICON));

		createButton(parent, IDialogConstants.OK_ID, Messages
				.getString("ExpressionBuilderDialog.ok.button"), true); //$NON-NLS-1$
		createButton(parent, IDialogConstants.CANCEL_ID, Messages
				.getString("ExpressionBuilderDialog.cancel.button"), false); //$NON-NLS-1$
		...
		exportButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				...
				if (filePath != null) {
					String expresionContent = expressionComposite
							.getExpression();
					...
					try {
						...
						operation.saveExpressionToFile(file, variables,
								expresionContent);
				...
				if (filePath != null) {
					...
					try {
						...
						if (list != null && list.size() != 0) {
							expressionComposite.setExpression((String) list
									.get(0), false);
							list.remove(0);
							if (list.size() > 0) {
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText(Messages
				.getString("ExpressionBuilderDialog.expression.builder")); //$NON-NLS-1$
	}
