...
public class DisplayResultComposite extends Composite implements
	...
	private void init() {
		...
		lblMinute.setText(Messages
				.getString("DisplayResultComposite.minuteLabel")); //$NON-NLS-1$
		...
		lblMonth.setText(Messages
				.getString("DisplayResultComposite.monthLabel")); //$NON-NLS-1$
		...
		lblWeekday.setText(Messages
				.getString("DisplayResultComposite.weekdayLabel")); //$NON-NLS-1$

		final Label lblCommand = new Label(this, SWT.NONE);
		lblCommand.setLayoutData(new GridData());
		lblCommand.setText(Messages
				.getString("DisplayResultComposite.commandLabel")); //$NON-NLS-1$
		new Label(this, SWT.NONE);

		rdbtnGenerated = new Button(this, SWT.RADIO);
		rdbtnGenerated.setLayoutData(new GridData());
		rdbtnGenerated.setText(Messages
				.getString("DisplayResultComposite.generatedCronTabText")); //$NON-NLS-1$
		...
		btnCopy.setText(Messages
				.getString("DisplayResultComposite.copyButtonText")); //$NON-NLS-1$
		...
		customCrontabEntryButton.setText(Messages
				.getString("DisplayResultComposite.customCronTabText")); //$NON-NLS-1$
	...
	/**
	 * Set the enablements of the widgets which implemented
	 * IWidgetEnableListener contained in the List enableWidgetsLisenter.
	 * 
	 * @param enable
	 *            boolean.
	 */
	...
	private void checkPlainCommand() throws SchedulerException {
		if (txtCustom.getText() == null
				|| txtCustom.getText().trim().equals("")) {
			throw new SchedulerException(
					Messages
							.getString("DisplayResultComposite.customCronTabEmptyPrompt")); //$NON-NLS-1$
		}
	}
	...
	public void check() throws SchedulerException {
		...
		if (txtCommand.getText().equals("")) { //$NON-NLS-1$
			message = Messages
					.getString("DisplayResultComposite.commandEmptyPrompt"); //$NON-NLS-1$
		} else if (txtDay.getText().equals("")) { //$NON-NLS-1$
			message = Messages
					.getString("DisplayResultComposite.dayEmptyPrompt"); //$NON-NLS-1$
		} else if (txtMonth.getText().equals("")) { //$NON-NLS-1$
			message = Messages
					.getString("DisplayResultComposite.monthEmptyPrompt"); //$NON-NLS-1$
		} else if (txtWeekday.getText().equals("")) { //$NON-NLS-1$
			message = Messages
					.getString("DisplayResultComposite.weekdayEmptyPrompt"); //$NON-NLS-1$
		} else if (txtHour.getText().equals("")) { //$NON-NLS-1$
			message = Messages
					.getString("DisplayResultComposite.hourEmptyPrompt"); //$NON-NLS-1$
		} else if (txtMinute.getText().equals("")) { //$NON-NLS-1$
			message = Messages
					.getString("DisplayResultComposite.minuteEmptyPrompt"); //$NON-NLS-1$
		}
