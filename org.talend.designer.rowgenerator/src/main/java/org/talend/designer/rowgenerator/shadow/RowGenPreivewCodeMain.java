...
public class RowGenPreivewCodeMain {
	...
	private void initPerlArray() {
		...
		MetadataTable table = (MetadataTable) this.component.getMetadataList()
				.get(0);
		if (map == null || map.isEmpty()) {
			...
			for (IMetadataColumn col : table.getListColumns()) {
				MetadataColumnExt ext = (MetadataColumnExt) col;
				this.component.setColumnValue(ext.getLabel(),
						getOneColData(ext));
			}
		}
	}
	...
	protected void getCodeGen() {
		...
		try {
			IRunProcessService runProcessService = RowGeneratorPlugin
					.getDefault().getRunProcessService();
			IProcessor processor = runProcessService.createCodeProcessor(proc,
					language, false);
			IContext context2 = new org.talend.designer.core.model.context.Context(
					PREVIEW);
	...
	protected Process runPreviewCode() {
		...
		IContext context2 = new org.talend.designer.core.model.context.Context(
				PREVIEW);
		try {
			return processor.run(context2, Processor.NO_STATISTICS,
	...
	public List<List<String>> getResultsByRun(final Button refresh,
			String number) {
		this.number = number;
		results.clear();
