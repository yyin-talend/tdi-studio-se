...
/**
 * This class is used for generating HTML file for Component 'tRowGenerator'.
 * <br/> <br/>
 * 
 */
public class RowGeneratorComponentDocumentation implements
	...
	public URL getHTMLFile() {
		String xmlFilepath = this.tempFolderPath + File.separatorChar
				+ this.componentName + IHTMLDocConstants.XML_FILE_SUFFIX;

		String htmlFilePath = this.tempFolderPath + File.separatorChar
				+ this.componentName + IHTMLDocConstants.HTML_FILE_SUFFIX;
		...
		HTMLHandler.generateHTMLFile(this.tempFolderPath, xslFilePath,
	...
	private void generateXMLInfo() {
		...
		externalNodeElement.addAttribute("name", HTMLDocUtils
				.checkString(this.componentName));
		externalNodeElement.addAttribute("preview", HTMLDocUtils
				.checkString(this.previewPicPath));
	...
	private void generateColumnInfo(Element externalNodeElement) {
		...
		for (IMetadataTable metadataTable : this.metadataListOut) {
			...
			for (IMetadataColumn tempColumn : metadataTable.getListColumns()) {
				...
				Element columnElement = externalNodeElement
						.addElement("column");
				columnElement.addAttribute("name", HTMLDocUtils
						.checkString(column.getLabel()));
				columnElement.addAttribute("key", String
						.valueOf(column.isKey()));
				columnElement.addAttribute("type", HTMLDocUtils
						.checkString(column.getTalendType()));
				columnElement.addAttribute("nullable", String.valueOf(column
						.isCustom()));
				columnElement.addAttribute("length", String.valueOf(column
						.getLength()));
				columnElement.addAttribute("precision", String.valueOf(column
						.getPrecision()));
				columnElement.addAttribute("default", HTMLDocUtils
						.checkString(column.getDefault()));
				columnElement.addAttribute("comment", HTMLDocUtils
						.checkString(column.getComment()));
				columnElement.addAttribute("functions", HTMLDocUtils
						.checkString(column.getFunction().getName()));
				columnElement.addAttribute("parameters", HTMLDocUtils
						.checkString(column.getParameter()));
