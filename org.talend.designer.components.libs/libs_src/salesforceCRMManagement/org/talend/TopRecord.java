...
public class TopRecord {
	...
	private void processQueryResult(QueryResult qr, String prefixName)
			throws Exception {
		...
		for (int i = 0; i < qr.getRecords().length; i++) {
			...
			processSObject(record, prefixName + TopConfig.COLUMNNAME_DELIMTER
					+ record.getType());

		}
	}

	public void processSObject(SObject record, String prefixName)
			throws Exception {
			...
			if (fields[j].getValue() == null) {
				...
				if (objectValue != null) {
					...
					if (objectValue instanceof SObject) {
						...
						processSObject(sobject, prefixName
								+ TopConfig.COLUMNNAME_DELIMTER + sobject.getType());
				...
				String newPrefixName = prefixName + TopConfig.COLUMNNAME_DELIMTER
						+ fields[j].getName();
				...
				if (value != null) {
					...
					valueMap.put(newPrefixName, value + TopConfig.VALUE_DELIMITER
							+ fields[j].getValue());
				} else {
