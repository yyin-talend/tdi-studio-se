...
public class tPaloDimensions {
	...
	//private Set<tPaloDimensionElements> lstPaloDimensionElements = new HashSet<tPaloDimensionElements>();
		...
		for(String[] strMainIn: lstTalendMainIn){
			for(int i=0;i<2;i++){
				...
				for(tPaloDimensionElements tParentDimensionElement : lstPaloDimensionElements){
					if(tParentDimensionElement.getElementName().equals(strMainIn[i])){
						bFound=true;
						break;
					}
				}
	...
	private int getElementLevel(String strParentName, int iPos){
		...
		String strX="";
		for(String[] strMainIn: lstTalendMainIn){
			if(strMainIn[1].equals(strParentName) && !strMainIn[0].equals(strParentName)){
				// SubLevel
				bFound=true;
				strX = strMainIn[0];
				iPos++;
		...
		for(String[] strMainIn : lstTalendMainIn){
			for(int i=0;i<strMainIn.length;i++){
				...
				if(!bElemenExists){
					...
					if(i>0){
						for(tPaloDimensionElements tParentDimensionElement : lstPaloDimensionElements){
							if(tParentDimensionElement.getElementName().equals(strMainIn[i-1])){
								iParentPosition = tParentDimensionElement.getPosition();
								break;
							}
						}
					}
					tPaloDimensionElements tDimensionElement = new tPaloDimensionElements(iPosition, strMainIn[i], iParentPosition, i );
					lstPaloDimensionElements.add(tDimensionElement);
					iPosition++;
				}
			}
	...
	public void buildConsolidationNormal(paloelements paloElements, String strElementName, int iParentPosition) throws paloexception{
			...
			for(tPaloDimensionElements tConsElement : lstPaloDimensionElements){
				if(tConsElement.getParentPosition() == iParentPosition && !tConsElement.getElementName().equals(strElementName)){
			...
			while(i.hasNext()){
				...
				if(null!=plElm){
				    ...
				    for (int zi = 0; itSorted.hasNext(); zi++) {
			    		...
			    		sbTypes2.append("1");
				        //merged[zi] = ((Integer)itSorted.next()).intValue();
				    }
				    
				//System.out.println(sbValues2.toString() + " " + sbTypes2.toString()); 
					
					//System.out.println("Jetzt");
					//try{
						if(hS!=null) plElm.updateElementConsolidation(sbValues2.toString(), sbTypes2.toString(), false);
					//}catch(Exception e){
