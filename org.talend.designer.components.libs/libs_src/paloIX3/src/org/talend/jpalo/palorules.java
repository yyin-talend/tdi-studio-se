...
public class palorules {
		...
		try{
			...
			while(csv.readRecord()){
				...
				paloRules.add(new palorule(
	        			...
	        			palohelpers.StringToBoolean(csv.get(5))
					)
				);
			}
        	csv.close();
	...
	public palorule getRule(long lIdentifier){
		for(palorule paloRule : paloRules){
			if(paloRule.getIdentifier()==lIdentifier) return paloRule;
		}
		return null;
	}
	...
	public palorule getRule(String strExtern_Id){
		for(palorule paloRule : paloRules){
			if(paloRule.getExtern_Id().equals(strExtern_Id)) return paloRule;
		}
		return null;
	}
