...
public final class JSONConnectionContextHelper {
    ...
    public static boolean showContextdialog(IProcess2 process, ContextItem contextItem, IContextManager contextManager,
        ...
        if (showDialog.open() == Window.OK) {
            if (JSONConnectionContextHelper.containsVariable(contextManager)) {
                Set<String> addedContext = JSONConnectionContextHelper.checkAndAddContextVariables(contextItem, addedVars,
                        contextManager, false);
                if (addedContext != null && addedContext.size() > 0) {
                    JSONConnectionContextHelper.addContextVarForJob(process, contextItem, addedVars);
                }
            } else {
    ...
    public static JSONFileConnection cloneOriginalValueJSONFileConnection(JSONFileConnection fileConn, ContextType contextType) {
        ...
        String filePath = getOriginalValue(contextType, fileConn.getJSONFilePath());
        String encoding = getOriginalValue(contextType, fileConn.getEncoding());
        ...
        for (JSONXPathLoopDescriptor desc : descs) {
            ...
            String xPathQuery = getOriginalValue(contextType, desc.getAbsoluteXPathQuery());
            ...
            for (SchemaTarget schemaTarget : schemaTargets) {
                SchemaTarget cloneSchemaTarget = JsonFactory.eINSTANCE.createSchemaTarget();
