...
//    }
	...
	private static final String LABLE_SEARCH = "search";
	...
	private Composite createLeftComponent() {
		...
		searchButton.setText(LABLE_SEARCH);
		final TreeViewer viewer = new TreeViewer(treeComposite, SWT.SINGLE
					...
					try {
						if (searchAdapter.getMatchTextFlag()) {
							htmlContent = HEADTAG_HTML
									+ searchAdapter.getCurrentStr()
									+ TAILTAG_HTML;
							htmlText.setText(htmlContent);
							searchAdapter.setMatchTextFlag(false);
						} else {
							String midContent = DocParser.getInstance().getDoc(
									anchor);
							htmlContent = HEADTAG_HTML + midContent
									+ TAILTAG_HTML;
							htmlText.setText(htmlContent);
						}
						return;
