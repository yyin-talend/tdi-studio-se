...
public class JobExportAction implements IRunnableWithProgress {
            ...
            if (jobVersion != null && jobVersion.equals(JobScriptsExportWizardPage.ALL_VERSIONS)) {
				...
				for (String version : allVersions) {
					monitor.subTask(Messages
							.getString(
									"JobScriptksExportWizardPage.exportJob0", node.getLabel(), version)); //$NON-NLS-1$
					if (!exportJobScript(nodes, version, bundleVersion,
							progressMonitor)) {
						return;
					}
				}
				...
				if (!exportJobScript(nodes, jobVersion, bundleVersion,
						progressMonitor)) {
					return;
				}
			}
	...
	boolean exportJobScript(List<RepositoryNode> nodes, String version,
			String bundleVersion, IProgressMonitor monitor) {
