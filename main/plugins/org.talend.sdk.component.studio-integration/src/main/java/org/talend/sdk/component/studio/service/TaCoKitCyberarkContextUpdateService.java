...
public class TaCoKitCyberarkContextUpdateService extends AbstractRepositoryContextUpdateService {
	...
	public boolean updateContextParameter(Connection conn, String oldValue, String newValue) {
		...
		if (conn.isContextMode()) {
			...
			ValueModel credProviderServicePortValue;

			try {
				credProviderServicePortValue = taCoKitConfigurationModel
						.getValue("configuration.credProviderServicePort");

				if (credProviderServicePortValue != null
						&& StringUtils.equals(oldValue, credProviderServicePortValue.getValue())) {
					taCoKitConfigurationModel.setValue("configuration.credProviderServicePort",
							credProviderServicePortValue.getValue());
					isModified = true;
				}
			} catch (Exception e) {
