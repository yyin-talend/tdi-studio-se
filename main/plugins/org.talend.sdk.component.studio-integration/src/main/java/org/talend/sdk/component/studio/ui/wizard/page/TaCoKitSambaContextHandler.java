...
public class TaCoKitSambaContextHandler extends AbstractRepositoryContextHandler {
	...
	public Set<ETaCoKitParamName> collectConParameters() {
		...
		set.add(ETaCoKitParamName.Host);
		set.add(ETaCoKitParamName.Domain);
	...
	public List<IContextParameter> createContextParameters(String prefixName, Connection connection,
		...
		for (IConnParamName param : paramSet) {
			if (param instanceof ETaCoKitParamName) {
				...
				case Host:
					ConnectionContextHelper.createParameters(varList, paramName, properties.get("configuration.host"));
					break;
				case Domain:
					ConnectionContextHelper.createParameters(varList, paramName,
							properties.get("configuration.domain"));
					break;
				case Password:
					ConnectionContextHelper.createParameters(varList, paramName,
	...
	public void revertPropertiesForContextMode(Connection connection, ContextType contextType) {
		...
		TaCoKitConfigurationModel taCoKitConfigurationModel = new TaCoKitConfigurationModel(connection);
		revertProperties(taCoKitConfigurationModel, contextType, "configuration.host");
		revertProperties(taCoKitConfigurationModel, contextType, "configuration.domain");
	...
	protected void matchContextForAttribues(Connection connection, IConnParamName param, String contextVariableName) {
		...
		if (param instanceof ETaCoKitParamName) {
			...
			switch (hadoopParam) {
			case Host:
				taCoKitConfigurationModel.setValue("configuration.host",
						ContextParameterUtils.getNewScriptCode(contextVariableName, LANGUAGE));
				break;
			case Domain:
				taCoKitConfigurationModel.setValue("configuration.domain",
						ContextParameterUtils.getNewScriptCode(contextVariableName, LANGUAGE));
				break;
			case Password:
