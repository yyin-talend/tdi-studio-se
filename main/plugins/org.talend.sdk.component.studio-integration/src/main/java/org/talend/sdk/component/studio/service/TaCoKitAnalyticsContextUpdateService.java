...
public class TaCoKitAnalyticsContextUpdateService extends AbstractRepositoryContextUpdateService {
	...
	public boolean updateContextParameter(Connection conn, String oldValue, String newValue) {
		...
		if (conn.isContextMode()) {
			...
			try {
				jsonCredentialsValue = taCoKitConfigurationModel.getValue("configuration.jsonCredentials");
				if (jsonCredentialsValue != null && StringUtils.equals(oldValue, jsonCredentialsValue.getValue())) {
					taCoKitConfigurationModel.setValue("configuration.jsonCredentials",
