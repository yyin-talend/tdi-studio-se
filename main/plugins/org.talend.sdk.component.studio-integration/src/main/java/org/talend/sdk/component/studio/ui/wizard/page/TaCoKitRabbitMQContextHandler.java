...
public class TaCoKitRabbitMQContextHandler extends AbstractRepositoryContextHandler {
	...
	public Set<ETaCoKitParamName> collectConParameters() {
		...
		set.add(ETaCoKitParamName.Port);
		set.add(ETaCoKitParamName.Hostname);
	...
	public List<IContextParameter> createContextParameters(String prefixName, Connection connection,
		...
		for (IConnParamName param : paramSet) {
			if (param instanceof ETaCoKitParamName) {
				...
				case Port:
					ConnectionContextHelper.createParameters(varList, paramName, properties.get("configuration.port"));
					break;
				case Hostname:
					ConnectionContextHelper.createParameters(varList, paramName,
							properties.get("configuration.hostname"));
					break;
				case Password:
					ConnectionContextHelper.createParameters(varList, paramName,
	...
	public void revertPropertiesForContextMode(Connection connection, ContextType contextType) {
		...
		TaCoKitConfigurationModel taCoKitConfigurationModel = new TaCoKitConfigurationModel(connection);
		revertProperties(taCoKitConfigurationModel, contextType, "configuration.port");
		revertProperties(taCoKitConfigurationModel, contextType, "configuration.hostname");
	...
	protected void matchContextForAttribues(Connection connection, IConnParamName param, String contextVariableName) {
		...
		if (param instanceof ETaCoKitParamName) {
			...
			switch (hadoopParam) {
			case Port:
				taCoKitConfigurationModel.setValue("configuration.port",
						ContextParameterUtils.getNewScriptCode(contextVariableName, LANGUAGE));
				break;
			case Hostname:
				taCoKitConfigurationModel.setValue("configuration.hostname",
						ContextParameterUtils.getNewScriptCode(contextVariableName, LANGUAGE));
				break;
			case Password:
