...
public class TaCoKitZendeskContextUpdateService extends AbstractRepositoryContextUpdateService {
	...
	public boolean updateContextParameter(Connection conn, String oldValue, String newValue) {
		...
		if (conn.isContextMode()) {
			...
			ValueModel authenticationLoginValue;
			ValueModel serverUrlValue;
			ValueModel apiTokenValue;
			try {
				authenticationLoginValue = taCoKitConfigurationModel.getValue("configuration.authenticationLogin");
				serverUrlValue = taCoKitConfigurationModel.getValue("configuration.serverUrl");
				apiTokenValue = taCoKitConfigurationModel.getValue("configuration.apiToken");
				if (authenticationLoginValue != null
						&& StringUtils.equals(oldValue, authenticationLoginValue.getValue())) {
					taCoKitConfigurationModel.setValue("configuration.authenticationLogin",
							authenticationLoginValue.getValue());
					isModified = true;
					...
					isModified = true;
				} else if (apiTokenValue != null && StringUtils.equals(oldValue, apiTokenValue.getValue())) {
					taCoKitConfigurationModel.setValue("configuration.apiToken", apiTokenValue.getValue());
					isModified = true;
				}
			} catch (Exception e) {
