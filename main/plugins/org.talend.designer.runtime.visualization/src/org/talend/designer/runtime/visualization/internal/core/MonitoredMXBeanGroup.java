...
public class MonitoredMXBeanGroup implements IMonitoredMXBeanGroup {
     ...
     */
    public MonitoredMXBeanGroup(MBeanServer mBeanServer, String name, AxisUnit axisUnit) {
    ...
    public IMonitoredMXBeanAttribute getAttribute(ObjectName objectName, String attributeName) {
        for (IMonitoredMXBeanAttribute attribute : attributes) {
            if (attribute.getObjectName().equals(objectName) && attribute.getAttributeName().equals(attributeName)) {
                return attribute;
            }
        }
        return null;
    ...
    public void addAttribute(String objectNameString, String attributeName, int[] rgb) throws JvmCoreException {
        for (IMonitoredMXBeanAttribute attribute : attributes) {
            if (attribute.getAttributeName().equals(attributeName)
                    && attribute.getObjectName().getCanonicalName().equals(objectNameString)) {
                return;
            }
        }
    ...
    public void removeAttribute(String objectName, String attributeName) {
        ...
        if (targetAttribute != null) {
            ...
            mBeanServer.fireMBeanServerChangeEvent(new MBeanServerEvent(MBeanServerState.MonitoredAttributeRemoved,
                    targetAttribute));
        }
    }
     ...
     */
    protected void addAttribute(String objectNameString, String attributeName, int[] rgb, boolean fireEvent)
            throws JvmCoreException {
