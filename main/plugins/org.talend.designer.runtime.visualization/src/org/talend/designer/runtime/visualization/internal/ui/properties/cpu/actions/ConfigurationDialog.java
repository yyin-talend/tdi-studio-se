...
import org.talend.designer.runtime.visualization.internal.ui.properties.cpu.PackageLabelProvider;
...
public class ConfigurationDialog extends SelectionDialog {
     ...
     * @param profilerType The profiler type
     * @param samplingPeriod The sampling period
     * @param bciProfilerState The state for BCI profiler
     * @param packages The packages
     */
    public ConfigurationDialog(Shell parentShell, ProfilerType profilerType, int samplingPeriod, ProfilerState bciProfilerState,
            Set<String> packages) {
        super(parentShell);
    ...
    boolean validate() {
        ...
        } else if (profilerState == ProfilerState.AGENT_NOT_LOADED || profilerState == ProfilerState.UNKNOWN) {
        ...
        bciButton.setEnabled(profilerState == ProfilerState.READY || profilerState == ProfilerState.RUNNING);
    ...
    private void createSamplingPeriodText(Composite parent) {
        ...
        samplingPeriodText.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
        samplingPeriodText.addModifyListener(new ModifyListener() {

            @Override
            public void modifyText(ModifyEvent e) {
                if (validate()) {
                    samplingPeriod = Integer.valueOf(samplingPeriodText.getText());
                }
            }
        });
    }
    ...
    private void createWarningMessageControls(Composite parent) {
        ...
        warningMessageLabel.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
    }
    ...
    private SelectionListener getRemoveButtonListener() {
        return new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                ...
                Object[] selections = ((IStructuredSelection) packagesViewer.getSelection()).toArray();
    ...
    private SelectionListener getAddButtonListener() {
        return new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                Object[] input = (Object[]) packagesViewer.getInput();
                AddPackageDialog dialog = new AddPackageDialog(getShell(), input);
                if (dialog.open() == Window.OK) {
                    ...
                    String[] items = packages.toArray(new String[packages.size()]);
                    Arrays.sort(items);
    ...
    private Image getWarningImage() {
        if (warningImage == null || warningImage.isDisposed()) {
            warningImage = PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_OBJS_WARN_TSK)
                    .createImage();
        }
        return warningImage;
    }
