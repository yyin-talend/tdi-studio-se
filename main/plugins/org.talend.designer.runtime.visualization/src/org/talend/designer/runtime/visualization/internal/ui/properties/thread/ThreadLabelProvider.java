...
import org.talend.designer.runtime.visualization.internal.ui.properties.overview.IFormat;
...
public class ThreadLabelProvider extends LabelProvider implements ITableLabelProvider {
    ...
    private static final ThreadColumn[] COLUMNS_TAKEN_INTO_ACCOUNT_FOR_FILTER = { ThreadColumn.THREAD, ThreadColumn.STATE,
            ThreadColumn.LOCK, ThreadColumn.LOCK_OWNWER };
    ...
    public Image getColumnImage(Object element, int columnIndex) {
        if (columnIndex == getColumnIndex(ThreadColumn.THREAD) && element instanceof IThreadElement) {
    ...
    public String getText(Object obj) {
        if (obj instanceof IThreadElement) {
            ...
            for (ThreadColumn column : COLUMNS_TAKEN_INTO_ACCOUNT_FOR_FILTER) {
                buffer.append(getColumnText(obj, getColumnIndex(column))).append(' ');
            }

            // stack traces
            if (Activator.getDefault().getPreferenceStore().getBoolean(IConstants.WIDE_SCOPE_THREAD_FILTER)) {
                for (StackTraceElement element : ((IThreadElement) obj).getStackTraceElements()) {
                    buffer.append(element.getClassName()).append('.').append(element.getMethodName()).append(' ');
     ...
     * 
     * @param element The thread list element
    ...
    private Image getThreadObjImage(IThreadElement element) {
        if (element.isDeadlocked()) {
            if (threadDeadlockedObjImage == null || threadDeadlockedObjImage.isDisposed()) {
                threadDeadlockedObjImage = Activator.getImageDescriptor(ISharedImages.THREAD_DEADLOCKED_IMG_PATH).createImage();
            }
            return threadDeadlockedObjImage;
        }

        if (element.isSuspended()) {
            if (threadSuspendedObjImage == null || threadSuspendedObjImage.isDisposed()) {
                threadSuspendedObjImage = Activator.getImageDescriptor(ISharedImages.THREAD_SUSPENDED_IMG_PATH).createImage();
            }
        ...
        if (state == State.WAITING || state == State.TIMED_WAITING) {
            if (threadWaitingObjImage == null || threadWaitingObjImage.isDisposed()) {
                threadWaitingObjImage = Activator.getImageDescriptor(ISharedImages.THREAD_WAITING_IMG_PATH).createImage();
            }
            return threadWaitingObjImage;
        } else if (state == State.RUNNABLE) {
            if (threadRunnableObjImage == null || threadRunnableObjImage.isDisposed()) {
                threadRunnableObjImage = Activator.getImageDescriptor(ISharedImages.THREAD_RUNNABLE_IMG_PATH).createImage();
            }
            return threadRunnableObjImage;
        } else if (state == State.BLOCKED) {
            if (threadBlockedObjImage == null || threadBlockedObjImage.isDisposed()) {
                threadBlockedObjImage = Activator.getImageDescriptor(ISharedImages.THREAD_BLOCKED_IMG_PATH).createImage();
            }
