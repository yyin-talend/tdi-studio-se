...
public class TimelineChart extends Chart implements IPropertyChangeListener {
     ...
     * @param group The monitored attribute group
    ...
    public void propertyChange(PropertyChangeEvent event) {
        if (IConstants.LEGEND_VISIBILITY.equals(event.getProperty()) && !isDisposed()) {
     ...
     */
    public void refresh() {
        refreshJob = new RefreshJob(NLS.bind(Messages.refreshChartJobLabel, attributeGroup.getName()), sectionId
     ...
     * @return The series
     */
    private ISeries addMonitoredSeries(final IMonitoredMXBeanAttribute attribute, boolean enableArea) {
        ILineSeries series = (ILineSeries) getSeriesSet().createSeries(SeriesType.LINE, getSeriesId(attribute));
     ...
     * 
     * @param rgb The RGB
     * @return The stored color corresponding to the given RGB, or <tt>null</tt> if not found
     */
    private Color getStoredColor(int[] rgb) {
        for (Color color : colors) {
            if (color.getRGB().red == rgb[0] && color.getRGB().green == rgb[1] && color.getRGB().blue == rgb[2]) {
                return color;
            }
        }
        return null;
    ...
    void refreshChartWidget() {
        ...
        for (IMonitoredMXBeanAttribute attribute : attributeGroup.getAttributes()) {
            ...
            if (series == null) {
                boolean enableArea = false;
                if (attribute.getAttributeName().equals(MonitorAttributeName.HEAP_MEMORY_USE)
                        || attribute.getAttributeName().equals(MonitorAttributeName.HEAP_MEMORY_SIZE)) {
                    enableArea = true;
                }
        ...
        if (axisUnit == AxisUnit.MBytes) {
            getAxisSet().getYAxis(0).getTick().setFormat(new DecimalFormat("#####.#M")); //$NON-NLS-1$
            getAxisSet().adjustRange();
        } else if (axisUnit == AxisUnit.Percent) {
            getAxisSet().getYAxis(0).getTick().setFormat(new DecimalFormat("###%")); //$NON-NLS-1$
