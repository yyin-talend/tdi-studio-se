...
import org.talend.designer.runtime.visualization.ITerminatedJvm;
...
public class Host implements IHost {
    ...
    public IActiveJvm addRemoteActiveJvm(int port, String userName, String password, int updatePeriod) throws JvmCoreException {
        ...
        IActiveJvm jvm = new ActiveJvm(port, userName, password, this, updatePeriod);
    ...
    public IPath getHostDir() throws JvmCoreException {
        IPath hostDir = Activator.getDefault().getStateLocation().append(File.separator + hostName + Host.DIR_SUFFIX);
        if (!hostDir.toFile().exists() && !hostDir.toFile().mkdir()) {
            throw new JvmCoreException(IStatus.ERROR, NLS.bind(Messages.createDirectoryFailedMsg, hostDir.toFile().getName()),
                    null);
        }
        return hostDir;
    }
