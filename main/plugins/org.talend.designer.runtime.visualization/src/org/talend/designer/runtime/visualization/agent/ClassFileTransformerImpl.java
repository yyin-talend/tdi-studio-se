 ...
 *******************************************************************************/
package org.talend.designer.runtime.visualization.agent;
...
import org.talend.designer.runtime.visualization.agent.asm.ClassAdapter;
import org.talend.designer.runtime.visualization.agent.asm.ClassReader;
import org.talend.designer.runtime.visualization.agent.asm.ClassWriter;
...
public class ClassFileTransformerImpl implements ClassFileTransformer {
     ...
     * The constructor.
     * 
     * @param targetClasses The target classes to transform
     * @param transformedClasses The transformed classes
     */
    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
        ...
        if (!isProfiledClassLoader(loader) || !matches(className, Config.getInstance().profiledPackages)
                || matches(className, Config.getInstance().ignoredPackages)) {
            return classfileBuffer;
        }

        /*
         * Classes could be loaded after collecting targetClasses in CpuBciProfilerMXBeanImpl.setFilter(String, String)
         */
    ...
    private static boolean matches(String className, Set<String> packages) {
        ...
        for (String pkg : packages) {
            if (pkg.endsWith("*")) {
                if (packageName.concat(".").startsWith(pkg.substring(0, pkg.length() - 1))) {
                    return true;
                }
            } else {
    ...
    /**
     * Gets the state indicating if the classes loaded by given class loader are profiled.
     * 
     * @param loader The class loader
     * @return <tt>true</tt> if the classes loaded by given class loader are profiled
     */
    private static boolean isProfiledClassLoader(ClassLoader loader) {
        ...
        for (String classLoader : Config.getInstance().profiledClassLoaders) {
            if (loader != null && loader.getClass().getName().equals(classLoader)) {
                return true;
            }
        }
        return false;
