...
/**
 * The active JVM that means the JVM is running. If JVM is terminated, it becomes {@link ITerminatedJvm}.
 */
public interface IActiveJvm extends IJvm {

    /**
     * Connects the target JVM with JVM Monitor via JMX. The client can check with {@link #isConnectionSupported()}
     * whether the target JVM supports connection.
     * 
     * @param updatePeriod The update period
     * @throws JvmCoreException if connecting JVM fails
     */
    ...
    /**
     * Gets the state indicating if the target JVM is connected with JVM Monitor.
     * 
