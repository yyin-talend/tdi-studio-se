...
public class RunProcessContext {
				...
				public void run() {
					if (display.getActiveShell() != null) {
						...
						if (!MessageDialogWithToggle.ALWAYS.equals(store.getString(TOGGLE_MVN_ONLINE))) {
							MessageDialogWithToggle dlg = MessageDialogWithToggle.openOkCancelConfirm(wizard,
									Messages.getString("MavenOnlineRunner.MavenConfirm"),
    ...
    public void exec(final Shell shell) {
		boolean isMsBuildType = Arrays.asList("ROUTE_MICROSERVICE", "REST_MS")
                                ...
                                display.syncExec(new Runnable() {
                                    ...
                                    public void run() {
										if (showMavenOnlineInfoMsg) {
											final String mavenOnlineError = Messages
													.getString("MavenOnlineRunner.info"); //$NON-NLS-1$
