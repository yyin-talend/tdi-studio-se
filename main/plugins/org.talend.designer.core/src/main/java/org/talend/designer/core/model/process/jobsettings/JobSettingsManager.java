...
public class JobSettingsManager {
    ...
    private static List<IElementParameter> getExtraOnDBParameters(IProcess process) {
        ...
        if (process.getElementParameter(dbVersionName) == null) {
            ...
            param.setRequired(true);
            param.setShowIf(dbCondition
                    + " and (" + dbTypeName + " == 'OCLE' or " + dbTypeName + " == 'OCLE_OCI' or " + dbTypeName + " =='ACCESS' or " + dbTypeName + " =='MSSQL' or " + dbTypeName + " =='POSTGRESQL' or "+dbTypeName + " =='MYSQL') "); //$NON-NLS-1$ //$NON-NLS-2$//$NON-NLS-3$ //$NON-NLS-4$//$NON-NLS-5$
            param.setGroup(IMPLICIT_GROUP);
            paramList.add(param);
        }
