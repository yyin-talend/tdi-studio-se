...
import static org.talend.designer.runtime.visualization.internal.tools.IConstants.*;
...
import org.talend.designer.runtime.visualization.IAgentLoadHandler;
...
/**
 * The agent load handler that loads the agent jar file <tt>lib/jvmmonitor-agent.jar</tt> to target JVM.
 */
public class AgentLoadHandler implements IAgentLoadHandler {
    ...
    public void loadAgent(IActiveJvm jvm) throws JvmCoreException {
        ...
        try {
            ...
            Activator.log(IStatus.ERROR, NLS.bind(Messages.loadAgentFailedMsg, agentJarPath), e);
        } finally {
    ...
    private void searchAgentJar() {
        URL entry = Activator.getDefault().getBundle().getEntry(BUNDLE_ROOT_PATH);
        ...
        if (!agentJar.exists()) {
            Activator.log(IStatus.ERROR, NLS.bind(Messages.agentJarNotFoundMsg, agentJar.getAbsolutePath()), new Exception());
        }

        agentJarPath = agentJar.getAbsolutePath();

        Activator.log(IStatus.INFO, NLS.bind(Messages.agentJarFoundMsg, agentJarPath), new Exception());
    }
}
