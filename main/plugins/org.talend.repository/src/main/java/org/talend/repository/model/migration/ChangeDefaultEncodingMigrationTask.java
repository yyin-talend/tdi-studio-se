	...
	private class AddExecuteMode implements IComponentConversion {
		...
		public void transform(NodeType node) {
			...
			if (encoding != null) {
				...
				if ("tSybaseOutputBulkExec".equals(componeName)) { //$NON-NLS-1$
					...
					if (sybaseEncoding == null) {
						...
						sybaseEncoding = ComponentUtilities.getNodeProperty(node, "SYBASE_ENCODING"); //$NON-NLS-1$
						changeEncodingValue(value, sybaseEncoding);
					}
				} else if ("tSybaseBulkExec".equals(componeName)) { //$NON-NLS-1$
					changeEncodingValue(value, encoding);
				} else if ("tOracleBulkExec".equals(componeName)) { //$NON-NLS-1$
					...
					if (enableEditOracleEncoding != null) {
						...
						if (oracleEnconding != null) {
							if("true".equals(enableEditOracleEncodingValue)){ //$NON-NLS-1$
								String oracleEncodingValue = oracleEnconding.getValue(); //$NON-NLS-1$
								encoding.setValue(oracleEncodingValue);
								ComponentUtilities.removeNodeProperty(node,"ENABLE_EDIT_ORACLE_ENCODING"); //$NON-NLS-1$
								ComponentUtilities.removeNodeProperty(node,"ORACLE_ENCODING"); //$NON-NLS-1$
							}else{
								ComponentUtilities.removeNodeProperty(node,"ENABLE_EDIT_ORACLE_ENCODING"); //$NON-NLS-1$
								ComponentUtilities.removeNodeProperty(node,"ORACLE_ENCODING"); //$NON-NLS-1$
								changeEncodingValue(value, encoding);
				...
				} else if ("tOracleOutputBulkExec".equals(componeName)) { //$NON-NLS-1$
					ElementParameterType enableEditOracleEncoding = ComponentUtilities.getNodeProperty(node, "ENABLE_EDIT_ORACLE_ENCODING"); //$NON-NLS-1$
					if (enableEditOracleEncoding != null) {
						ElementParameterType oracleEnconding = ComponentUtilities.getNodeProperty(node, "ORACLE_ENCODING"); //$NON-NLS-1$
						if (oracleEnconding != null) {
							...
							if ("true".equals(enableEditOracleEncodingValue)) { //$NON-NLS-1$
								ComponentUtilities.removeNodeProperty(node,"ENABLE_EDIT_ORACLE_ENCODING"); //$NON-NLS-1$
								oracleEnconding.setField("OPEN_LIST"); //$NON-NLS-1$
							} else {
								ComponentUtilities.removeNodeProperty(node,"ENABLE_EDIT_ORACLE_ENCODING"); //$NON-NLS-1$
								oracleEnconding.setField("OPEN_LIST"); //$NON-NLS-1$
						...
						enableEditOracleEncoding = ComponentUtilities.getNodeProperty(node, "ORACLE_ENCODING"); //$NON-NLS-1$
