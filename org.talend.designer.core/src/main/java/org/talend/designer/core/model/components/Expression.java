...
public final class Expression {
    ...
    public static boolean evaluate(final String string, List<? extends IElementParameter> listParam) {
        if (string.contains("(") && (isThereCondition(string, AND) || isThereCondition(string, OR))) {
    ...
    public static boolean isThereCondition(String expression, String condition) {
        ...
        if (expression.matches("(.*)[')][ ]*" + condition.toUpperCase() + "[ ]*[\\w(](.*)")) {
            return true;
        }
        return false;
    }
            ...
            for (IElementParameter param : listParam) {
                if (param.getName().equals(variableName)) {
                    ...
                    if (param.getListItemsValue(ECodeLanguage.PERL) instanceof String[]) {
                        String[] values = (String[]) param.getListItemsValue(ECodeLanguage.PERL);
                        for (int i = 0; i < values.length && !found; i++) {
                            if (values[i].equals(param.getValue())) {
                                String variableCode = param.getListItemsDisplayCodeName(ECodeLanguage.PERL)[i];
    ...
    private static Expression evaluateExpression(Expression expression, List<? extends IElementParameter> listParam) {
        ...
        for (int i = 0; i < string.length() && !conditionFound; i++) {
                ...
                if (expressionLevel == 0) {
                    if (isThereCondition(string, AND) || isThereCondition(string, OR)) {
                        ...
                        if (isThereCondition(leftString, AND) || isThereCondition(leftString, OR)) {
                            ...
                            // debug: System.out.println(leftString + " => " + leftExpression.isValid());
                        }
                    } else {
                        String newValue; // remove brackets
                        newValue = string.replace("(", "");
            ...
            if (conditionFound) {
                if (expression.getLeftExpression() == null) { // no bracket == evaluate expression
                ...
                if (rightString.contains("(") || isThereCondition(rightString, AND)
                    ...
                    // debug: System.out.println(rightString + " => " + rightExpression.isValid());
                }
        ...
        // if after an expression between bracket there's no other expression, then the validation of the expression
        ...
        // debug: System.out.println(expression.getExpressionString() + " => " + expression.isValid());
        return expression;
    }
}
