...
public class ChangeValuesFromRepository extends ChangeMetadataCommand {
	...
	public ChangeValuesFromRepository(Element elem, Connection connection,
	...
	public void execute() {
		...
		IElementParameter elemParam = elem
				.getElementParameter(EParameterName.REPOSITORY_ALLOW_AUTO_SWITCH
						.getName());
		if (elemParam != null) {
		...
		if (!allowAutoSwitch && (elem instanceof Node)) {
			...
			// query is not set.
			Node node = (Node) elem;
			boolean isSchemaEmpty = node.getMetadataList().get(0)
					.getListColumns().size() == 0;
			...
			for (IElementParameter curParam : node.getElementParameters()) {
				if (curParam.getField().equals(EParameterFieldType.MEMO_SQL)) {
					...
					if (curParam.getDefaultValues().size() > 0) {
						defaultValue = (String) curParam.getDefaultValues()
								.get(0).getDefaultValue();
					}
					String paramValue = (String) curParam.getValue();
					isQueryEmpty = paramValue.equals("")
							|| paramValue.equals("''")
							|| paramValue.equals("\"\"")
		...
		if (propertyName.equals(EParameterName.PROPERTY_TYPE.getName())) {
			elem
					.setPropertyValue(EParameterName.PROPERTY_TYPE.getName(),
							value);
			if (allowAutoSwitch) {
				setOtherProperties();
			}
		} else {
			oldMetadata = (String) elem
					.getPropertyValue(EParameterName.REPOSITORY_PROPERTY_TYPE
							.getName());
			elem.setPropertyValue(EParameterName.REPOSITORY_PROPERTY_TYPE
					.getName(), value);
			if (allowAutoSwitch) {
				setOtherProperties();
			}
		...
		if (propertyName.equals(EParameterName.PROPERTY_TYPE.getName())
			...
			for (IElementParameter param : elem.getElementParameters()) {
				...
				if (param.isShow(elem.getElementParameters())
					...
					Object objectValue = RepositoryToComponentProperty
							.getValue(connection, repositoryValue);
					if (objectValue != null) {
						oldValues.put(param.getName(), param.getValue());

						if (param.getField().equals(
							...
							for (int i = 0; (i < list.length) && (!found); i++) {
								if (objectValue.equals(list[i])) {
									found = true;
									elem.setPropertyValue(param.getName(),
	...
	private String getFirstRepositoryTable(IElementParameter schemaParam,
			String repository) {
		IElementParameter repositorySchemaTypeParameter = schemaParam
				.getChildParameters().get(
						EParameterName.REPOSITORY_SCHEMA_TYPE.getName());
		String[] listId = (String[]) repositorySchemaTypeParameter
				.getListItemsValue();
	...
	private void setOtherProperties() {
		...
		if (elem instanceof Node) {
			...
			boolean hasSchema = elem
					.getElementParameterFromField(EParameterFieldType.SCHEMA_TYPE) != null;
			if (value.equals(EmfComponent.BUILTIN)) {
				if (!metadataInput && hasSchema) {
					elem.setPropertyValue(EParameterName.SCHEMA_TYPE.getName(),
							value);
				}
				elem.setPropertyValue(EParameterName.QUERYSTORE_TYPE.getName(),
						value);
			} else {
				if (hasSchema) {
					for (IElementParameter param : elem.getElementParameters()) {
						if (param.getField().equals(
								EParameterFieldType.SCHEMA_TYPE)) {
							if (!metadataInput) {
								param.getChildParameters().get(
										EParameterName.SCHEMA_TYPE.getName())
										.setValue(EmfComponent.REPOSITORY);
								IElementParameter repositorySchemaTypeParameter = param
										.getChildParameters()
										.get(
												EParameterName.REPOSITORY_SCHEMA_TYPE
														.getName());
								String repositoryTable;
								if (propertyName
										.equals(EParameterName.PROPERTY_TYPE
												.getName())) {
									repositoryTable = (String) repositorySchemaTypeParameter
											.getValue();
								} else {
									repositoryTable = getFirstRepositoryTable(
											param, value);
									repositorySchemaTypeParameter
											.setValue(repositoryTable);
								}
								IMetadataTable table = repositoryTableMap
										.get(repositoryTable);
								if (table != null) {
									table = table.clone();
									table.setTableName(node.getMetadataList()
											.get(0).getTableName());
									if (!table.sameMetadataAs(node
											.getMetadataList().get(0))) {
										ChangeMetadataCommand cmd = new ChangeMetadataCommand(
												node, param, null, table);
								...
								if (sourceNode != null) {
									IMetadataTable sourceMetadataTable = sourceNode
											.getMetadataList().get(0);
									Object sourceSchema = sourceNode
											.getPropertyValue(EParameterName.SCHEMA_TYPE
													.getName());
									boolean isTake = !sourceNode
											.isExternalNode()
											&& sourceSchema != null
											&& elem
													.getPropertyValue(EParameterName.SCHEMA_TYPE
									...
									if (isTake && getTake()) {
										ChangeMetadataCommand cmd = new ChangeMetadataCommand(
												(Node) elem, param, null,
												sourceMetadataTable);
										cmd.execute(true);
										elem.setPropertyValue(
												EParameterName.SCHEMA_TYPE
														.getName(),
												sourceSchema);
										if (sourceSchema
												.equals(EmfComponent.REPOSITORY)) {
											elem
													.setPropertyValue(
															EParameterName.REPOSITORY_SCHEMA_TYPE
																	.getName(),
															sourceNode
																	.getPropertyValue(EParameterName.REPOSITORY_SCHEMA_TYPE
																			.getName()));
										}
									}
								}
						...
						&& !queriesmap
								.get(
										elem
												.getPropertyValue(EParameterName.REPOSITORY_PROPERTY_TYPE
														.getName())).isEmpty()) {
					elem.setPropertyValue(EParameterName.QUERYSTORE_TYPE
							.getName(), value);
				}
			} else {
				elem.setPropertyValue(EParameterName.QUERYSTORE_TYPE.getName(),
	...
	protected Node getRealSourceNode(INode target) {
		...
		for (org.talend.designer.core.ui.editor.connections.Connection connec : incomingConnections) {
			if (connec.isActivate()
					&& connec.getLineStyle().hasConnectionCategory(
	...
	private boolean getTake() {
		Boolean take = MessageDialog
				.openQuestion(
						new Shell(),
						Messages
								.getString("ChangeValuesFromRepository.messageDialog.take"),
						Messages
	...
	public void undo() {
		// Force redraw of Commponents propoerties
		elem.setPropertyValue(EParameterName.UPDATE_COMPONENTS.getName(),
				new Boolean(true));
			...
			for (IElementParameter param : elem.getElementParameters()) {
				...
				if (param.isShow(elem.getElementParameters())
					...
					Object objectValue = RepositoryToComponentProperty
							.getValue(connection, repositoryValue);
					if (objectValue != null) {
						elem.setPropertyValue(param.getName(), oldValues
								.get(param.getName()));
						param.setRepositoryValueUsed(false);
					}
				}
		...
		if (propertyName.equals(EParameterName.PROPERTY_TYPE.getName())) {
			if (value.equals(EmfComponent.BUILTIN)) {
				elem.setPropertyValue(EParameterName.PROPERTY_TYPE.getName(),
						EmfComponent.REPOSITORY);
			} else {
				elem.setPropertyValue(EParameterName.PROPERTY_TYPE.getName(),
						EmfComponent.BUILTIN);
	 ...
	 */
	public void setMaps(Map<String, List<String>> tablesmap,
			Map<String, List<String>> queriesmap,
