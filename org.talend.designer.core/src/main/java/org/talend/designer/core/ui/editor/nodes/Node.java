...
/**
 * Object that describes the node. All informations on nodes are stored in this
 * class. <br/>
 * 
 * $Id$
 * 
...
public class Node extends Element implements INode {
	...
	public static final Color START_COLOR = new Color(null, new RGB(0xB0, 0xE7,
			0));
	...
	/**
	 * This constructor is called when the node is created from the palette the
	 * unique name will be determined with the number of components of this
	 * type.
	 * 
	 * @param component
	 */
	public Node(IComponent component) {
		...
		if (editorPart instanceof MultiPageTalendEditor) {
			Process activeProcess = ((MultiPageTalendEditor) editorPart)
					.getTalendEditor().getProcess();
			process = activeProcess;
		}
		currentStatus = 0;
		init(component);
		IElementParameter param = getElementParameter(EParameterName.REPOSITORY_ALLOW_AUTO_SWITCH
				.getName());
		if (param != null) {
	...
	private void init(IComponent newComponent) {
		...
		labelToParse = store
				.getString(TalendDesignerPrefConstants.DEFAULT_LABEL);
		hintToParse = store.getString(TalendDesignerPrefConstants.DEFAULT_HINT);
		showHint = store
				.getBoolean(TalendDesignerPrefConstants.DEFAULT_HINT_USED);
		...
		String uniqueName = ((Process) getProcess())
				.generateUniqueNodeName(this);
		((Process) getProcess()).addUniqueNodeName(uniqueName);
		setPropertyValue(EParameterName.UNIQUE_NAME.getName(), uniqueName);

		IElementParameter mappingParameter = MetadataTool
				.getMappingParameter((List<IElementParameter>) this
						.getElementParameters());

		for (IMetadataTable table : metadataList) {
			if (table.getAttachedConnector().equals(
					EConnectionType.FLOW_MAIN.getName())
					|| table.getAttachedConnector().equals(
							EConnectionType.TABLE.getName())) {
				table.setTableName(uniqueName);
			} else {
				table.setTableName(table.getAttachedConnector());
			}
			if (mappingParameter != null) {
				if (mappingParameter.getValue() != null
						&& (!mappingParameter.getValue().equals(""))) {
			...
			for (int i = 0; i < getElementParameters().size(); i++) {
				...
				if (param.getField().equals(EParameterFieldType.SCHEMA_TYPE)
						&& param.getContext().equals(
								table.getAttachedConnector())) {
					if (param.getValue() instanceof IMetadataTable) {
						IMetadataTable paramTable = (IMetadataTable) param
								.getValue();
						table.getListColumns().addAll(
								paramTable.getListColumns());
		...
		setPropertyValue(EParameterName.SHOW_HINT.getName(), new Boolean(
				showHint));
	...
	/**
	 * 
	 * Note that if there is several connectors of the same type, it will return
	 * the first one.
	 * 
	 * @param connType
	 * @return
	...
	/**
	 * 
	 * Note that if there is several connectors the same name, it will return
	 * the first one.
	 * 
	 * @param connName
	 * @return
	...
	public List<INodeConnector> getConnectorsFromType(
			final EConnectionType connType) {
		INodeConnector nodeConnector = null;
		List<INodeConnector> listConnectors = new ArrayList<INodeConnector>();
		int nbConn = 0;
	...
	public void setShowHint(final Boolean showHint) {
		...
		IElementParameter param = getElementParameter(EParameterName.SHOW_HINT
				.getName());
	...
	public String getUniqueName() {
		...
		IElementParameter param = getElementParameter(EParameterName.UNIQUE_NAME
				.getName());
	 ...
	 * 
	 * @param start
	 *            boolean that will give the status
	 */
	public void setStart(final boolean start) {
		this.start = start;
		firePropertyChange(EParameterName.START.getName(), null, null);

		IElementParameter param = getElementParameter(EParameterName.START
				.getName());
	...
	/**
	 * Set the label of a node. <br/><b> /!\ This is the text of the label, not
	 * the name of the component</b>
	 * 
	 * @param titleName
	 */
	...
	public void updateVisibleData() {
		...
		String newshowHintText = ElementParameterParser
				.parse(this, hintToParse);
	...
	public void addInput(final Connection connection) {
		...
		if (!mainConnector.isBuiltIn()
				...
				&& (connection.getLineStyle() == EConnectionType.FLOW_MAIN
						|| (connection.getLineStyle() == EConnectionType.TABLE) || ((connection
						.getLineStyle() == EConnectionType.FLOW_MERGE) && (connection
						.getInputId() == 1)))
			...
			boolean repositoryModeOrReadOnly = false;
			for (IElementParameter param : getElementParameters()) {
				if ((param.getField().equals(EParameterFieldType.SCHEMA_TYPE))
						&& (param.getContext().equals(mainConnector.getName()))) {
					IElementParameter schemaTypeParam = param
							.getChildParameters().get(
									EParameterName.SCHEMA_TYPE.getName());
					if (schemaTypeParam.isReadOnly()
							|| schemaTypeParam.getValue().equals(
			...
			if (repositoryModeOrReadOnly
					&& (connection.getSource().getSchemaParameterFromConnector(
							mainConnector.getName()) != null)) {
				if ((connection.getSource().getOutgoingConnections(
						connection.getConnectorName()).size() == 1)) {
					if (takeSchema == null) {
						takeSchema = getTakeSchema();
					}
					if (takeSchema) {
						connection.getSource().takeSchemaFrom(this,
								mainConnector.getName());
					}
				} else {
				...
				for (INodeConnector connector : getListConnector()) {
					if (mainConnector.getName().equals(
							connector.getBaseSchema())) {

						IMetadataTable targetTable = this
								.getMetadataFromConnector(connector.getName());
						boolean customFound = false;
						for (int i = 0; i < targetTable.getListColumns().size(); i++) {
							IMetadataColumn column = targetTable
									.getListColumns().get(i);
							if (column.isCustom()) {
								customFound = true;
								break;
							}
						}
						if (((customFound && targetTable.isReadOnly())
								|| (outputs.size() == 0) || (connection
	...
	private void takeSchemaFrom(Node nodeTarget, String connector) {
		IElementParameter paramTarget = nodeTarget
				.getSchemaParameterFromConnector(connector);
		IMetadataTable tableTarget = nodeTarget
				.getMetadataFromConnector(connector);
		...
		ChangeMetadataCommand cmc = new ChangeMetadataCommand(this, param,
				null, tableTarget);
		...
		if (schemaParamTarget.getValue().equals(EmfComponent.REPOSITORY)) {
			IElementParameter repositorySchemaParamTarget = paramTarget
					.getChildParameters().get(
							EParameterName.REPOSITORY_SCHEMA_TYPE.getName());
			/*
			 * param.getChildParameters()
			 * .get(EParameterName.SCHEMA_TYPE.getName()).setValue(
			 * EmfComponent.REPOSITORY);
			 */
			param.getChildParameters().get(
					EParameterName.REPOSITORY_SCHEMA_TYPE.getName()).setValue(
					repositorySchemaParamTarget.getValue());
			this.setPropertyValue(EParameterName.SCHEMA_TYPE.getName(),
					EmfComponent.REPOSITORY);
	...
	private CommandStack getCommandStack() {
		...
		if (part instanceof MultiPageTalendEditor) {
			TalendEditor talendEditor = ((MultiPageTalendEditor) part)
					.getTalendEditor();
			cmdStack = (CommandStack) talendEditor
		...
		if (!mainConnector.isBuiltIn() && component.isSchemaAutoPropagated()
			...
			for (INodeConnector connector : getListConnector()) {
				if (mainConnector.getName().equals(connector.getBaseSchema())) {
					IElementParameter schemaParam = getSchemaParameterFromConnector(connector
							.getName());
					IMetadataTable originTable = getMetadataFromConnector(connector
							.getName());
					if ((schemaParam == null || !schemaParam.isReadOnly())
							&& originTable.isReadOnly()) {
						List<IMetadataColumn> columnToSave = new ArrayList<IMetadataColumn>();
						for (IMetadataColumn column : originTable
								.getListColumns()) {
							if (column.isCustom()) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.talend.designer.core.ui.editor.Element#setPropertyValue(java.lang.Object,
	 *      java.lang.Object)
	 */
		...
		if (id.equals(EParameterName.PROCESS_TYPE_CONTEXT.getName())) {
			...
			if (part instanceof MultiPageTalendEditor) {
				if (process.isActivate()
						&& ((MultiPageTalendEditor) part).getProcess().equals(
								process)) {
					ProcessorUtilities
							.generateCode(
									(String) getPropertyValue(EParameterName.PROCESS_TYPE_PROCESS
											.getName()), (String) value, false,
									false,
	...
	private void setActivate(final boolean activate) {
		...
		List<Connection> listConnections = (List<Connection>) this
				.getOutgoingConnections();
		for (int i = 0; i < listConnections.size(); i++) {
			if (listConnections.get(i).getTarget().isActivate()) {
				listConnections.get(i).setPropertyValue(
						EParameterName.ACTIVATE.getName(), activate);
			}
		}
				...
				if (connec.isActivate()) {
					if ((connec.getLineStyle().equals(EConnectionType.RUN_IF)
							|| connec.getLineStyle().equals(
									EConnectionType.RUN_IF_ERROR) || connec
							.getLineStyle().equals(EConnectionType.RUN_IF_OK))) {
						runIf = true;
					}
					if (!runIf) {
				...
				for (int j = 0; j < getIncomingConnections().size(); j++) {
					...
					if (connec.isActivate()) {
						if (connec.getLineStyle().hasConnectionCategory(
								IConnectionCategory.MAIN)) {
							return false;
						}
					}
				}
			} else {
				if (!(Boolean) getPropertyValue(EParameterName.STARTABLE
						.getName())) {
					return false;
				}
			}
	...
	/**
	 * Will return the first item of the subprocess. If "withCondition" is true,
	 * if there is links from type RunIf / RunAfter / RunBefore, it will return
	 * the first element found. If "withCondition" is false, it will return the
	 * first element with no active link from type Main/Ref/Iterate.<br>
	 * <i><b>Note:</b></i> This function doesn't work if the node has several
	 * start points (will return a random start node).
	 * 
	 * @param withCondition
	...
	public Node getSubProcessStartNode(boolean withConditions) {
		...
		for (int j = 0; j < getIncomingConnections().size(); j++) {
			...
			if (!connec.getLineStyle().hasConnectionCategory(
				...
				return connec.getSource()
						.getSubProcessStartNode(withConditions);
			}
		}
		return null;
	}
	...
	private Node getMainBranch() {
		...
		for (int i = 0; i < getOutgoingConnections().size()
				&& targetWithRef == null; i++) {
	...
	private void checkParameters() {
		for (IElementParameter param : this.getElementParameters()) {
			...
			if (param.isRequired() && param.isShow(getElementParameters())) {
				...
				switch (fieldType) {
				case TABLE:
					List<Map<String, String>> tableValues = (List<Map<String, String>>) param
							.getValue();
				...
				case MEMO_SQL:
					...
					// Query" button.
					if (currentQuery.indexOf(COMPARE_STR1) != -1
							|| currentQuery.indexOf(COMPARE_STR2) != -1) {
						Problems.add(ProblemStatus.WARNING, this, errMessage);
						break;
					}
					currentQuery = currentQuery.toUpperCase();
					if (currentQuery.contains("SELECT")
							&& currentQuery.contains("FROM")) {
						currentQuery = currentQuery.substring(currentQuery
								.indexOf("SELECT")
								+ "SELECT".length(), currentQuery
								.indexOf("FROM"));
						...
						if (changedColumnSize == 1 && compareStr.equals("")) {
							Problems.add(ProblemStatus.WARNING, this,
									errMessage);
							break;
						}
						...
						IMetadataTable metaTable = this.getMetadataList()
								.get(0);
						if (metaTable == null
								|| metaTable.getListColumns() == null) {
							break;
						}
						int originColumnSize = metaTable.getListColumns()
								.size();

						if (changedColumnSize != originColumnSize
								&& (compareStr != null && (!compareStr
										.equals("*")))) {
				...
				case CLOSED_LIST:
					...
					if (value.equals("")) { //$NON-NLS-1$
						...
						if (param.getListItemsValue().length != 0) {
							boolean found = false;
							for (int i = 0; i < param.getListItemsValue().length
									&& !found; i++) {
	...
	public int getCurrentActiveLinksNbInput(EConnectionType type) {
		...
		for (Connection connection : inputs) {
			if (connection.isActivate()
					&& connection.getLineStyle().equals(type)) {
				nb++;
			}
		}
		return nb;
	...
	public int getCurrentActiveLinksNbInput(int connCategory) {
		...
		for (Connection connection : inputs) {
			if (connection.isActivate()
					&& connection.getLineStyle().hasConnectionCategory(
							connCategory)) {
				nb++;
			}
		}
	...
	private void checkModules() {
		ILibrariesService moduleService = CorePlugin.getDefault()
				.getLibrariesService();
	...
	private void checkLinks() {
		...
		for (INodeConnector nodeConnector : listConnector) {
			if (!nodeConnector.getDefaultConnectionType()
					.hasConnectionCategory(IConnectionCategory.USE_HASH)
				...
				if (nbMaxOut != -1) {
					if (curLinkOut > nbMaxOut) {
						String errorMessage = "This component has too much \""
								+ typeName + "\" type outputs.";
						Problems.add(ProblemStatus.WARNING, this, errorMessage);
					}
				}
				...
				if (nbMinOut != 0) {
					if (curLinkOut < nbMinOut) {
						String errorMessage = "This component has not enough \""
								+ typeName + "\" type outputs.";
						Problems.add(ProblemStatus.WARNING, this, errorMessage);
					}
				}
	...
	private void checkSchema() {
		...
		if (!isExternalNode()) {
			if (canEditSchema) {
				if ((mainConnector.getMaxLinkInput() == 0)
						&& (mainConnector.getMaxLinkOutput() != 0)) {
					if (getMetadataFromConnector(mainConnector.getName())
				...
				if (getMetadataList() != null) {
					for (IMetadataTable meta : getMetadataList()) {
						if (meta.getListColumns().size() == 0) {
							...
							String errorMessage = "The output schema/link named \""
									+ tableLabel
		...
		if (component.isSchemaAutoPropagated()
			...
			IMetadataTable inputMeta = null, outputMeta = getMetadataList()
					.get(0);
			for (Connection connection : inputs) {
				if (connection.isActivate()
						&& (connection.getLineStyle().equals(
								EConnectionType.FLOW_MAIN) || connection
			...
			if (inputMeta != null) {
				if (!outputMeta.sameMetadataAs(inputMeta,
					...
					String errorMessage = "The schema from the input link \""
		...
		if (component.useMerge()) {
			if (getMetadataList().get(0).getListColumns().size() == 0) {
				...
				boolean isSame = firstSchema.sameMetadataAs(getMetadataList()
						.get(0));
				if (!isSame) {
			...
			if (inputs.size() > 1) {
				...
				for (int i = 1; i < inputs.size(); i++) {
					if (!firstSchema.sameMetadataAs(inputs.get(i)
							.getMetadataTable())) {
						isSame = false;
						break;
					}
	...
	public boolean canModifySchema() {
		...
		List<? extends IElementParameter> listParam = this
				.getElementParameters();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.talend.core.model.process.INode#renameMetadataColumnName(java.lang.String,
	 *      java.lang.String, java.lang.String)
	 */
	public void metadataInputChanged(IODataComponent dataComponent,
			String connectionToApply) {
		log
				.trace("InputChanged : Node=" + this + ", IOData=[" + dataComponent + "] on " + connectionToApply); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		if (externalNode != null) {
	...
	public void metadataOutputChanged(IODataComponent dataComponent,
			String connectionToApply) {
		log
				.trace("OutputChanged : Node=" + this + ", IOData=[" + dataComponent + "] on " + connectionToApply); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		if (externalNode != null) {
			externalNode
					.metadataOutputChanged(dataComponent, connectionToApply);
		}
	}
	...
	public List<? extends IConnection> getOutgoingSortedConnections() {
		return org.talend.core.model.utils.NodeUtil
				.getOutgoingSortedConnections(this);
	}

	public List<? extends IConnection> getMainOutgoingConnections() {
		return org.talend.core.model.utils.NodeUtil
				.getMainOutgoingConnections(this);
	}

	public List<? extends IConnection> getOutgoingConnections(
			EConnectionType connectionType) {
		return org.talend.core.model.utils.NodeUtil.getOutgoingConnections(
				this, connectionType);
	}
	...
	public void renameData(String oldName, String newName) {
		...
		for (IElementParameter param : this.getElementParameters()) {
			if (param.getValue() instanceof String) { // for TEXT / MEMO etc..
				...
				List<Map<String, Object>> tableValues = (List<Map<String, Object>>) param
						.getValue();
	...
	public List<? extends IConnection> getIncomingConnections(
			EConnectionType connectionType) {
		return org.talend.core.model.utils.NodeUtil.getIncomingConnections(
				this, connectionType);
	}
