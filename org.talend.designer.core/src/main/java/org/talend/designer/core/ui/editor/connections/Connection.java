...
/**
 * Class that define the connection. It's the model part of the Gef element
 * <br/>
 * 
 * $Id$
 * 
...
public class Connection extends Element implements IConnection, IPerformance {
	...
	public Connection(Node source, Node target, EConnectionType lineStyle,
			String connectorName, String metaName, String linkName) {
	...
	private void init(Node source, Node target, EConnectionType lineStyle,
			String connectorName, String metaName, String linkName) {
	...
	private void createMeterParameters(Process process) {
		...
		param.setNumRow(1);
		addElementParameter(param);
		Node meterAttached = new Node(ComponentsFactoryProvider.getInstance()
				.get("tFlowMeter"), process);
		for (IElementParameter curParam : meterAttached.getElementParameters()) {
			if (curParam.getCategory() == EComponentCategory.PROPERTY) {
				...
				if (curParam.getShowIf() == null
						|| curParam.getShowIf().equals("")) {
					curParam.setShowIf("MONITOR_CONNECTION == 'true'");
				} else {
					curParam.setShowIf("(" + curParam.getShowIf()
							+ " and MONITOR_CONNECTION == 'true')");
				}
	...
	public void updateName() {
		...
		if (getLineStyle().equals(EConnectionType.TABLE)) {
			...
			if (sourceNodeConnector.getDefaultConnectionType().equals(
					getLineStyle())) { // if it's the standard
				// link
				if (outputId >= 0) {
			...
			} else if (sourceNodeConnector.getName().equals(
				...
				if (outputId >= 0) {
					labelText += " (" + getLineStyle().getDefaultLinkName()
							+ ", " + sourceNodeConnector.getLinkName()
							+ " order:" + outputId + ")";
				} else {
					labelText += " (" + getLineStyle().getDefaultLinkName()
							+ ", " + sourceNodeConnector.getLinkName() + ")";
			...
			if (outputId >= 0) {
				labelText += " (Main order:" + outputId + ", "
						+ getLineStyle().getDefaultLinkName() + " order:"
						+ inputId + ")";
			} else {
				labelText += " (" + getLineStyle().getDefaultLinkName()
						+ " order:" + inputId + ")";
			...
			updateName = true;
		} else if (getLineStyle().equals(EConnectionType.RUN_IF)
			 ...
			 * labelText += " (" + nodeConnector.getLinkName() + ")"; updateName =
			 * true; }
			 */
	...
	/**
	 * Reconnect the connection Used after delete a connection, for the undo
	 * command and when the connection is connected to new source or target.
	 */
	public void reconnect() {
		if (!isConnected) {
			if (lineStyle.equals(EConnectionType.TABLE)) {
				if (uniqueName == null) {
					uniqueName = source.getProcess()
							.generateUniqueConnectionName("table");
				}
				// if (source.getConnectorFromType(lineStyle).isBuiltIn()) {
				IMetadataTable table = getMetadataTable();
			...
			if ((lineStyle.equals(EConnectionType.TABLE) && sourceNodeConnector
					.isBuiltIn())
					|| lineStyle
							.hasConnectionCategory(IConnectionCategory.UNIQUE_NAME)) {
	...
	/**
	 * Disconnect the connection This function is used before delete or
	...
	public void disconnect() {
		if (isConnected) {
			if (!sourceNodeConnector.isBuiltIn()) {
				if (lineStyle
						.hasConnectionCategory(IConnectionCategory.CUSTOM_NAME)) {
	...
	public IMetadataTable getMetadataTable() {
		if (source != null
				&& this.getLineStyle().hasConnectionCategory(
						IConnectionCategory.DATA)) {
			if (sourceNodeConnector.isBuiltIn()) {
				return source.getMetadataTable(metaName);
			} else {
				return source.getMetadataFromConnector(sourceNodeConnector
						.getName());
			}
		}
		return null;
	...
	private void orderConnectionsByMetadata() {
		...
		List<IConnection> connectionList = (List<IConnection>) source
		...
		for (IMetadataTable table : tableList) {
			...
			for (IConnection connection : tmpList) {
				if (connection.getLineStyle().hasConnectionCategory(
						IConnectionCategory.DATA)
						&& connection.getMetadataTable().getTableName().equals(
								tableName)) {
	...
	public int getOutputId(EConnectionType type) {
		...
		case THEN_RUN:
			boolean isExecutionOrder = type
					.hasConnectionCategory(IConnectionCategory.EXECUTION_ORDER);
			boolean isTestedConnectionExecutionOrder;
			if (source != null) {
				for (int i = 0; i < source.getOutgoingConnections().size(); i++) {
					IConnection connection = source.getOutgoingConnections()
							.get(i);
					isTestedConnectionExecutionOrder = connection
							.getLineStyle().hasConnectionCategory(
	...
	public boolean isUseByMetter() {
		...
		List<INode> metterNodes = (List<INode>) sourceNode.getProcess()
				.getNodesOfType("tFlowMeter");
		if (metterNodes.size() > 0) {
			...
			while (it.hasNext()) {
				...
				String absolute = (String) node.getElementParameter("ABSOLUTE")
						.getValue();
				String reference = (String) node.getElementParameter(
						"CONNECTIONS").getValue();

				if (absolute.equals("false")
						&& reference.equals(this.getUniqueName())) {
					return true;
				}
			}
		}
