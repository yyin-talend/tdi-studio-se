...
public class CheckController extends AbstractElementPropertySectionController {
			...
			if (o instanceof Control) {
				...
				if (ctrl.equals(event.getSource())) {
					if (ctrl instanceof Button) {
						...
						// before
						if (!elem.getPropertyValue(name).equals(
								new Boolean(((Button) ctrl).getSelection()))) {
							Command cmd = null;
							Boolean value = new Boolean(((Button) ctrl)
									.getSelection());
							if (name.equals(EParameterName.ACTIVATE.getName())) {
								if (elem instanceof Node) {
									...
									cmd = new ChangeActivateStatusElementCommand(
											value, nodeList, connList);
								}
							} else {
								cmd = new PropertyChangeCommand(elem, name,
										value);
							}
							return cmd;
						}
	...
	public Control createControl(final Composite subComposite,
		...
		final DecoratedField dField = new DecoratedField(subComposite,
				SWT.BORDER, new IControlCreator() {
					...
					public Control createControl(Composite parent, int style) {
						return getWidgetFactory().createButton(parent,
								param.getDisplayName(), SWT.CHECK);
					}
		...
		if (param.isRepositoryValueUsed()) {
			...
			decoration.setDescription(Messages
					.getString("CheckController.decoration.description")); //$NON-NLS-1$
			dField
					.addFieldDecoration(decoration, SWT.RIGHT | SWT.BOTTOM,
							false);
		}
		...
		dynamicTabbedPropertySection.setCurRowSize(initialSize.y
	 ...
	 */
	@Override
	public int estimateRowSize(Composite subComposite,
	...
	@Override
	public void refresh(IElementParameter param,
			boolean checkErrorsWhenViewRefreshed) {
