...
/**
 * The diagram will contain all elements (nodes, connections) The xml that
 * describes the diagram will be saved from the information of this class. <br/>
 * 
 * $Id$
 * 
 */
public class Process extends Element implements IProcess {
	...
	public static final Color READ_ONLY_COLOR = new Color(null, new RGB(0xE7,
			0xE7, 0xE7));

	public static final Color READ_WRITE_COLOR = new Color(null, new RGB(255,
			255, 255));
	...
	private void createMainParameters() {
		...
		param.setDisplayName(EParameterName.COMP_DEFAULT_FILE_DIR
				.getDisplayName());
		param.setNumRow(99);
		param.setShow(false);
		param.setValue(DesignerPlugin.getDefault().getPluginPreferences()
	...
	private void retrieveAttachedViewer() {
		...
		if (editorPart instanceof MultiPageTalendEditor) {
			viewer = ((MultiPageTalendEditor) editorPart).getTalendEditor()
					.getViewer();
		}
	}
	...
	private void saveElementParameters(TalendFileFactory fileFact,
			List<? extends IElementParameter> paramList, EList listParamType,
			ProcessType process) {
		IElementParameter param;
		for (int j = 0; j < paramList.size(); j++) {
			param = paramList.get(j);
			saveElementParameter(param, process, fileFact, paramList,
					listParamType);
			for (String key : param.getChildParameters().keySet()) {
				saveElementParameter(param.getChildParameters().get(key),
						process, fileFact, paramList, listParamType);
			}
			// accept only one level of child parameters.
		}
	}

	private void saveElementParameter(IElementParameter param,
			ProcessType process, TalendFileFactory fileFact,
			List<? extends IElementParameter> paramList, EList listParamType) {
		...
		if ((!param.isReadOnly())
				|| param.getName().equals(EParameterName.UNIQUE_NAME.getName())
				|| param.getName().equals(EParameterName.VERSION.getName())) {
			pType = fileFact.createElementParameterType();
			if (param.getParentParameter() != null) {
				pType.setName(param.getParentParameter().getName() + ":"
						+ param.getName());
			} else {
			...
			if (param.getField().equals(EParameterFieldType.TABLE)) {
				...
				for (Map<String, Object> currentLine : tableValues) {
					for (int i = 0; i < currentLine.size(); i++) {
						ElementValueType elementValue = fileFact
								.createElementValueType();
						elementValue.setElementRef(param
								.getListItemsDisplayCodeName()[i]);
						Object o = currentLine.get(param
								.getListItemsDisplayCodeName()[i]);
						String strValue = ""; //$NON-NLS-1$
						if (o instanceof Integer) {
							IElementParameter tmpParam = (IElementParameter) param
									.getListItemsValue()[i];
							if (tmpParam.getListItemsValue().length == 0) {
								strValue = ""; //$NON-NLS-1$
							} else {
								strValue = (String) tmpParam
										.getListItemsValue()[(Integer) o];
							}
						} else {
	...
	private void loadElementParameters(Element elemParam, EList listParamType) {
		...
		for (int j = 0; j < listParamType.size(); j++) {
			...
			if (pType != null) {
				IElementParameter param = elemParam.getElementParameter(pType
						.getName());
				if (param != null) {
					if (param.isReadOnly()
							&& !(param.getName().equals(
									EParameterName.UNIQUE_NAME.getName()) || param
									.getName().equals(
					...
					if (param.getField().equals(EParameterFieldType.CHECK)) {
						elemParam.setPropertyValue(pType.getName(),
								new Boolean(pType.getValue()));
					} else if (param.getField().equals(
							EParameterFieldType.TABLE)) {
						...
						for (ElementValueType elementValue : (List<ElementValueType>) pType
								.getElementValue()) {
							boolean found = false;
							for (int i = 0; i < codeList.length && !found; i++) {
								if (codeList[i].equals(elementValue
										.getElementRef())) {
									found = true;
								}
							}
						...
						elemParam
								.setPropertyValue(pType.getName(), tableValues);
					} else if (!param.getField().equals(
							EParameterFieldType.SCHEMA_TYPE)) {
						elemParam.setPropertyValue(pType.getName(), pType
								.getValue());
					}
				}
			}
	...
	public ProcessType saveXmlFile(final IFile file) throws IOException {
		...
		saveElementParameters(fileFact, this.getElementParameters(), process
				.getParameters().getElementParameter(), process);
		...
		for (Element element : elem) {
			if (element instanceof Node) {
				saveNode(fileFact, process, nList, cList, (Node) element,
						factory);
	...
	private void saveNote(TalendFileFactory fileFact, ProcessType process,
			Note note) {
	...
	private void saveNode(TalendFileFactory fileFact, ProcessType process,
			EList nList, EList cList, Node node, MetadataEmfFactory factory) {
		NodeType nType;
		...
		if ((node.getSize().width != Node.DEFAULT_SIZE)
				|| (node.getSize().height != Node.DEFAULT_SIZE)) {
	...
	public void loadXmlFile(ProcessType process) {
		...
		if (process.getParameters() != null) {
			loadElementParameters(this, process.getParameters()
					.getElementParameter());
		}
	...
	private void loadNotes(ProcessType process) {
		for (Iterator iter = process.getNote().iterator(); iter.hasNext();) {
			...
			note.setSize(new Dimension(noteType.getSizeWidth(), noteType
					.getSizeHeight()));
	...
	private void loadNodes(ProcessType process,
			Hashtable<String, Node> nodesHashtable) throws PersistenceException {
		EList nodeList;
		NodeType nType;
		...
		for (int i = 0; i < nodeList.size(); i++) {
			...
			Point offset = new Point(nType.getOffsetLabelX(), nType
					.getOffsetLabelY());
		...
		if (!unloadedNodeNames.isEmpty()) {
			throw new PersistenceException(Messages
					.getString("Process.componentsUnloaded")); //$NON-NLS-1$
		}
	}
	...
	/**
	 * Checks if there are unloaded nodes.If there are some nodes unloaded,
	...
	public void checkLoadNodes() throws PersistenceException {
		...
		if (unloadedNodeNames.size() == 1) {
			errorMessage = Messages.getString(
					"Process.component.notloaded", unloadedNodeNames.get(0)); //$NON-NLS-1$
		} else {
			...
			errorMessage = Messages.getString(
					"Process.components.notloaded", curentName.toString()); //$NON-NLS-1$
	...
	private void loadSchema(Node nc, NodeType nType) {
		...
		for (int j = 0; j < listMetaType.size(); j++) {
			...
			if (nc.isELTComponent()) {
				metadataTable.setAttachedConnector(EConnectionType.TABLE
						.getName());
			}
			MetadataTool.initilializeSchemaFromElementParameters(metadataTable,
					(List<IElementParameter>) nc.getElementParameters());
		}
		List<IMetadataTable> oldComponentMetadataList = new ArrayList<IMetadataTable>(
				nc.getMetadataList());
		...
		for (IMetadataTable table : oldComponentMetadataList) {
			if (nc.getMetadataFromConnector(table.getAttachedConnector()) == null) {
				...
				// list.
				String baseSchema = nc.getConnectorFromName(
						table.getAttachedConnector()).getBaseSchema();
				if (!table.getAttachedConnector().equals(baseSchema)) {
					MetadataTool.copyTable(nc
							.getMetadataFromConnector(baseSchema), table);
				}
	...
	private boolean checkNodeSchemaFromRepository(final Node node,
			final List<MetadataUpdateCheckResult> resultList) {
					...
					if (schemaParam.getValue().equals(EmfComponent.REPOSITORY)) {
						String metaRepositoryName = (String) currentParam
								.getChildParameters().get(
										EParameterName.REPOSITORY_SCHEMA_TYPE
												.getName()).getValue();
						IMetadataTable repositoryMetadata = MetadataTool
								.getMetadataFromRepository(metaRepositoryName);

						MetadataUpdateCheckResult result = new MetadataUpdateCheckResult(
								node);
						...
						if (repositoryMetadata != null) {
							...
							if (!copyOfrepositoryMetadata
								...
								result
										.setResult(
												MetadataUpdateCheckResult.RepositoryType.schema,
												MetadataUpdateCheckResult.ResultType.change,
							...
							result
									.setResult(
											MetadataUpdateCheckResult.RepositoryType.schema,
											MetadataUpdateCheckResult.ResultType.delete,
											null);
	...
	private boolean checkNodePropertiesFromRepository(final Node node,
		...
		String propertyType = (String) node
				.getPropertyValue(EParameterName.PROPERTY_TYPE.getName());
		if (propertyType != null) {
			if (propertyType.equals(EmfComponent.REPOSITORY)) {
				...
				try {
					metadataConnectionsItem = factory
				...
				if (repositoryConnection != null) {
					...
					for (IElementParameter param : node.getElementParameters()) {
						...
						if (param.isShow(node.getElementParameters())
							...
							Object objectValue = (Object) RepositoryToComponentProperty
									.getValue(repositoryConnection,
											repositoryValue);
					...
					if (!sameValues) {
						...
						result
								.setResult(
										MetadataUpdateCheckResult.RepositoryType.property,
										MetadataUpdateCheckResult.ResultType.change,
										repositoryConnection);

						modified = true;
					} else {
						for (IElementParameter param : node
								.getElementParameters()) {
							String repositoryValue = param.getRepositoryValue();
							if (param.isShow(node.getElementParameters())
									&& (repositoryValue != null)
									&& (!param.getName().equals(
											EParameterName.PROPERTY_TYPE
													.getName()))) {
	...
	public boolean checkDifferenceWithRepository() {
		...
		if (resultList.size() > 0) {
			MetadataUpdateCheckDialog checkDlg = new MetadataUpdateCheckDialog(
					PlatformUI.getWorkbench().getDisplay().getActiveShell(),
					resultList, Messages
							.getString("Process.IfToUpgradeMetadata")); //$NON-NLS-1$
			checkDlg.setTitle(Messages
					.getString("Process.metadataModificationDetected")); //$NON-NLS-1$
	...
	private void updateNodeswithMetadata(final List<Object> list) {
		...
		for (int k = 0; k < list.size(); k++) {
			...
			MetadataUpdateCheckResult result = (MetadataUpdateCheckResult) list
					.get(k);
			...
			if (result.getRepositoryType() == MetadataUpdateCheckResult.RepositoryType.property) {
				...
				if (result.getResultType() == MetadataUpdateCheckResult.ResultType.change) {
					...
					if (result.isChecked()) {
							...
							if (param.isShow(node.getElementParameters())
								...
								if (objectValue != null) {
									if (param.getField().equals(
										...
										node.setPropertyValue(param.getName(),
												objectValue);
									}
									param.setRepositoryValueUsed(true);
								}
						...
						node.setPropertyValue(EParameterName.PROPERTY_TYPE
				...
				if (result.getResultType() == MetadataUpdateCheckResult.ResultType.change) {
					...
					if (result.isChecked()) {
						node
								.getMetadataTable(node.getUniqueName())
								.setListColumns(
										((IMetadataTable) result.getParameter())
												.getListColumns());
					} else { // result.isChecked()==false
						node.setPropertyValue(EParameterName.SCHEMA_TYPE
								.getName(), EmfComponent.BUILTIN);
					}
				} else { // MetadataUpdateCheckResult.ResultType.delete
	...
	private void loadConnections(ProcessType process,
			Hashtable<String, Node> nodesHashtable) {
		EList listParamType;
		EList connecList;
		ConnectionType cType;
		...
		for (int i = 0; i < connecList.size(); i++) {
			...
			if (connectorName != null) {
				...
				if (source.getConnectorFromName(connectorName) != null
						&& (source.getConnectorFromName(connectorName)
								.getDefaultConnectionType().getId() == lineStyleId)) {
			...
			if (source.getMetadataTable(metaname) == null) {
				...
				if (source.getMetadataTable(metaname) == null) {
					...
					if (source.getMetadataList().size() > 0) {
						metaname = source.getMetadataList().get(0)
								.getTableName();
					}
			...
			if (connectionTypeFound) {
				connec = new Connection(source, target, EConnectionType
						.getTypeFromId(lineStyleId), connectorName, metaname,
						cType.getLabel(), cType.getMetaname());
			} else {
				EConnectionType type = EConnectionType
						.getTypeFromId(lineStyleId);
				connec = new Connection(source, target, type, source
						.getConnectorFromType(type).getName(), metaname, cType
			...
			Point offset = new Point(cType.getOffsetLabelX(), cType
					.getOffsetLabelY());
			INodeConnector nodeConnectorSource = connec
					.getSourceNodeConnector();
			nodeConnectorSource.setCurLinkNbOutput(nodeConnectorSource
					.getCurLinkNbOutput() + 1);
			INodeConnector nodeConnectorTarget = connec
					.getTargetNodeConnector();
			nodeConnectorTarget.setCurLinkNbInput(nodeConnectorTarget
					.getCurLinkNbInput() + 1);
				...
				for (int i = 0; i < connecList.size(); i++) {
					...
					if (cType.getTarget().equals(node.getUniqueName())) {
						if (cType.isSetMergeOrder()) {
							Connection connection = connectionsHashtable
									.get(cType);
		...
		if (connectionsProblems.size() > 0) {
			...
			mb.setText(Messages
					.getString("Process.errorLoadingConnectionTitle")); //$NON-NLS-1$
			String message = Messages
					.getString("Process.errorLoadingConnectionMessage"); //$NON-NLS-1$
	...
	private void loadContexts(ProcessType process) {
		...
		if (process.getContext() == null) {
			...
			contextManager = new JobContextManager(process.getContext(),
					defaultContextToLoad);
		}
	}
	...
	public boolean checkReadOnly() {
		IProxyRepositoryFactory repFactory = DesignerPlugin.getDefault()
				.getProxyRepositoryFactory();
		boolean readOnlyLocal = !repFactory
				.isEditableAndLockIfPossible(property.getItem());
	...
	/**
	 * Check if the given name will be unique in the process. If another link
	 * already exists with that name, false will be returned.
	 * 
	 * @param uniqueName
	 * @param checkEsists
		...
		if (uniqueConnectionName != null) {
			if (checkValidConnectionName(uniqueConnectionName)) {
				...
				throw new IllegalArgumentException(
						"The name of the connection is not valid: " + uniqueConnectionName); //$NON-NLS-1$
			}
		}
	}
	...
	public String generateUniqueNodeName(INode node) {
		...
		return UniqueNodeNameGenerator.generateUniqueNodeName(baseName,
				uniqueNodeNameList);
	}

	/**
	 * This function will take a unique name and update the list with the given
	 * name. This function should be private only and should be called only when
			...
			for (Connection connec : (List<Connection>) node
				...
				if (connec.getSource().isActivate() != active) {
					if (connec.getLineStyle().hasConnectionCategory(
						...
						if (connec.getSource().getSubProcessStartNode(false)
								.isActivate() != active) {
							setActivateSubjob(connec.getSource()
									.getSubProcessStartNode(false), active,
									activateNode);
						}
					}
				}
			}
			node.setPropertyValue(EParameterName.ACTIVATE.getName(),
					new Boolean(active));
			for (Connection connec : (List<Connection>) node
					.getOutgoingConnections()) {
			...
			if (node.getSubProcessStartNode(false).equals(mainSubProcess)) {
				...
				for (Connection connec : (List<Connection>) node
					...
					if (connec.getLineStyle().hasConnectionCategory(
							IConnectionCategory.DATA)) {
						if (connec.getSource().isActivate() != active) {
							setActivateSubjob(connec.getSource(), active,
									activateNode);
						}
					}
				}
				for (Connection connec : (List<Connection>) node
					...
					if (connec.getLineStyle().hasConnectionCategory(
							IConnectionCategory.DATA)) {
						if (connec.getTarget().isActivate() != active) {
							setActivateSubjob(connec.getTarget(), active,
									activateNode);
						}
					}
				}
	...
	public void checkStartNodes() {
		for (Node node : nodes) {
			if ((Boolean) node.getPropertyValue(EParameterName.STARTABLE
					.getName())) {
				if (node.isActivate()) {
					if (node.isELTComponent()) {
						// is there condition link, then don't set the start.
						boolean isThereConditionLink = false;
						for (int j = 0; j < node.getIncomingConnections()
								.size()
								&& !isThereConditionLink; j++) {
							Connection connection = (Connection) node
									.getIncomingConnections().get(j);
							if (connection.isActivate()
									&& connection
											.getLineStyle()
											.hasConnectionCategory(
													IConnectionCategory.DEPENDENCY)) {
	...
	/**
	 * If the node link with the merge node, it will return the merge link
	 * order, or it will return -1 Purpose: only in the branch of the first
	 * merge link can be as a start node.
	 * 
	 * @param node
	 * @return
	 */
		...
		for (int i = 0; (i < outgoingConnections.size()) && (returnValue == -1); i++) {
			...
			if (connec.isActivate()) {
				if (connec.getLineStyle().hasConnectionCategory(
					...
					break;
				} else if (connec.getLineStyle().hasConnectionCategory(
						EConnectionType.MAIN)
	...
	/**
	 * just like the getMergelinkOrder(), there will return more info the key is
	 * the Merge node, and value is inputId. if don't link with merge, it will
	 * return null. Notice: make sure there only link with one merge node. It
	 * can't support to link with more merge node.
	 * 
	 * @param node
	 * @return
	 */
	...
	/**
	 * This function check if in this subprocess there should be a start or not
	 * depends on the ref links. If in this subprocess there is only one main
	 * flow and one ref then this function will return true. If there is several
	 * flow in the output of one component in this subprocess,it'll return
	 * false.
	 * 
	 * @param node
	 * @return
	...
	/**
	 * check the problems of node.compare with the checkProblems(),this method
	 * can't refresh problems view.
	 */
	...
	public void setPropertyValue(String id, Object value) {
		if (id.equals(EParameterName.SCHEMA_TYPE.getName())
			...
			setPropertyValue(EParameterName.UPDATE_COMPONENTS.getName(),
					Boolean.TRUE);
		}
	 ...
	 * 
	 * @param filter
	 *            only return the filter matched connections
	 * @return
	 */
	...
	public Project getProject() {
		return ((org.talend.core.context.RepositoryContext) CorePlugin
	...
	public ProcessItem getMasterJob() {
		...
		try {
			IRepositoryObject repositoryObject = factory.getProcess()
					.getMember(getProject().getMasterJobId());
	...
	public Set<String> getNeededLibraries(boolean withChildrens) {
		...
		for (INode node : nodeList) {
			List<ModuleNeeded> moduleList = node.getComponent()
					.getModulesNeeded();
			...
			if (processItem.getProcess().getRequired() != null) {
				...
				for (int j = 0; j < jobList.size(); j++) {
					...
					if (!childrensList.contains(jType.getName())) {
						...
						ProcessItem childItem = ProcessorUtilities
								.getProcessItem(jType.getName());
						if (childItem == null) {
							continue;
						}
	...
	public Set<String> getSubJobs(String processName) {
		...
		for (Iterator iter = currentProcess.getGraphicalNodes().iterator(); iter
				.hasNext();) {
			Node node = (Node) iter.next();
			if ((node != null)
					&& node.getComponent().getName().equals("tRunJob")) {
				String curProcessName = (String) node
