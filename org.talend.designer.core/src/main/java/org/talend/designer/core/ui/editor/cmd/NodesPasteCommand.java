...
public class NodesPasteCommand extends Command {
	...
	private String checkExistingNames(String oldName) {
		...
		for (Node node : (List<Node>) process.getGraphicalNodes()) {
			for (Connection connection : (List<Connection>) node
	...
	private Point findLocationForNodeInProcess(final Point location,
		...
		for (Node node : (List<Node>) process.getGraphicalNodes()) {
			Rectangle currentRect = new Rectangle(node.getLocation(), node
					.getSize());
	...
	private void createNodeContainerList() {
		...
		for (NodePart copiedNodePart : nodeParts) {
			...
			Node pastedNode = new Node(ComponentsFactoryProvider.getInstance()
					.get(copiedNode.getComponent().getName()), process);
			...
			pastedNode.setLocation(findLocationForNode(location, copiedNode
					.getSize()));
			...
			if (pastedNode.getExternalNode() == null) {
				...
				for (IMetadataTable metaTable : copiedNode.getMetadataList()) {
					...
					newTable.setTableName(createNewConnectionName(metaTable
							.getTableName()));
					oldMetaToNewMeta
							.put(pastedNode.getUniqueName() + ":"
									+ metaTable.getTableName(), newTable
									.getTableName());

					for (IMetadataColumn column : metaTable.getListColumns()) {
						if (column.isCustom()) {
							IMetadataColumn newColumn = newTable
									.getColumn(column.getLabel());
							newColumn.setReadOnly(column.isReadOnly());
							newColumn.setCustom(column.isCustom());
						}
				...
				if (copiedNode.getExternalData() != null) {
					try {
						externalNode.setExternalData(copiedNode
								.getExternalData().clone());
				...
				for (IMetadataTable metaTable : copiedNode.getMetadataList()) {
					...
					String newName = oldMetaToNewMeta.get(pastedNode
							.getUniqueName()
							+ ":" + metaTable.getTableName());
					externalNode.renameOutputConnection(oldName, newName);
				}
			}
			pastedNode.getNodeLabel().setOffset(
					new Point(copiedNode.getNodeLabel().getOffset()));
			oldNameTonewNameMap.put(copiedNode.getUniqueName(), pastedNode
					.getUniqueName());

			for (ElementParameter param : (List<ElementParameter>) copiedNode
					.getElementParameters()) {
				if (!EParameterName.UNIQUE_NAME.getName().equals(
						param.getName())) {
					if (param.getField() == EParameterFieldType.TABLE) {
						...
						pastedNode.getElementParameter(param.getName())
								.setValue(newValues);
					} else {
						// pastedNode.getElementParameter(param.getName()).setValue(param.getValue());
						pastedNode.setPropertyValue(param.getName(), param
								.getValue());
					}
				}
			}
		...
		for (NodePart copiedNodePart : nodeParts) {
			Node copiedNode = (Node) copiedNodePart.getModel();
			for (Connection connection : (List<Connection>) copiedNode
					.getOutgoingConnections()) {
				Node pastedTargetNode = null, pastedSourceNode = null;

				String nodeSource = oldNameTonewNameMap.get(copiedNode
						.getUniqueName());
				for (NodeContainer nodeContainer : nodeContainerList) {
					Node node = nodeContainer.getNode();
				...
				// string will be null
				String nodeToConnect = oldNameTonewNameMap.get(targetNode
						.getUniqueName());
				...
				if ((pastedSourceNode != null) && (pastedTargetNode != null)) {
					...
					if (connection.getLineStyle().hasConnectionCategory(
							IConnectionCategory.UNIQUE_NAME)) {
						newConnectionName = createNewConnectionName(connection
								.getName());
					} else {
						newConnectionName = connection.getName();
					}

					String meta = oldMetaToNewMeta.get(pastedSourceNode
							.getUniqueName()
							+ ":" + connection.getMetaName());
					if (meta != null) {
						if (pastedSourceNode.getConnectorFromType(
								connection.getLineStyle()).isBuiltIn()
						...
						if (pastedSourceNode.getConnectorFromType(
								connection.getLineStyle()).isBuiltIn()) {
							metaTableName = pastedSourceNode.getMetadataList()
									.get(0).getTableName();
						} else {
							metaTableName = pastedSourceNode.getUniqueName(); // connection.getMetaName();
						}
					}
					Connection pastedConnection = new Connection(
							pastedSourceNode, pastedTargetNode, connection
									.getLineStyle(), connection
									.getConnectorName(), metaTableName,
							newConnectionName);
					// pastedConnection.setActivate(pastedSourceNode.isActivate());
					for (ElementParameter param : (List<ElementParameter>) connection
							.getElementParameters()) {
						// pastedConnection.getElementParameter(param.getName())
						// .setValue(param.getValue());
						pastedConnection.setPropertyValue(param.getName(),
								param.getValue());
					}
					pastedConnection.getConnectionLabel().setOffset(
							new Point(connection.getConnectionLabel()
									.getOffset()));
					INodeConnector connector = pastedConnection
							.getSourceNodeConnector();
					connector
							.setCurLinkNbOutput(connector.getCurLinkNbOutput() + 1);
					connector = pastedConnection.getTargetNodeConnector();
					connector
							.setCurLinkNbInput(connector.getCurLinkNbInput() + 1);
					IExternalNode externalNode = pastedTargetNode
							.getExternalNode();
					if (externalNode != null) {
						externalNode.renameInputConnection(
								connection.getName(), newConnectionName);
					}
				}
			}
		}
		...
		if (useOldComponentsName) {
			...
			if (msgBox.open() == SWT.YES) {
				for (NodeContainer nodeContainer : nodeContainerList) {
					Node currentNode = nodeContainer.getNode();
					for (String oldName : oldNameTonewNameMap.keySet()) {
						currentNode.renameData(oldName, oldNameTonewNameMap
								.get(oldName));
					}
				}
			}
	...
	public void execute() {
		...
		MultiPageTalendEditor multiPageTalendEditor = (MultiPageTalendEditor) PlatformUI
				.getWorkbench().getActiveWorkbenchWindow().getActivePage()
				.getActiveEditor();
		GraphicalViewer viewer = multiPageTalendEditor.getTalendEditor()
				.getViewer();

		// save old selection
		if (!multipleCommand) {
			oldSelection = new ArrayList<EditPart>();
			for (EditPart editPart : (List<EditPart>) viewer
		...
		if (!multipleCommand) {
			EditPart processPart = (EditPart) viewer.getRootEditPart()
					.getChildren().get(0);
			if (processPart instanceof ProcessPart) { // can only be
				// ProcessPart but still
				// test
				for (EditPart editPart : (List<EditPart>) processPart
						.getChildren()) {
					if (editPart instanceof NodePart) {
						Node currentNode = (Node) editPart.getModel();
						if (nodeContainerList.contains(currentNode
								.getNodeContainer())) {
	...
	public void undo() {
		...
		for (NodeContainer nodeContainer : nodeContainerList) {
			...
			for (Connection connection : (List<Connection>) nodeContainer
					.getNode().getOutgoingConnections()) {
		...
		if (!multipleCommand) {
			for (EditPart editPart : oldSelection) {
				/*
				 * if (editPart instanceof NodePart) { Node node = (Node)
				 * editPart.getModel(); }
				 */
				viewer.appendSelection(editPart);
			}
	 ...
	 * 
	 * @param multipleCommand
	 *            the multipleCommand to set
	 */
