...
/**
 * Command that changes a given property. It will call the set or get property
 * value in an element. This element can be either a node, a connection or a
 * process. <br/>
 * 
 * $Id: PropertyChangeCommand.java 295 2006-11-02 08:28:03 +0000 (jeu., 02 nov.
 * 2006) smallet $
 * 
 */
public class PropertyChangeCommand extends Command {
	...
	/**
	 * The property is defined in an element, which can be either a node or a
	 * connection.
	 * 
	 * @param elem
	 * @param propName
					...
					if (param.getListItemsShowIf(language) != null) {
						for (int j = 0; j < param.getListItemsShowIf(language).length
							...
							if (showIf != null) {
								...
								if (notShowIf != null) {
									if (notShowIf.contains(currentParam
											.getName())) {
										toUpdate = true;
									}
								}
