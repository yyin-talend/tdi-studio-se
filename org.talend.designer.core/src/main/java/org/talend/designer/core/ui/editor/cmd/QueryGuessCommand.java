...
/**
 * This class is used for generating new query when "Guess Query" button is
 * selected. <br/>
 * 
 */
public class QueryGuessCommand extends Command {
	...
	public void execute() {
		...
		if (this.realTableId != null
				&& this.dbNameAndDbTypeMap.containsKey(this.realTableId)) {
			dbType = this.dbNameAndDbTypeMap.get(this.realTableId);
		}

		String schema = this.dbNameAndSchemaMap.get(this.realTableId);

		String newQuery = TalendTextUtils.addSQLQuotes(QueryUtil
				.generateNewQuery(node, newOutputMetadataTable, dbType, schema,
						realTableName));

		for (IElementParameter param : (List<IElementParameter>) node
	 ...
	 */
	public void setMaps(Map<String, String> dbNameAndDbTypeMap,
