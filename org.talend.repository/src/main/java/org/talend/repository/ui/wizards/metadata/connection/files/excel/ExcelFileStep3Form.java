...
public class ExcelFileStep3Form extends AbstractExcelFileStepForm {
	 ...
	 * @param Composite
	 */
	public ExcelFileStep3Form(Composite parent, ConnectionItem connectionItem,
	...
	protected void addFields() {
		...
		metadataNameText = new LabelledText(mainComposite, Messages
				.getString("FileStep3.metadataName")); //$NON-NLS-1$
		metadataCommentText = new LabelledText(mainComposite, Messages
				.getString("FileStep3.metadataComment")); //$NON-NLS-1$

		// Group MetaData
		Group groupMetaData = Form.createGroup(this, 1, Messages
				.getString("FileStep3.groupMetadata"), 280); //$NON-NLS-1$
		...
		guessButton = new UtilsButton(compositeGuessButton, Messages
				.getString("FileStep3.guess"), WIDTH_BUTTON_PIXEL, //$NON-NLS-1$
				HEIGHT_BUTTON_PIXEL);
		...
		metadataEditor = new MetadataEmfTableEditor(Messages
				.getString("FileStep3.metadataDescription")); //$NON-NLS-1$
		...
		if (!isInWizard()) {
			...
			// Button Cancel
			cancelButton = new UtilsButton(compositeBottomButton, Messages
					.getString("CommonWizard.cancel"), WIDTH_BUTTON_PIXEL, //$NON-NLS-1$
					HEIGHT_BUTTON_PIXEL);
		}
					...
					if (!guessButton.getEnabled()) {
						...
						if (MessageDialog
								.openConfirm(
										getShell(),
										Messages
												.getString("FileStep3.guessConfirmation"), Messages //$NON-NLS-1$
	 ...
	 * 
	 * WARNING ::field FieldSeparator, RowSeparator, EscapeChar and
	 * TextEnclosure are surround by double quote.
	 * 
	 * 
	 * @return processDescription
	 */
	private ProcessDescription getProcessDescription(
			FileExcelConnection originalValueConnection) {
		...
		processDescription.setEncoding(TalendTextUtils
				.addQuotes(originalValueConnection.getEncoding()));
	...
	protected void runShadowProcess() {
		...
		try {
			...
			CsvArray csvArray = ShadowProcessHelper.getCsvArray(
					processDescription, "FILE_EXCEL"); //$NON-NLS-1$

			if (csvArray == null) {
				informationLabel
						.setText("   " + Messages.getString("FileStep3.guessFailure")); //$NON-NLS-1$ //$NON-NLS-2$
			} else {
			...
			if (getParent().getChildren().length == 1) {
				new ErrorDialogWidthDetailArea(
						...
						Messages.getString("FileStep3.guessFailureTip") + "\n" //$NON-NLS-1$ //$NON-NLS-2$
								+ Messages
										.getString("FileStep3.guessFailureTip2"), e.getMessage()); //$NON-NLS-1$
			} else {
				new ErrorDialogWidthDetailArea(getShell(), PID, Messages
						.getString("FileStep3.guessFailureTip"), e.getMessage()); //$NON-NLS-1$
			}
			log
					.error(Messages.getString("FileStep3.guessFailure") + " " + e.getMessage()); //$NON-NLS-1$ //$NON-NLS-2$
		}
	...
	public void refreshMetaDataTable(final CsvArray csvArray,
		...
		if (csvArray == null) {
			...
			String[] excelStyleTitles = ExcelReader
					.getColumnsTitle(label.length);// Excel style column title
				...
				if (firstRowToExtractMetadata == 1) {
					...
					if (numberOfCol <= fields.length) {// if current field size
						...
						if (fields[i] != null && !("").equals(fields[i])) { //$NON-NLS-1$
							...
							label[i] = ColumnNameValidator
									.validateColumnNameFormat(label[i], i);
						} else {
			...
			for (int i = 0; i < numberOfCol.intValue(); i++) {
				...
				while (globalType == null) {
					if (LanguageManager.getCurrentLanguage() == ECodeLanguage.JAVA) {
						if (i >= csvRows.get(current).length) {
							...
							globalType = JavaDataTypeHelper
									.getTalendTypeOfValue(csvRows.get(current)[i]);
							current++;
				...
				for (int f = firstRowToExtractMetadata; f < csvRows.size(); f++) {
					...
					if (fields.length > i) {
						...
						if (!value.equals("")) { //$NON-NLS-1$
							if (LanguageManager.getCurrentLanguage() == ECodeLanguage.JAVA) {
								if (!JavaDataTypeHelper.getTalendTypeOfValue(
										value).equals(globalType)) {
									globalType = JavaDataTypeHelper
											.getCommonType(
													globalType,
													JavaDataTypeHelper
								...
								if (!PerlDataTypeHelper.getTalendTypeOfValue(
										value).equals(globalType)) {
									globalType = PerlDataTypeHelper
											.getCommonType(
													globalType,
				...
				if (LanguageManager.getCurrentLanguage() == ECodeLanguage.JAVA) {
					...
					if (globalType.equals(JavaTypesManager.FLOAT.getId())
							|| globalType.equals(JavaTypesManager.DOUBLE
									.getId())) {
					...
					talendType = PerlTypesManager
							.getNewTypeName(MetadataTalendType.loadTalendType(
	...
	public Integer getRightFirstRow(List<String[]> csvRows) {
		...
		for (int i = 0; i < parserLine; i++) {
			if (csvRows.get(i) != null) {
				...
				if (numbersOfColumns == null
						|| nbRow.length >= numbersOfColumns) {
	...
	protected boolean checkFieldsValue() {
		if (metadataNameText.getCharCount() == 0) {
			...
			updateStatus(IStatus.ERROR, Messages
					.getString("FileStep1.nameAlert")); //$NON-NLS-1$
			return false;
		} else if (!Pattern.matches(
				RepositoryConstants.REPOSITORY_ITEM_PATTERN, metadataNameText
						.getText())) {
			metadataNameText.forceFocus();
			updateStatus(IStatus.ERROR, Messages
					.getString("FileStep1.nameAlertIllegalChar")); //$NON-NLS-1$
			return false;
		} else if (isNameAllowed(metadataNameText.getText())) {
			updateStatus(IStatus.ERROR, Messages
					.getString("CommonWizard.nameAlreadyExist")); //$NON-NLS-1$
			return false;
		}
	...
	private FileExcelConnection getOriginalValueConnection() {
		if (getConnection().isContextMode() && getContextModeManager() != null) {
			return (FileExcelConnection) FileConnectionContextUtils
