...
public class HTMLDocGenerator {
	...
	private static final String TITLE = "Generated by Talend.";
	...
	/**
	 * This method is used for generating HTML file base on an instance of
	 * <code>ExportFileResource</code>
	 * 
	 * @param resource
	 */
	public static void generateHTMLFile(ExportFileResource resource) {
		try {
			...
			final URL xslFileUrl = FileLocator.toFileURL(FileLocator.find(b,
					new Path(XSL_FILE_PATH), null));
			final URL logoFileUrl = FileLocator.toFileURL(FileLocator.find(b,
					new Path(LOGO_FILE_PATH), null));
			...
			FileCopyUtils.copy(logoFilePath, picFolderPath + File.separatorChar
					+ TALEND_LOGO_FILE_NAME);

			Set keySet = picFilePathMap.keySet();
			for (Object key : keySet) {
				String value = (String) picFilePathMap.get(key);
				FileCopyUtils.copy(value, picFolderPath + File.separatorChar
						+ key);
			}

			List<URL> resultFiles = parseXML2HTML(tempFolderPath, jobName,
					xslFilePath);
	 ...
	 * @param resource
	 */
	private static String checkPicDirIsExists(ExportFileResource resource,
			String tempFolderPath) {
		String picFolderPath = tempFolderPath + File.separator
				+ PIC_FOLDER_NAME;
	...
	private static String checkTempDirIsExists(ExportFileResource resource) {
		String tempDirPath = getTmpFolder() + File.separator
				+ resource.getDirectoryName();
	 ...
	 * @throws Exception
	 */
	private static List<URL> parseXML2HTML(String tempFolderPath,
			String jobName, String xslFilePath) throws Exception {
	...
	/**
	 * Generates the xml file base on an instance of
	 * <code>ExportFileResource</code> and the temporary folder path.
	 * 
	 * @param resource
	 * @param tempFolderPath
	 */
	private static void generateXMLFile(ExportFileResource resource,
		...
		try {
			...
			String filePath = tempFolderPath + File.separatorChar
					+ processItem.getProperty().getLabel() + ".xml";
	 ...
	 */
	private static void generateConnectionsInfo(Element jobElement,
			EList connectionList) {
		...
		for (int j = 0; j < connectionList.size(); j++) {
			...
			connectionElement.addAttribute("label",
					checkString(type.getLabel()));
			connectionElement.addAttribute("lineStyle", checkString(type
					.getLineStyle()
					+ ""));
			connectionElement.addAttribute("metaname", checkString(type
					.getMetaname()));
			connectionElement.addAttribute("offsetLabelX", checkString(type
					.getOffsetLabelX()
					+ ""));
			connectionElement.addAttribute("offsetLabelY", checkString(type
					.getOffsetLabelY()
					+ ""));
			connectionElement.addAttribute("source", checkString(type
					.getSource()));
			connectionElement.addAttribute("target", checkString(type
					.getTarget()));
		}
	}
	 ...
	 */
	private static void generateComponentsInfo(Element jobElement,
			ProcessItem processItem) {
		IProcess process = CorePlugin.getDefault().getDesignerCoreService()
				.getProcessFromProcessItem(processItem);
		List<INode> graphicalNodeList = (List<INode>) process
				.getGraphicalNodes();
		...
		if (componentList.size() != 0) {
			Element componentNameListElement = jobElement
					.addElement("componentList");
			Element componentItemElement = null;
			for (INode componentNode : componentList) {
				componentItemElement = componentNameListElement
						.addElement("componentItem");
				componentItemElement.addAttribute("name", componentNode
						.getUniqueName());
				componentItemElement.addAttribute("link", componentNode
						.getUniqueName());
				componentItemElement.addAttribute("type", componentNode
		...
		for (INode node : graphicalNodeList) {
			...
			String componentIconFileName = relativePath.substring(relativePath
					.lastIndexOf("/") + 1, relativePath.length() - 1);

			String uniqueName = node.getUniqueName();
			componentElement.addAttribute("icon", PICTUREFOLDERPATH
					+ componentIconFileName);
			componentElement.addAttribute("uniqueName", uniqueName);

			// Stores the path of component icon.
			picFilePathMap.put(componentIconFileName, relativePath.substring(
					relativePath.indexOf("/") + 1, relativePath.length() - 1));
			...
			if (istMap) {
				previewImagePath = getPreviewImagePath(node
						.getElementParameters());
				if (!previewImagePath.equals("")) {
					IPath filePath = RepositoryPathProvider
							.getPathFileName(RepositoryConstants.IMG_DIRECTORY,
									previewImagePath);
					storedPreviewImagePath = filePath.toOSString();
					picFilePathMap
							.put(previewImagePath, storedPreviewImagePath);
					componentElement.addAttribute("preview", PICTUREFOLDERPATH
							+ previewImagePath);
				}
			}
			...
			if (targetList != null && targetList.size() > 0) {
				for (String string : targetList) {
					Element outputElement = componentElement
							.addElement("output");
			...
			Element componentTypeElement = componentElement
					.addElement("componentType");
			componentTypeElement.setText(checkString(node.getComponent()
					.getName()));

			Element parametersElement = componentElement
					.addElement("parameters");

			List elementParameterList = node.getElementParameters();

			generateComponentElementParamInfo(istMap, istRunJob,
					parametersElement, elementParameterList);

			List metaDataList = node.getMetadataList();
			if (metaDataList != null && metaDataList.size() != 0) {
				Element schemaElement = null;
				for (int j = 0; j < metaDataList.size(); j++) {
					IMetadataTable type = (IMetadataTable) metaDataList.get(j);

					List columnTypeList = type.getListColumns();
					for (int k = 0; k < columnTypeList.size(); k++) {
						IMetadataColumn columnType = (IMetadataColumn) columnTypeList
								.get(k);
						if (schemaElement == null) {
							schemaElement = componentElement
									.addElement("schema");
						}
						Element columnElement = schemaElement
								.addElement("column");
						columnElement.addAttribute("name",
								checkString(columnType.getLabel()));

						columnElement.addAttribute("key",
								checkString(columnType.isKey() + ""));
						columnElement.addAttribute("type",
								checkString(columnType.getType()));
						columnElement.addAttribute("length",
								checkString(columnType.getLength() + ""));
						columnElement.addAttribute("precision",
								checkString(columnType.getPrecision() + ""));
						columnElement.addAttribute("nullable",
								checkString(columnType.isNullable() + ""));
						columnElement.addAttribute("comment",
								checkString(ElementParameterParser.parse(node,
	 ...
	 */
	private static void generateComponentElementParamInfo(boolean istMap,
			boolean istRunJob, Element parametersElement,
			List elementParameterList) {
		if (elementParameterList != null && elementParameterList.size() != 0) {
			if (istMap) {
				generateElementInfoOnlyFortMap(parametersElement,
				...
				for (int j = 0; j < elementParameterList.size(); j++) {
					IElementParameter type = (IElementParameter) elementParameterList
							.get(j);
					if (type.isShow(elementParameterList)
							&& ((type.getCategory().equals(
									EComponentCategory.PROPERTY) || (type
									.getCategory()
							...
							&& (!type.getName().equals("PROPERTY") && (!(istRunJob && type
									.getName().equals("PROCESS"))))) {

						// Gets the display name of a selected combo item.
						if (type.getName().equals("TYPE")) {

							int index = type.getIndexOfItemFromList(type
									.getDisplayName());
							String displayName = type.getListItemsDisplayName()[index];
							parameterElement = parametersElement
									.addElement("parameter");
							Element columnElement = parameterElement
									.addElement("column");
							columnElement.addAttribute("name", checkString(type
									.getDisplayName()));
						...
						if (type.getNumRow() == numRow
							...
							if (type.getValue() != null) {
								columnElement.setText(checkString(type
										.getValue().toString()));
							} else {
	 ...
	 * @param elementParameterList
	 */
	private static void generateElementInfoOnlyFortMap(
			Element parametersElement, List elementParameterList) {
		if (elementParameterList != null && elementParameterList.size() != 0) {
			Element parameterElement = null;
			for (int j = 0; j < elementParameterList.size(); j++) {
				IElementParameter type = (IElementParameter) elementParameterList
				...
				if (type.isShow(elementParameterList)
						&& ((type.getCategory().equals(
								EComponentCategory.PROPERTY) || (type
								.getCategory().equals(EComponentCategory.DOC))))) {
	...
	/**
	 * Gets tMap's preview image path base on a list of
	 * <code>IElementParameter</code>
	 * 
	 * @param elementParameters
	 * @return
	 */
	private static String getPreviewImagePath(
	...
	/**
	 * Generates job(process) information in XML base on
	 * <code>ProcessItem</code> and project element.
	 * 
	 * @param processItem
	 ...
	 * @return an instance of <code>Element</code>
	 */
	private static Element generateJobInfo(ProcessItem processItem,
		...
		jobElement.addAttribute("author", checkString(process.getAuthor()
				.toString()));
		...
		jobElement.addAttribute("description", checkString(process
				.getDescription()));

		DateFormat formatter = DateFormat.getDateTimeInstance();
		jobElement.addAttribute("creation", checkString(formatter
				.format(processItem.getProperty().getCreationDate())));
		jobElement.addAttribute("modification", checkString(formatter
				.format(processItem.getProperty().getModificationDate())));

		String picName = jobName + JOB_PREVIEW_PIC_SUFFIX;
		IPath filePath = RepositoryPathProvider.getPathFileName(
				RepositoryConstants.IMG_DIRECTORY_OF_JOB_OUTLINE, picName);
	...
	private static Element generateProjectInfo(Document document) {
		...
		projectElement.addAttribute("logo", PICTUREFOLDERPATH
				+ TALEND_LOGO_FILE_NAME);
		projectElement.addAttribute("title", TITLE);
		projectElement.addAttribute("language", getProject().getLanguage()
				.getName());
		projectElement.addAttribute("description", getProject()
				.getDescription());
	 ...
	 *            <code>ExportFileResource</code>
	 * @param resultFiles
	 *            a <code>List</code> of <code>URL</code>
	 */
	private static void addResources(ExportFileResource resource,
			List<URL> resultFiles) {
	...
	/**
	 * Get source connections and target connections base on given
	 * <code>ProcessItem</code>.
	 * 
	 * @param processItem
	 *            ProcessItem
