...
public class HTMLDocGenerator {
    ...
    private static final String PICDIRECTORY = "pictures";

    private static final String TEMPFOLDERNAME = "userHTMLDoc";

    private static Map<String, String> picFilePathMap;

    private static final String EMPTYELEMENTVALUE = "none";

    private static final String TITLE = "Generated by Talend.";

    private static final String JOBPREVIEWPICSUFFIX = ".jpg";

    private static final String PICTUREFOLDERPATH = "pictures/";

    private static final String TALENDLOGOFILENAME = "logo.jpg";

    private static final String XSLFILEPATH = "resources/job.xsl";

    private static final String LOGOFILEPATH = "icons/logo.jpg";
    ...
    public static void generateHTMLFile(ExportFileResource resource) {
        try {
            ...
            final URL xslFileUrl = FileLocator.toFileURL(FileLocator.find(b, new Path(XSLFILEPATH), null));
            final URL logoFileUrl = FileLocator.toFileURL(FileLocator.find(b, new Path(LOGOFILEPATH), null));
            ...
            FileCopyUtils.copy(logoFilePath, picFolderPath + File.separatorChar + TALENDLOGOFILENAME);
    ...
    private static String checkPicDirIsExists(ExportFileResource resource, String tempFolderPath) {
        String picFolderPath = tempFolderPath + File.separator + PICDIRECTORY;
    ...
    private static void generateXMLFile(ExportFileResource resource, String tempFolderPath) throws Exception {
        ...
        try {
            ...
            output = new XMLWriter(new FileWriter(new File(tempFolderPath + File.separatorChar
                    + processItem.getProperty().getLabel() + ".xml")), format);
    ...
    private static void generateComponentsInfo(Element jobElement, ProcessItem processItem) {
        ...
        for (INode node : graphicalNodeList) {
            ...
            if (sourceList != null && sourceList.size() > 0) {
                ...
                Element inputElement = componentElement.addElement("input");
                inputElement.setText(EMPTYELEMENTVALUE);
            }
                ...
                for (int j = 0; j < metaDataList.size(); j++) {
                    ...
                    for (int k = 0; k < columnTypeList.size(); k++) {
                        ...
                        if (schemaElement == null) {
                            schemaElement = componentElement.addElement("schema");
                        }
    ...
    private static Element generateJobInfo(ProcessItem processItem, Element projectElement) {
        ...
        jobElement.addAttribute("creation", checkString(processItem.getProperty().getCreationDate().toLocaleString()));

        jobElement.addAttribute("modification", checkString(processItem.getProperty().getModificationDate()
                .toLocaleString()));

        Element previewElement = jobElement.addElement("preview");

        String picName = jobName + JOBPREVIEWPICSUFFIX;
        IPath filePath = RepositoryPathProvider.getPathFileName(RepositoryConstants.IMG_DIRECTORY_OF_JOB_OUTLINE,
                picName);
        picFilePathMap.put(picName, filePath.toOSString());
    ...
    private static Element generateProjectInfo(Document document) {
        ...
        projectElement.addAttribute("logo", PICTUREFOLDERPATH + TALENDLOGOFILENAME);
    ...
    protected static String getTmpFolder() {
        String tmpFold = System.getProperty("user.dir") + File.separatorChar + TEMPFOLDERNAME; //$NON-NLS-1$
        return tmpFold;
    }
