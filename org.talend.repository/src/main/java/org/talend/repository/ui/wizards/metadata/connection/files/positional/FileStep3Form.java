...
public class FileStep3Form extends AbstractPositionalFileStepForm {
	 ...
	 * @param Style
	 */
	public FileStep3Form(Composite parent, ConnectionItem connectionItem,
	...
	private ProcessDescription getProcessDescription(
			PositionalFileConnection originalValueConnection) {
		...
		if (originalValueConnection.isUseLimit()) {
			int i = ConnectionContextHelper
					.convertValue(originalValueConnection.getLimitValue());
	...
	public void refreshMetaDataTable(final CsvArray csvArray) {
		...
		String fieldSeparatorValues = getOriginalValueConnection()
		...
		for (int i = 0; i < numberOfCol; i++) {
			...
			if (firstRowToExtractMetadata == 1) {
				...
				label[i] = ColumnNameValidator.validateColumnNameFormat(
						label[i], i);
			...
			for (int f = firstRowToExtractMetadata; f < csvRows.size(); f++) {
				...
				if (fields.length > i) {
					...
					if (!value.equals("")) { //$NON-NLS-1$
						...
						if (LanguageManager.getCurrentLanguage() == ECodeLanguage.JAVA) {
							if (!JavaDataTypeHelper.getTalendTypeOfValue(value)
									.equals(globalType)) {
								globalType = JavaDataTypeHelper.getCommonType(
										globalType, JavaDataTypeHelper
												.getTalendTypeOfValue(value));
							}
						} else {
							if (!PerlDataTypeHelper.getTalendTypeOfValue(value)
									.equals(globalType)) {
								globalType = PerlDataTypeHelper.getCommonType(
										globalType, PerlDataTypeHelper
												.getTalendTypeOfValue(value));
							}
						}
			...
			if (fieldSeparatorValueArray[i] != null) {
				if (fieldSeparatorValueArray[i].equals("*")) { //$NON-NLS-1$
					...
					metadataColumn.setLength(Integer.valueOf(
							fieldSeparatorValueArray[i]).intValue());
				}
			}
	...
	private PositionalFileConnection getOriginalValueConnection() {
		if (getConnection().isContextMode() && getContextModeManager() != null) {
			return (PositionalFileConnection) FileConnectionContextUtils
