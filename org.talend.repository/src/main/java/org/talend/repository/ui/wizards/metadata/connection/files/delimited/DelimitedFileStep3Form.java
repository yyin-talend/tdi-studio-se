...
public class DelimitedFileStep3Form extends AbstractDelimitedFileStepForm {
	 ...
	 * @param Composite
	 */
	public DelimitedFileStep3Form(Composite parent,
	...
	private ProcessDescription getProcessDescription(
			DelimitedFileConnection originalValueConnection) {
		...
		if (originalValueConnection.isFirstLineCaption()) {
			int i = ConnectionContextHelper
					.convertValue(originalValueConnection.getHeaderValue());
	...
	protected void runShadowProcess() {
		...
		try {
			...
			if (Escape.CSV_LITERAL.equals(originalValueConnection
					.getEscapeType())) {
	...
	private DelimitedFileConnection getOriginalValueConnection() {
		if (isContextMode() && getContextModeManager() != null) {
			return (DelimitedFileConnection) FileConnectionContextUtils
