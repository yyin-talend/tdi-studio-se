...
public class DatabaseForm extends AbstractForm {
	 ...
	 * @param ISelection
	 */
	public DatabaseForm(Composite parent, ConnectionItem connectionItem,
	...
	public void initialize() {
		Integer indexOfCombo = urlDataStringConnection
				.getIndexOfLabel(getConnection().getDatabaseType());
		if (indexOfCombo > -1) {
		...
		getConnection().setSqlSynthax(
				Messages.getString("DatabaseForm.sqlSyntax")); //$NON-NLS-1$
		sqlSyntaxCombo
				.select(getSqlSyntaxIndex(getConnection().getSqlSynthax()));
	...
	private void checkAS400SpecificCase() {
		if (getConnection().isStandardSQL() == getConnection().isSystemSQL()) { // create
			// connection
			boolean b = CorePlugin.getDefault().getPreferenceStore()
	...
	protected void addFields() {
		int width = getSize().x;
		GridLayout layout2;
		databaseSettingGroup = Form.createGroup(this, 1, Messages
				.getString("DatabaseForm.groupDatabaseSettings"), 310);
		compositeGroupDbSettings = Form.startNewGridLayout(
				databaseSettingGroup, 1);
		...
		Label label = new Label(compositeDbSettings, SWT.SEPARATOR
				| SWT.H_SCROLL);
	...
	private void addFieldsForTypeDB(Composite compositeGroupDbSettings) {
		...
		typeDbCompositeParent = new Composite(compositeGroupDbSettings,
				SWT.NULL);
		...
		dbVersionCombo = new LabelledCombo(typeDbCompositeParent, Messages
				.getString("DatabaseForm.dbversion"), Messages
				.getString("DatabaseForm.dbversion.tip"), versions, 2, true);
		dbVersionCombo.select(0);

		// Field connectionString
		urlDataStringConnection.setSelectionIndex(dbTypeCombo
				.getSelectionIndex());
		urlConnectionStringText = new LabelledText(typeDbCompositeParent,
				Messages.getString("DatabaseForm.stringConnection"), 2); //$NON-NLS-1$
		urlConnectionStringText.setEditable(false);

		// Field login & password
		usernameText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.login"), 2); //$NON-NLS-1$
		passwordText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.password"), 2, SWT.BORDER //$NON-NLS-1$
				| SWT.SINGLE | SWT.PASSWORD);

		// Another fields
		serverText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.server"), 2); //$NON-NLS-1$
		portText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.port"), 2); //$NON-NLS-1$
		// portText.setTextLimit(5);
		sidOrDatabaseText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.database"), 2); //$NON-NLS-1$
		schemaText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.schema"), 2); //$NON-NLS-1$
		datasourceText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.dataSource"), 2); //$NON-NLS-1$
		additionParamText = new LabelledText(typeDbCompositeParent, Messages
				.getString("DatabaseForm.AddParams"), 2); //$NON-NLS-1$

		String[] extensions = { "*.*" }; //$NON-NLS-1$
		fileField = new LabelledFileField(typeDbCompositeParent, Messages
				.getString("DatabaseForm.mdbFile"), extensions); //$NON-NLS-1$
		directoryField = new LabelledDirectoryField(typeDbCompositeParent,
				"DB Root Path"); //$NON-NLS-1$
	}
	...
	private void addFieldsForGeneralDB(Composite parent) {
		...
		generalJdbcUrlText = new LabelledText(generalDbCompositeParent,
				Messages.getString("DatabaseForm.general.url"), 2); //$NON-NLS-1$

		generalJdbcDriverjarText = new LabelledText(generalDbCompositeParent,
				Messages.getString("DatabaseForm.general.jarfile"), 1);
		...
		generalJdbcUserText = new LabelledText(generalDbCompositeParent,
				Messages.getString("DatabaseForm.general.user"), 2); //$NON-NLS-1$
		...
		generalMappingFileText = new LabelledText(generalDbCompositeParent,
				Messages.getString("DatabaseForm.general.mapping"), 1);
	 ...
	 * @param compositeGroupDbSettings
	 */
	private void addCheckAndStandardButtons(int width,
			Composite compositeGroupDbSettings) {
		...
		directoryField.hide();
		// Button Check
		Composite compositeCheckButton = Form.startNewGridLayout(
				compositeGroupDbSettings, 1, false, SWT.CENTER, SWT.TOP);
		layout2 = (GridLayout) compositeCheckButton.getLayout();
		layout2.marginHeight = 0;
		layout2.marginTop = 0;
		layout2.marginBottom = 0;
		checkButton = new UtilsButton(compositeCheckButton, Messages
				.getString("DatabaseForm.check"), WIDTH_BUTTON_PIXEL, //$NON-NLS-1$
				HEIGHT_BUTTON_PIXEL);
		checkButton.setEnabled(false);

		// Group Database Properties
		Group group1 = Form.createGroup(this, 1, Messages
				.getString("DatabaseForm.groupDatabaseProperties")); //$NON-NLS-1$
		GridData gridData = new GridData(GridData.FILL_HORIZONTAL);
		...
		Composite compositeGroupDbProperties = Form
				.startNewDimensionnedGridLayout(group1, 4, width, 70);

		// PTODO !StandBy! (use width SQL Editor): to define the values of SQL
		// Syntax (need by SQL Editor)
		String[] item = { "SQL 92" }; //$NON-NLS-1$
		sqlSyntaxCombo = new LabelledCombo(compositeGroupDbProperties, Messages
				.getString("DatabaseForm.sqlSyntax"), null, item, //$NON-NLS-1$
				3);

		stringQuoteText = new LabelledText(compositeGroupDbProperties, Messages
				.getString("DatabaseForm.stringQuote"), false); //$NON-NLS-1$
		nullCharText = new LabelledText(compositeGroupDbProperties, Messages
				.getString("DatabaseForm.nullChar"), false); //$NON-NLS-1$
	...
	/**
	 * DOC YeXiaowei Comment method "addDBSelectCombo". Extract method form
	 * addFields()
	 */
	private void addDBSelectCombo() {
		...
		if (LanguageManager.getCurrentLanguage() == ECodeLanguage.PERL) {
			Collection<String> databasePerl = new ArrayList<String>(Arrays
					.asList(urlDataStringConnection.getItem()));
			...
			String[] dbPerl = databasePerl.toArray(new String[databasePerl
					.size()]);
			dbTypeCombo = new LabelledCombo(compositeDbSettings, Messages
					.getString("DatabaseForm.dbType"), Messages //$NON-NLS-1$
					.getString("DatabaseForm.dbTypeTip"), dbPerl, 2, true); //$NON-NLS-1$
		} else {
			dbTypeCombo = new LabelledCombo(
					compositeDbSettings,
					Messages.getString("DatabaseForm.dbType"), Messages //$NON-NLS-1$
	...
	private void checkDBTypeAS400() {
		if (dbTypeCombo.getSelectionIndex() == 16
				|| dbTypeCombo.getSelectionIndex() == 25
	...
	private void checkConnection() {
		...
		if (connectionItem.getConnection() instanceof DatabaseConnection) {
			DatabaseConnection c = (DatabaseConnection) connectionItem
					.getConnection();
			if (c.getProductId().equals(
		...
		if (isContextMode()) { // context mode
			...
			String urlStr = DBConnectionContextUtils
					.setManagerConnectionValues(managerConnection,
							connectionItem, dbTypeCombo.getItem(dbTypeCombo
									.getSelectionIndex()), dbTypeCombo
									.getSelectionIndex());
			if (urlStr == null) {
			...
			// set the value
			managerConnection.setValue(0, dbTypeCombo.getItem(dbTypeCombo
					.getSelectionIndex()), isGeneralJDBC() ? generalJdbcUrlText
					.getText() : urlConnectionStringText.getText(), serverText
					.getText(), isGeneralJDBC() ? generalJdbcUserText.getText()
					: usernameText.getText(),
					isGeneralJDBC() ? generalJdbcPasswordText.getText()
							: passwordText.getText(), sidOrDatabaseText
							.getText(), portText.getText(),
					fileField.getText(), datasourceText.getText(),
					isGeneralJDBC() ? "" : schemaText.getText(),
					additionParamText.getText(), generalJdbcClassNameText
							.getText(), generalJdbcDriverjarText.getText(),
					oracleVersionEnable() ? dbVersionCombo.getText() : null);

			managerConnection.setDbRootPath(directoryField.getText());

		}
		managerConnection.setValueProperties(sqlSyntaxCombo
				.getItem(sqlSyntaxCombo.getSelectionIndex()), stringQuoteText
		...
		if (databaseSettingIsValide) {
			...
			MessageDialog.openInformation(getShell(), Messages
					.getString("DatabaseForm.checkConnectionTitle"), "\"" //$NON-NLS-1$ //$NON-NLS-2$
					+ connectionItem.getProperty().getLabel()
					+ "\" " + Messages.getString("DatabaseForm.checkIsDone")); //$NON-NLS-1$ //$NON-NLS-2$
			if (!isReadOnly()) {
				if (isContextMode()) {
					adaptFormToEditable();
				} else {
			...
			new ErrorDialogWidthDetailArea(getShell(), PID, mainMsg,
			...
			public void handleEvent(final Event e) {
				if (!isContextMode()) {
					if (dbTypeCombo.getSelectionIndex() >= 0) {
						...
						updateStatus(IStatus.ERROR, Messages.getString(
								"DatabaseForm.alert", dbTypeCombo.getLabel())); //$NON-NLS-1$
					}
				}
			}
		});
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					if (urlConnectionStringText.getEditable()) {
						if (!databaseSettingIsValide) {
							updateStatus(IStatus.INFO, Messages
									.getString("DatabaseForm.checkInformation")); //$NON-NLS-1$
						}
						databaseSettingIsValide = false;
						checkButton.setEnabled(true);
						String[] s = urlDataStringConnection
								.getAnalyse(urlConnectionStringText.getText());
	...
	protected void addFieldsListeners() {
				...
				if (!isContextMode()) {
					...
					if (getConnection().getDatabaseType().equals("Ingres")) { //$NON-NLS-1$
						b = Pattern
								.matches(
										Messages
												.getString("DatabaseForm.ingresDBRegex"), portText.getText()); //$NON-NLS-1$
					} else {
						b = Pattern
								.matches(
										Messages
												.getString("DatabaseForm.otherDBRegex"), portText.getText()); //$NON-NLS-1$
					}
					if (b) {
		...
		datasourceText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					if (!urlConnectionStringText.getEditable()) {
						getConnection().setDatasourceName(
		...
		dbVersionCombo.addModifyListener(new ModifyListener() {
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					getConnection()
							.setDbVersionString(dbVersionCombo.getText());
					checkFieldsValue();
				}
			}
		});
		...
		additionParamText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					if (!urlConnectionStringText.getEditable()) {
						getConnection().setAdditionalParams(
		...
		standardButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (!isContextMode()) {
					getConnection().setStandardSQL(
							standardButton.getSelection());
			...
			public void modifyText(final ModifyEvent e) {
				...
				if (!isContextMode()) {
					...
					final String product = EDatabaseTypeName
							.getTypeFromDisplayName(
									getConnection().getDatabaseType())
							.getProduct();
					...
					if (product == null || product.equals("General JDBC")) {
						...
						if (MetadataTalendType
								.getDefaultDbmsFromProduct(product) != null) {
							mapping = MetadataTalendType
					...
					if (dbTypeCombo.getSelectionIndex() == 0) {
						...
						// mySQlDefaultValue);
						additionParamText
								.setText(DataStringConnection.mySQlDefaultValue);
					}
					if (dbTypeCombo.getSelectionIndex() == 16) {
						additionParamText
								.setText(DataStringConnection.as400DefaultValue);
					}
					checkAS400SpecificCase();
					checkFieldsValue();
			...
			public void handleEvent(final Event e) {
				if (!isContextMode()) {
					if (dbTypeCombo.getSelectionIndex() == -1) {
						...
						if (urlDataStringConnection
								.getStringConnectionTemplate().contains(
										"<filename>")) { //$NON-NLS-1$
							setPropertiesFormEditable(true);
							urlConnectionStringText.setEditable(false);
						}
						getConnection().setFileFieldName(
								PathUtils.getPortablePath(fileField.getText()));
					}
				}
			}
		});
	...
	private void addGeneralDbFieldsListeners() {
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					if (validText(generalJdbcClassNameText.getText())) {
						getConnection().setDriverClass(
								generalJdbcClassNameText.getText());
						checkFieldsValue();
					}
				}
			}
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					if (validText(generalJdbcDriverjarText.getText())) {
						getConnection().setDriverJarPath(
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					if (validText(generalJdbcPasswordText.getText())) {
						getConnection().setPassword(
								generalJdbcPasswordText.getText());
						checkFieldsValue();
					}
				}
			}
			...
			public void modifyText(final ModifyEvent e) {
				if (!isContextMode()) {
					if (validText(generalMappingFileText.getText())) {
						getConnection().setDbmsId(
								generalMappingFileText.getText());
						checkFieldsValue();
					}
				}
			}
		...
		browseJarFilesButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				dialog.setFilterPath(userDir + pathSeparator + "lib"
						+ pathSeparator + "java");
		...
		mappingSelectButton.addSelectionListener(new SelectionAdapter() {
			...
			@Override
			public void widgetSelected(SelectionEvent e) {
				MappingFileSelectDialog dialog = new MappingFileSelectDialog(
						getShell());
				dialog.open();
	...
	public boolean checkFieldsValue() {
		...
		databaseSettingIsValide = false;
		urlConnectionStringText.setText(getStringConnection());
		urlConnectionStringText.setToolTipText(urlDataStringConnection
				.getStringConnectionTemplate());
		updateCheckButton();
		setPropertiesFormEditable(false);
		...
		if (sqlSyntaxCombo.getSelectionIndex() == -1) {
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", sqlSyntaxCombo.getLabel())); //$NON-NLS-1$
			return false;
		}
		if (nullCharText.getCharCount() == 0) {
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", nullCharText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		if (stringQuoteText.getCharCount() == 0) {
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", stringQuoteText.getLabelText())); //$NON-NLS-1$
			return false;
		}
	...
	private boolean checkGeneralDBFieldValues() {
		...
		if (!validText(value)) {
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", generalJdbcUrlText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		...
		if (!validText(value)) {
			updateStatus(
					...
					Messages
							.getString(
									"DatabaseForm.alert", generalJdbcDriverjarText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		...
		if (!validText(value)) {
			updateStatus(
					...
					Messages
							.getString(
									"DatabaseForm.alert", generalJdbcClassNameText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		...
		if (!validText(value)) {
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", generalJdbcUserText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		...
		if (!validText(value)) {
			updateStatus(
					...
					Messages
							.getString(
									"DatabaseForm.alert", generalJdbcPasswordText.getLabelText())); //$NON-NLS-1$
			return false;
		}
	...
	private boolean checkTypeDBFieldValues() {
		...
		if (s.contains("<host>") && serverText.getCharCount() == 0) { //$NON-NLS-1$
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", serverText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		if (s.contains("<port>") && portText.getCharCount() == 0) { //$NON-NLS-1$
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", portText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		if ((s.contains("<sid>") || s.contains("<service_name>")) && sidOrDatabaseText.getCharCount() == 0) { //$NON-NLS-1$ //$NON-NLS-2$
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", sidOrDatabaseText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		if (s.contains("<filename>") && fileField.getText() == "") { //$NON-NLS-1$ //$NON-NLS-2$
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", fileField.getLabelText())); //$NON-NLS-1$
			return false;
		}
		if (s.contains("<datasource>") && datasourceText.getText() == "") { //$NON-NLS-1$ //$NON-NLS-2$
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", datasourceText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		if (s.contains("<dbRootPath>") && directoryField.getText() == "") { //$NON-NLS-1$ //$NON-NLS-2$
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", directoryField.getLabelText())); //$NON-NLS-1$
			return false;
		}

		if (urlDataStringConnection.isSchemaNeeded()
				&& schemaText.getCharCount() == 0) {
			updateStatus(IStatus.WARNING, Messages.getString(
					"DatabaseForm.alert", schemaText.getLabelText())); //$NON-NLS-1$
			return false;
		}
		updateStatus(IStatus.OK, null);
		return true;
	...
	private String getStringConnection() {
		String s = null;
		if (isContextMode()) {
			s = DBConnectionContextUtils.getUrlConnectionString(
					dbTypeCombo.getSelectionIndex(), connectionItem, true)
					.getUrlConnectionStr();
		} else {
			s = urlDataStringConnection.getString(dbTypeCombo
					.getSelectionIndex(), serverText.getText(), usernameText
					.getText(), passwordText.getText(), portText.getText(),
					sidOrDatabaseText.getText(), fileField.getText()
							.toLowerCase(), datasourceText.getText(),
					directoryField.getText(), additionParamText.getText());
		}
	...
	private void updateCheckButton() {
		...
		if (isContextMode()) {
			...
			checkButton.setEnabled((dbTypeCombo.getSelectionIndex() >= 0)
					&& (getStringConnection() != urlDataStringConnection
							.getStringConnectionTemplate()));
			if (isGeneralJDBC()) {
				checkButton.setEnabled(true);
			}
	...
	private void setPropertiesFormEditable(boolean visible) {
		...
		// update the UI label
		if (urlDataStringConnection.isSchemaNeeded()) {
			if (urlDataStringConnection.getStringConnectionTemplate().contains(
					"(description=(address=(protocol=tcp)")) { //$NON-NLS-1$
				sidOrDatabaseText.setLabelText(Messages
						.getString("DatabaseForm.service_name")); //$NON-NLS-1$
				sidOrDatabaseText.setLabelWidth(65);
				sidOrDatabase = EDBParamName.ServiceName;
			} else {
				sidOrDatabaseText.setLabelText(Messages
						.getString("DatabaseForm.sid")); //$NON-NLS-1$
				sidOrDatabase = EDBParamName.Sid;
			}
		} else {
			sidOrDatabaseText.setLabelText(Messages
					.getString("DatabaseForm.database")); //$NON-NLS-1$
		...
		if (dbTypeCombo.getSelectionIndex() < 0) {
			...
			if (s.contains("<filename>")) { // && //$NON-NLS-1$
				...
				if (EDatabaseTypeName.getTypeFromDisplayName(
						getConnection().getDatabaseType()).equals(
	...
	private boolean oracleVersionEnable() {
		...
		return dbTypeCombo.getText().startsWith("Oracle")
				&& LanguageManager.getCurrentLanguage().equals(
						ECodeLanguage.JAVA);
	}
	...
	public void setVisible(boolean visible) {
		...
		if (isContextMode()) {
			...
			setPropertiesFormEditable((urlDataStringConnection
					.getIndexOfLabel(getConnection().getDatabaseType()) > -1));
		}
	...
	protected void evaluateTextField(boolean b) {
		...
		if (b) {
			...
			updateStatus(IStatus.ERROR, Messages
					.getString("DatabaseForm.portError")); //$NON-NLS-1$
