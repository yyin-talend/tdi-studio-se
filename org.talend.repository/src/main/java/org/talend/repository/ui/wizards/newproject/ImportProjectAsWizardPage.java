...
public class ImportProjectAsWizardPage extends WizardPage {
	...
	public ImportProjectAsWizardPage() {
		...
		IBrandingService brandingService = (IBrandingService) GlobalServiceRegister
				.getDefault().getService(IBrandingService.class);
		setTitle(Messages.getString("ImportProjectAsWizardPage.title")); //$NON-NLS-1$
		setDescription(Messages
				.getString(
						"ImportProjectAsWizardPage.description", brandingService.getShortProductName())); //$NON-NLS-1$
	...
	private void createNamesFields(Composite workArea) {
		...
		technicalNameLab.setText(Messages
				.getString("NewProjectWizardPage.technicalName")); //$NON-NLS-1$
	...
	private void createProjectsRoot(Composite workArea) {
		...
		projectFromDirectoryRadio.setText(Messages
				.getString("ImportProjectAsWizardPage.form.selectDirectory")); //$NON-NLS-1$
		...
		browseDirectoriesButton.setText(Messages
				.getString("ImportProjectAsWizardPage.form.browse")); //$NON-NLS-1$
		setButtonLayoutData(browseDirectoriesButton);

		// new project from archive radio button
		projectFromArchiveRadio = new Button(projectGroup, SWT.RADIO);
		projectFromArchiveRadio.setText(Messages
				.getString("ImportProjectAsWizardPage.form.selectArchive")); //$NON-NLS-1$
		...
		browseArchivesButton.setText(Messages
				.getString("ImportProjectAsWizardPage.form.browse")); //$NON-NLS-1$
	...
	private void handleLocationDirectoryButtonPressed() {
		...
		dialog
				.setMessage(Messages
						.getString("ImportProjectAsWizardPage.dialog.selectDirectory.desc")); //$NON-NLS-1$
	...
	private void handleLocationArchiveButtonPressed() {
		...
		dialog
				.setText(Messages
						.getString("ImportProjectAsWizardPage.dialog.selectArchive.title")); //$NON-NLS-1$
	...
	private void evaluateSpecifiedPath(String path) {
		...
		if (!dirSelected && ArchiveFileManipulations.isTarFile(path)) {
			...
			TarLeveledStructureProvider provider = new TarLeveledStructureProvider(
					sourceTarFile);
			Object child = provider.getRoot();

			if (!ImportProjectsUtilities.collectProjectFilesFromProvider(
					projectFiles, provider, child, 0, null,
					IProjectDescription.DESCRIPTION_FILE_NAME)) {
				return;
			}
			if (!ImportProjectsUtilities.collectProjectFilesFromProvider(
					talendProjectFiles, provider, child, 0, null,
			...
			ZipLeveledStructureProvider provider = new ZipLeveledStructureProvider(
					sourceFile);
			Object child = provider.getRoot();
			...
			if (!ImportProjectsUtilities.collectProjectFilesFromDirectory(
					projectFiles, directory, null,
					IProjectDescription.DESCRIPTION_FILE_NAME)) {
				return;
			}
			if (!ImportProjectsUtilities.collectProjectFilesFromDirectory(
					talendProjectFiles, directory, null,
					ImportProjectsUtilities.TALEND_PROJECT_FILE_NAME)) {
				return;
			}
			if (!ImportProjectsUtilities.collectProjectFilesFromDirectory(
		...
		if (projectFiles.size() != 1 || talendProjectFiles.size() != 1) {
			IBrandingService brandingService = (IBrandingService) GlobalServiceRegister
					.getDefault().getService(IBrandingService.class);
			String string = Messages.getString(
					"ImportProjectAsWizardPage.error.notATalendProject", //$NON-NLS-1$
					brandingService.getShortProductName());
			fileContentStatus = new Status(IStatus.ERROR,
					RepositoryPlugin.PLUGIN_ID, IStatus.OK, string, null);
		} else {
			fileContentStatus = createOkStatus();
		}
	...
	private TarFile getSpecifiedTarSourceFile(String fileName) {
		...
		try {
			...
			displayErrorDialog(Messages
					.getString("ImportProjectAsWizardPage.error.zip")); //$NON-NLS-1$
		} catch (IOException e) {
			displayErrorDialog(Messages
					.getString("ImportProjectAsWizardPage.error.zip")); //$NON-NLS-1$
		}
	...
	private boolean isProjectNameAlreadyUsed(String newProjectName) {
		...
		for (Project project : projects) {
			if (Project.createTechnicalName(newProjectName).compareTo(
					project.getTechnicalLabel()) == 0) {
				return true;
			}
		}
		return false;
	...
	protected void checkFieldsValue() {
		...
		if (nameText.getText().length() == 0) {
			nameStatus = new Status(IStatus.ERROR, RepositoryPlugin.PLUGIN_ID,
					IStatus.OK, Messages
							.getString("NewProjectWizardPage.nameEmpty"), null); //$NON-NLS-1$
		} else {
			technicalNameText.setText(Project.createTechnicalName(nameText
					.getText()));
			if (!Pattern.matches(RepositoryConstants.PROJECT_PATTERN, nameText
					.getText())) {
				nameStatus = new Status(
						IStatus.ERROR,
