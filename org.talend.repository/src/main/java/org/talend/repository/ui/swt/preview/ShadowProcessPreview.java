 ...
 * 
 * $Id:ShadowProcessPreview.java 1185 2006-12-22 10:43:29 +0000 (星期五, 22 十二月
 * 2006) amaumont $
 * 
 */
public class ShadowProcessPreview {

	protected int maximumRowsToPreview = CorePlugin.getDefault()
			.getPreferenceStore()
	 ...
	 *            (null or path)
	 */
	public ShadowProcessPreview(Composite composite, String filePath,
	 ...
	 */
	public void refreshTablePreview(final CsvArray csvArray,
			final boolean firstRowIsLabel) {
		...
		for (int i = 0; i < numbersOfColumns; i++) {
			if (firstRowIsLabel) {
				if (numbersOfColumns <= firstRowFields.length) {
					if (firstRowFields[i] != null
							&& !("").equals(firstRowFields[i])) { //$NON-NLS-1$[
						title[i] = firstRowFields[i];
					} else {
						title[i] = Messages
								.getString("DelimitedFilePreview.column") + " " + i; //$NON-NLS-1$ //$NON-NLS-2$
					}
				} else {
	 ...
	 */
	public void refreshTablePreview(final CsvArray csvArray,
			final boolean firstRowIsLabel, ProcessDescription processDescription) {
	 ...
	 */
	public void refreshTablePreview(final CsvArray csvArray,
			final boolean firstRowIsLabel, List<SchemaTarget> schemaTarget) {
		List<String[]> csvRows = csvArray.getRows();
		...
		for (int i = 0; i < numbersOfColumns; i++) {
			if (firstRowIsLabel) {
				...
				if (schemaTarget.get(i).getTagName() != null
						&& !schemaTarget.get(i).getTagName().equals("")) { //$NON-NLS-1$
	 ...
	 * @param firstRowIsLabel
	 */
	private void refreshPreviewItem(final List<String[]> csvRows,
			final boolean firstRowIsLabel) {
