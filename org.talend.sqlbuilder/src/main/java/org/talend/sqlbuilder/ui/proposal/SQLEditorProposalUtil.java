...
public class SQLEditorProposalUtil {
	...
	public SQLEditorProposalUtil(RepositoryNode session, String language) {
		...
		dbType = SQLBuilderRepositoryNodeManager
				.getDbTypeFromRepositoryNode(session);
	...
	public IContentProposal[] getSQLEditorContentProposals(String content,
		...
		try {
			...
			if (queryStrings.isEmpty()
					|| (curSql[1].length() == 0 && curSql[0].length() == 0)) {
	...
	private String[] getCurrentSqlQuery(String content) {
		...
		if (before != null && before.trim().length() != 0) {
			if (before.lastIndexOf(";") == -1) {
				...
				curSql[0] = before.substring(before.lastIndexOf(";") + 1,
						position).replaceAll("\n", "");
			}
		} else {
			curSql[0] = "";
	...
	private String[] getAllKeywords() {
		...
		String[] keywords = mode.getDefaultRuleSet().getKeywords().get(
				"KEYWORD1");
		return keywords;
	}
	...
	private void getAllProposalString() {
		tableAndColumns = SQLBuilderRepositoryNodeManager
				.getAllNamesByRepositoryNode(session);
			...
			for (String string : list) {
				...
				if (index > -1) {
					tmp2 = string.substring(index + 2, string.length() - 1)
							.replaceAll("\"", "");
					if (index2 > index) {
						column = string.substring(index2 + 2,
								string.length() - 1).replaceAll("\"", "");
						if (hasIndex == -1) {
							tmp2 = "";
						}
					}
	 ...
	 * @return
	 */
	private SQLEditorAllProposal createAllProposal(String hasInput,
			String string) {
		SQLEditorAllProposal p = new SQLEditorAllProposal(hasInput, string,
				position, contents, dbType);
		return p;
	}
	...
	private List<String> getColumnsByTableName(String tableName) {
		...
		for (String string : alltablenames) {
			tables.put(string.substring(string.indexOf(".") + 2, string
					.length() - 1), string);
		}
		List<String> columns = tableAndColumns.get(tables.get(tableName
				.replaceAll("\"", "").trim()));
		return columns;
	}
}
