...
class ExtendedCompletionProposal implements ICompletionProposal {
	...
	public ExtendedCompletionProposal(String proposalsString, int i, int j,
			int k, Image tmpImage, String str, TableNode tb) {
		compProposal = new CompletionProposal(proposalsString, i, j, k,
				tmpImage, str, null, null);
		this.tn = tb;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.jface.text.contentassist.ICompletionProposal#geString[] proposalsString = dictionary.matchTablePrefix(lastPart
						.toLowerCase());
				ArrayList propList = new ArrayList();
				for (int i = 0; i < proposalsString.length; i++) {
					ArrayList ls = dictionary
							.getTableObjectList(proposalsString[i]);
					for (int j = 0; j < ls.size(); j++) {
						TableNode tbNode = (TableNode) ls.get(j);
						Image tmpImage = null;
						TableFolderNode totn = (TableFolderNode) tbNode
								.getParent();
						...
						if (catSchema == node) {
							...
							ICompletionProposal cmp = new ExtendedCompletionProposal(
									proposalsString[i], documentOffset
											- lastPart.length(), lastPart
											.length(), proposalsString[i]
											.length(), tmpImage,
				...
				ICompletionProposal[] res = new ICompletionProposal[propList
						.size()];
				System
						.arraycopy(propList.toArray(), 0, res, 0, propList
								.size());
				return res;tContextInformation()
	 */
...
public class SQLCompletionProcessor implements IContentAssistProcessor {
		...
		if (string.charAt(length - 1) == ' ') {
			...
			if (st != null) {
				...
				for (int i = 0; i < arr.length; i++) {
					result[i] = new CompletionProposal(arr[i], documentOffset,
							0, arr[i].length(), colImage, arr[i], null,
							tableDesc);
				}
				return result;
			}
			...
			TreeSet st = (TreeSet) dictionary
					.getColumnListByTableName(firstPart);
			if (st != null) {
				...
				while (iter.hasNext()) {
					...
					if (colName.length() >= length2) {
						if ((colName.substring(0, lastPart.length()))
								.equalsIgnoreCase(lastPart)) {
							CompletionProposal cmp = new CompletionProposal(
									colName, documentOffset - length2, length2,
									colName.length(), colImage, colName, null,
									null);
							propList.add(cmp);
						}
					}
						...
						for (int j = 0; j < tables.length; j++) {
							...
							if (tables[j] instanceof TableNode) {
								if (((TableNode) tables[j]).isTable()) {
									tmpImage = tableImage;
								} else if (((TableNode) tables[j])
										.isView()) {
									tmpImage = viewImage;
								}
								propList
										.add(new ExtendedCompletionProposal(
												tableName,
												documentOffset, 0,
												tableName.length(),
												tmpImage, tableName,
	...
	private ICompletionProposal[] inputNotContainDot(int documentOffset, String string, int length) {
		...
		for (int i = 0; i < keywordProposal.length; i++) {
			resKey[i] = new CompletionProposal(keywordProposal[i],
					documentOffset - length, length, keywordProposal[i]
							.length(), keywordImage, keywordProposal[i],
					null, null);
		}

		String[] proposalsString = dictionary.matchTablePrefix(string
				.toLowerCase());
		...
		for (int i = 0; i < proposalsString.length; i++) {
			...
			for (int j = 0; j < ls.size(); j++) {
				...
				ICompletionProposal cmp = new ExtendedCompletionProposal(
						proposalsString[i], documentOffset - length,
						length, proposalsString[i].length(), tmpImage,
						proposalsString[i], tbNode);
				propList.add(cmp);

			}
		}
		String[] proposalsString2 = dictionary
				.matchCatalogSchemaPrefix(string.toLowerCase());
		ICompletionProposal[] resKey2 = new ICompletionProposal[proposalsString2.length];
		for (int i = 0; i < proposalsString2.length; i++) {
			resKey2[i] = new CompletionProposal(proposalsString2[i],
					documentOffset - length, length, proposalsString2[i]
							.length(), catalogImage, proposalsString2[i],
					null, null);
		}

		ICompletionProposal[] res = new ICompletionProposal[propList.size()
				+ keywordProposal.length + resKey2.length];
		System.arraycopy(resKey, 0, res, 0, resKey.length);
		System.arraycopy(propList.toArray(), 0, res, resKey.length,
				propList.size());
		System.arraycopy(resKey2, 0, res, resKey.length + propList.size(),
				resKey2.length);
		Arrays.sort(res, new ICompletionProposalComparator());
		return res;
	}
